...
public class Account
{
  ...
  private long Time_richesse;
  private long Time_prosp;
  private long Time_expl;
  private long Time_tresor;
  private Player currentPlayer;
  private GameClient gameClient;
  private byte state;
  private int vip;
  private long Time_dj;
  private long Time_pxp;
  private long Time_pdrop;
  private int event = 0;
	public boolean enMapaPortal = false;
...
public Account(int guid, String name, String pseudo, String answer, boolean banned, String lastIp,
		...
		long muteTime, String mutePseudo, int vip , long dj, long pxp, long pdrop, String uuid, String uuidNew, long pexpl, long ptresor, long pprosp, long prichesse)
  {
    ...
    this.Time_dj = dj;
    this.Time_pxp = pxp;
    this.Time_pdrop = pdrop;
    this.Time_expl = pexpl;
    this.Time_tresor = ptresor;
    this.Time_prosp = pprosp;
    this.Time_richesse = prichesse;
...
public boolean isBank_load() {
  ...
  public long getTime_dj() {
	...
	public void setTime_dj(long time_dj) {
	 ...
	 public long getTime_pxp() {
		  long remaining=this.Time_pxp-System.currentTimeMillis();
		    return remaining<=0L ? 0L : remaining;
		}

		public void setTime_pxp(long time_pxp) {
			Time_pxp = time_pxp;
		}
		 public long getTime_pdrop() {
			  long remaining=this.Time_pdrop-System.currentTimeMillis();
			    return remaining<=0L ? 0L : remaining;
			}

			public void setTime_pdrop(long time_pdrop) {
  ...
  public void setUUIDNew(String i)
  {
	  uuidNew=i;
  }
  ...
  {
    points=Database.getStatics().getAccountData().loadPoints(this.name);
    return points;
  }
  public int getvip()
  {
  ...
  public boolean gettimevoterpg()
  {
	...
	/////////////////////////Système PNJ SEMILIKE EXCLU//////////////////////////////////////////////////////
	
	 public long getTime_richesse() {
		  long remaining=this.Time_richesse-System.currentTimeMillis();
		    return remaining<=0L ? 0L : remaining;
		}

		public void setTime_richesse(long time_richesse) {
			Time_richesse = time_richesse;
		}
	
		 public long getTime_prosp() {
			  long remaining=this.Time_prosp-System.currentTimeMillis();
			    return remaining<=0L ? 0L : remaining;
			}

			public void setTime_prosp(long time_prosp) {
				Time_prosp = time_prosp;
			}
	
			 public long getTime_expl() {
				  long remaining=this.Time_expl - System.currentTimeMillis();
				    return remaining<=0L ? 0L : remaining;
				}

				public void setTime_expl(long time_expl) {
					Time_expl = time_expl;
				}
				
				 public long getTime_tresor() {
					  long remaining=this.Time_tresor-System.currentTimeMillis();
					    return remaining<=0L ? 0L : remaining;
					}

					public void setTime_tresor(long time_tresor) {
						Time_tresor = time_tresor;
					}
  ...
  {
    points=i;
    Database.getStatics().getAccountData().updatePoints(this.id,points);
  }
...
public int getAccept() {
	accept = Database.getStatics().getAccountData().loadAccept(name);
	return accept;
}
public void setAccept(int accept) {
	 Database.getStatics().getAccountData().updateAccept(id);  
		this.accept = accept;
}
}
