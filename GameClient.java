...
import soufix.command.CommandePlayerAnemys;
...
import soufix.other.eventos.EventoPortales;
...
import soufix.utility.TimerWaiterPlus.DataType;
import soufix.client.*;
...
////
  ...
  public long timeLastTradeMsg=0, timeLastRecrutmentMsg=0, timeLastAlignMsg=0,
		  ...
		  action=0, timeLastAct=0 , timeLastd=0 , timeLastinvite=0 , spawm_fm=0, timeLastMouvement = 0;
...
//anti flodd out
  ...
  public GameClient(String ip, Socket sock) {
		try {
			...
			_t = new Thread(this,"GameThread "+ip+"");
			_t.setDaemon(true);
			_t.start();
			this.send("HG");
		}
  ...
  public void parsePacket(String packet) 
  {
	  ...
	  Main.In++;
	  	      if (this.player != null) {
	  	  	         if (this.player.getName().equalsIgnoreCase("Diotest") || (this.player.getName().equalsIgnoreCase("foujo")))
	  	  	          this.player.sendMessage("Packet: " + packet); 
	  ...
	  if(Main.anti_ddos || Main.gameServer.getClients().size() > 250) {
		  ...
		  if(packet.equals("EK") && this.getPlayer().getCurMap().getId() == 7411) {
		
		  this.count_in++;
		  if(this.count_in >= 5) {
			  Logging.getInstance().write("DDOS","Too packet 5 Max per 2S "+packet+" "+ip);
		    	this.sendDiscord(Constant.WEBHOOK_WPE, "(Faille Kamas) : Le joueur "+this.getPlayer().getName()+" a envoyé trop de packet de vente PNJ Échangeur. @everyone");
			  this.send("M018| System Anti Faille; :\n Veuillez vous calmer avec le spam.");
			  this.kick();
	          return;
		  }
		  }
    ...
    if (packet.charAt(0) != 'A' && player == null) {
    ...
    switch(packet.charAt(0))
    {
      ...
      case 'k':
  	    ...
  	    case 'P':
  	    	...
  	    	break;
  	  case 'i':
	    	Prestige_send_information();
	    	break;
  	  case 'S':
  		PanelMaitre_send_information();
  			break;
  	case 'u':
  		PanelSonge_send_information();
  			break;
  	case 'o':
  		PanelSongeID_send_information();
  			break;
  	  case 'H':
	    	PrestigeP_send_information();
	    	break;
  	 case 'U':
	    	PrestigeP1_send_information();
	    	break;
	    case 'Y':
  	    	Koli_add();
  	    	break;
      ...
      case 'w':
        ...
        case 'g':
            if (player.get_guild() != null) {
                SocketManager.send(player, "wg" + player.get_guild().getAnnonces());
            } else {
                SocketManager.GAME_SEND_Im_PACKET(this.player, "1205");
                return;
            }
            break;
        case 'G':
            String anuncio = "";
            String infos = packet.substring(2);

            // Validation du nom du this.playernnage
            boolean isValid = true;
            String name = infos.toLowerCase();
            // VeÂ¿Â½rifie d'abord si il contient des termes deÂ¿Â½finit

            // Si le nom passe le test, on veÂ¿Â½rifie que les caracteÂ¿Â½re entreÂ¿Â½ sont
            // correct.
            final String abcMin = "abcdefghijklmnopqrstuvwxyz-:/. '";
    		byte cantSimbol = 0, cantLetras = 0;
    		char letra_A = ' ', letra_B = ' ';
    		for (final char letra : name.toLowerCase().toCharArray()) {
        	    ...
        	    if(!isValid)
        	    {
        	      player.sendMessage("Veuillez ne pas mettre de caractères spéciaux.");
        	      return;
        	    }
            if (player.get_guild() != null) {
                anuncio = packet.substring(2);
                player.get_guild().SetAnnonces(anuncio);
                int GuildId = player.get_guild().getId();
                for (Player z : Main.world.getGuild(GuildId).getMembers()) {
                    if (z == null)
                        continue;
                    if (z.isOnline()) {
                        SocketManager.GAME_SEND_MESSAGE(z, "<b>[Annonce de guilde] : </b>" + anuncio, Config.getInstance().colorAnnounc);
                    }
                }
            }
            break;
  	  ...
  	  case 'I': // buy ornements
          ...
          break;
  	    case 'Y':
  	        World.get_Succes(this.getPlayer().getId()).Check(player, 19);
  	    	long getTemporisCoin = player.getTemporisCoin();
  	    	SocketManager.send(player, "wY" + getTemporisCoin + "#" + Config.getInstance().NORMAL_CHALLENGE + "#" + Config.getInstance().EXOTIC_CHALLENGE);
  	    	break;
		case 'r':
			if (player != null) {
				...
				StringBuilder logrosTemp = new StringBuilder();
				if (player.getLogros().size() > 0) {
					int pos = 1;
					for (LogroPersonaje logro : player.getLogros()) {
						if (logrosTemp.length() > 0) {
							logrosTemp.append(";");
						}
						String cantidad;//if logro is null it explote
						switch (logro.getLogro().getTipo()) {
						case 4:
							cantidad = "1";
							break;
						case 5:
						case 6:
							cantidad = logro.getLogro().getArgs();
							break;
						default:
							cantidad = logro.getLogro().getArgs().split(",")[1];
							break;
						}
					...
					SocketManager.ENVIAR_wr_PANEL_OBJETIVOS(player, logrosTemp.toString());
					//System.out.println(logrosTemp.toString());
		...
		case 'x':
			if (player != null) {
				final String[] infosS = packet.substring(2).split(Pattern.quote(";"));
				int Posicion = Integer.parseInt(infosS[0]) - 1;
				LogroPersonaje logro = player.getLogros().get(Posicion);
				String titulo = logro.getLogro().getTitulo();
				String Descripcion = logro.getLogro().getDescripcion();
				int reecompensa = logro.getLogro().getRecompensa();
				String cantidad = logro.getLogro().getCantidad();
				String ItemID = "0";
				String item = "";
				/*if (reecompensa == 1) {
					try {
						long expTotal = World.getExpPersonaje(player.getLevel());
						BigDecimal b = new BigDecimal("0.02");
						BigDecimal expNecesaria = new BigDecimal(expTotal);
						BigDecimal XpAdd = expNecesaria.multiply(b);		
						cantidad = (Math.round(XpAdd.longValue())) + "";
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
				if (reecompensa == 3) {
					cantidad = logro.getLogro().getCantidad().split(",")[1];
					ItemID = logro.getLogro().getCantidad().split(",")[0];
					item = World.getObjetoModelo(Integer.parseInt(ItemID)).getNombre();
				}*/
				SocketManager.ENVIAR_wy_PANEL_OBJETIVOS(player, titulo, Descripcion, reecompensa, cantidad, item,
						ItemID, Posicion, logro.getLogro().getTipo(), Main.LOGRO_REGALO_FIN);

			}
			break;
		case 'X':
			if (player != null) {
				if (!player.logroDiarioValidado) {
					if (player.getLogros().size() > 0) {
						...
						if (validar) {
							  int template= Main.LOGRO_REGALO_FIN;
							int cant = 50;
					        GameObject obj=Main.world.getObjTemplate(template).createNewItem(cant,false);
					        if(player.addObjet(obj,true))
					          World.addGameObject(obj,true);
					        SocketManager.GAME_SEND_Ow_PACKET(player);
							SocketManager.ENVIAR_Im_INFORMACION(player, "021;" + cant + "~" + template);
							player.logroDiarioValidado = true;
							player.setNObjDiarios(player.getNObjDiarios() + 1);
							//Sucess.launch(player, (byte) 13, null, 0);
							SocketManager.GAME_SEND_MESSAGE(player,
									"Félicitation! Vous avez terminé tout les objectifs quotidiens.");
						} else {
							SocketManager.GAME_SEND_MESSAGE(player,
									"Vous n'avez pas encore complété tout les objectifs quotidiens.");
						}
					}
				} else {
					SocketManager.GAME_SEND_MESSAGE(player, "Vous avez déjà  réclamé vos prix, revenez plus-tard !");
				}
		...
		case 'P':
			if (player != null) {
				String info = packet.substring(2);
				int Posicion = Integer.parseInt(info);
				LogroPersonaje logro = player.getLogros().get(Posicion);
				logro.confirmarMision(null, "", player);
      ...
      case 'Z':
       // parseCustomPacket(packet);
  	  ...
  	  case 'N':
  	   ...
  	   case 'S' : // stuf rapid id|name|id,pos
           switch(packet.charAt(2)) {
               case 'B' :
                   player.borrarSetRapido(Integer.parseInt(packet.substring(3)));
                   player.send("BN");
                   break;
               case 'C' :
                   ...
                   player.addSetRapido(id, nombre, icono, data);
                   player.send("BN");
                   break;
               case 'U' :
                   SetRapido set = player.getSetRapido(Integer.parseInt(packet.substring(3)));
                   if (set == null) {
                       return;
                   }
  ...
  private void ornements (String packet) {
	    ...
	    if(player.getOrnementsList().contains(ornement))
	        ...
	        if(player.getAccount().getPoints() < World.getOrnements().get(ornement).getPrice()) {
	            player.sendMessage("Vous n'avez pas assez de point boutiques pour acheter cet ornement !");
	        } else {
	            player.setOrnement(ornement);
	            ...
	            player.sendMessage("Vous venez d'acheter un nouvel ornement, il vous reste <b>"+player.getAccount().getPoints()+"</b> point boutique.");
	        }
      ...
      case 'L':
    	//  SocketManager.PACKET_ALL_CLASSE(this); 
    	 /* this.send("à âA");
    	  this.send("à âB");
    	  this.send("à âa0");
    	  this.send("à âe2.0,100,100");
    	  this.send("à ârR");
    	  this.send("à âD10");
    	  this.send("à âI");
    	  this.send("à âp20");
    	  this.send("à âVhttp://localhost/mp3/");
    	  this.send("bo1");*/
    		...
    		switch(packet.charAt(2))
    	  	  {
    	        		...
    	        		case 'c':
	        				...
	        				player.restatKeepParcho();
    	        		      ...
    	        		      player.sendMessage("(<b>Infos</b>) : Vous avez bien reset vos caractéristiques.");
    	        		      break;
    	        		case 'C':
    	        			if (player.getFight() != null) {
    	        				return;
    	        			 ...
    	        			 SocketManager.GAME_SEND_STATS_PACKET(player);
    	        		      ...
    	        		      player.sendMessage("(<b>Infos</b>) : Vous avez bien reset vos caractéristiques. Veuillez utiliser le restats simple afin de regagner vos stats prestige.");
    	        		      break;
    	  	  }
    		break;
  ...
  private String crearPacketKey() {
		if (Config.getInstance().PARAM_ENCRIPTAR_PACKETS) {
			currentKey = Formulas.getRandomInt(1, 15);
			...
			_aKeys[currentKey] = Encriptador.prepareKey(key);
			   ...
			   crypt = true;
			return Integer.toHexString(currentKey).toUpperCase() + key;
		}
		return "0";
	}
	private void parseShopPacket(String packet) {
		switch (packet.charAt(1)) {
		...
		case 'b':
			...
			case 'b': {
				try {
					...
					idObjetos.addAll(Database.getDynamics().getObjetosData().loadObjetosTienda());
					...
					for (TiendaObjetos tienda : idObjetos) {
						...
						if (idObjeto == tienda.getIdObjeto()) {
							if(tienda.getTipo()==18&&cantidad>1)
					        {
					          this.player.sendMessage("Merci de n'acheter qu'un seul animal de compagnie, à  la fois !");
					          return;
					        }
							if (cantidad >= 101) {
						          this.player.sendMessage("Limite de 100 !");
								return;
							}
							if (player.getPuntos() < precio) {
						          this.player.sendMessage("Vous n'avez pas les points nécessaires !");
								return;
							}
							if (cantidad == 0) {
								return;
					        ...
					        if(tienda.getPrice() == 0 || cantidad <= 0) {
							...
							player.send("?r" + player.getPuntos());
							 SocketManager.GAME_SEND_MESSAGE(player,"Vous venez d'acheter "+(cantidad>1 ? "l'items" : "l'item")+": <b>"+objT.getName()+"</b> pour <b>"+precio+"</b> points!","000000");
						        SocketManager.GAME_SEND_MESSAGE(this.player,"Il te reste : " + this.account.getPoints() + " points boutique !");
							    Database.getStatics().getPlayerData().shop_item(this.player.getName(), this.player.getAccID(), precio, objT.getId(), cantidad);
							    World.get_Succes(this.getPlayer().getId()).boutique_add(precio,player);
...
//				?midObjeto;Precio;Stats,Stats;???;itemIdBox
			...
			case 'o':
				...
				idCategoria2.addAll(Database.getDynamics().getCategoriaData().loadCategorias());
  ...
  private void addCharacter(String packet)
  {
    ...
    //Và Â¯àÂ¿àÂ½rifie d'abord si il contient des termes dà Â¯àÂ¿àÂ½finit
    ...
    //Si le nom passe le test, on và Â¯àÂ¿àÂ½rifie que les caractà Â¯àÂ¿àÂ½re entrà Â¯àÂ¿àÂ½ sont correct.
    if(isValid)
    {
      int tiretCount=0;
			...
			switch (packet.charAt(2)) {
				...
				case 'U' :
					...
					TimerWaiterPlus.addNext(() -> {
					 ...
					 },time, TimerWaiterPlus.DataType.CLIENT);
					//objeto_Equipar_Set(set);
					//if (cambio >= 1) {
	...
	private int objeto_Desequipar_Set() {
		try {
			...
			byte[] orden = {Constant.ITEM_POS_DOFUS1, Constant.ITEM_POS_DOFUS2, Constant.ITEM_POS_DOFUS3,
					Constant.ITEM_POS_DOFUS4, Constant.ITEM_POS_DOFUS5, Constant.ITEM_POS_DOFUS6, Constant.ITEM_POS_DOFUS7, Constant.ITEM_POS_DOFUS8, Constant.ITEM_POS_DOFUS9, Constant.ITEM_POS_DOFUS10, Constant.ITEM_POS_DOFUS11,
			...
			, Constant.OBJETO_POS_GEMA_1, Constant.OBJETO_POS_GEMA_2, Constant.OBJETO_POS_GEMA_3, Constant.ITEM_POS_RELIQUE,Constant.ITEM_POS_RELIQUE2,Constant.ITEM_POS_RELIQUE3, Constant.ITEM_POS_ARTEFACT
			, Constant.OBJETO_POS_GEMA_6, Constant.OBJETO_POS_GEMA_7};
			int time=0;
			for (byte i : orden) {
	   ...
	   private int objeto_Equipar_Set(SetRapido set) {
	        ...
	        try {
	            ...
	            if (player.getFight() != null) // si en combat dàÂ©marràÂ©
	            {
	                if (player.getFight().getState() != Constant.FIGHT_STATE_PLACE) {
	                    player.send("BN");
	            ...
	            byte[] orden = { Constant.ITEM_POS_DOFUS1, Constant.ITEM_POS_DOFUS2, Constant.ITEM_POS_DOFUS3,
						...
						Constant.ITEM_POS_ARME, Constant.OBJETO_POS_GEMA_1, Constant.OBJETO_POS_GEMA_2, Constant.OBJETO_POS_GEMA_3, Constant.ITEM_POS_RELIQUE,Constant.ITEM_POS_RELIQUE2,Constant.ITEM_POS_RELIQUE3, Constant.ITEM_POS_ARTEFACT
						, Constant.OBJETO_POS_GEMA_6, Constant.OBJETO_POS_GEMA_7 };
	            for (byte i : orden) {
	                ...
	                Object_move2(player, player.getGameClient(), 1, idObjeto, i,false);
	            }
	        } catch (final Exception e) {
	            player.send("BN");
	            e.printStackTrace();
	    ...
	    private synchronized void Object_move2(Player _perso,
	                                           GameClient _out, int quantity, int id, int position, boolean isRapidStuff)  {
	       // player.sendMessage("ObjectMove0");
	        try {

	            GameObject object = Main.world.getGameObject(id);
	            ...
	            if (this.player.getFight() != null)
	                if (this.player.getFight().getState() > Constant.FIGHT_STATE_ACTIVE)
	                    return;
	            if(this.player.get_isClone())
	                this.player.set_isClone(false);
	            /** Pet subscribe **/
	                ...
	                if (object.getTemplate().getType() == 41) {
	                    if (object.getQuantity() > 0) {
	                        ...
	                        if (object.getQuantity() - quantity > 0) {
	                            ...
	                            object.setQuantity(newQua, _perso);
	            ...
	            if (position == Constant.ITEM_POS_FAMILIER && object.getTemplate().getType() != Constant.ITEM_TYPE_FAMILIER && this.player.getObjetByPos(position) != null) {
	                ...
	                if (p.getType() == 2 || p.getType() == 3
	                    ...
	                    if (object.getQuantity() - 1 > 0) {//Si il en reste
	                        object.setQuantity(object.getQuantity() - 1, _perso);
	                        SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(this.player, object);
	                    } else {
	              ...
	              /*  if (object.hasSpellBoostStats()) {
	                    SocketManager.GAME_SEND_SB_PACKET(player,
	                            object.getBoostSpellStats(),
	                            (position != Constant.ITEM_POS_NO_EQUIPED) ? true : false,null);
	                }*/
	                ...
	                if (!object.getTemplate().getConditions().equalsIgnoreCase("") && !ConditionParser.validConditions(this.player, object.getTemplate().getConditions())) {
	                    SocketManager.GAME_SEND_Im_PACKET(this.player, "119|44"); // si le this.player ne vàÂ¯àÂ¿àÂ½rifie pas les conditions diverses
	                    return;
	                }
	                ...
	                //On ne peut àÂ¯àÂ¿àÂ½quiper 2 items de panoplies identiques, ou 2 Dofus identiques
	                ...
	                if (object.getTemplate().getType() == 113) {
	                    ...
	                    if (exObj.getObvijevanPos() != 0) {// si il y a dàÂ¯àÂ¿àÂ½jàÂ¯àÂ¿àÂ½ un obvi
	                        SocketManager.GAME_SEND_BN(this.player);
	                        return;
	                    }
	                    if(exObj.getTxtStat().get(Constant.STATS_MIMI) != null){
	                        this.player.sendMessage("Impossible de mettre un Obvi sur un item mimibioté.");
	                        return;
	                    }
	                    if (object.getTemplate().getId() == 9233 && exObj.getTemplate().getType() == Constant.ITEM_TYPE_SAC_DOS) {
	                        SocketManager.GAME_SEND_BN(this.player);
	                        return;
	                    }
	                    exObj.setObvijevanPos(object.getObvijevanPos()); // L'objet qui àÂ¯àÂ¿àÂ½tait en place a maintenant un obvi
	                    Database.getDynamics().getObvejivanData().add(object, exObj);
	                    this.player.removeItem(object.getGuid(), 1, false, false); // on enlàÂ¯àÂ¿àÂ½ve l'existance de l'obvi en lui-màÂ¯àÂ¿àÂ½me
	                    SocketManager.send(this.player, "OR" + object.getGuid()); // on le pràÂ¯àÂ¿àÂ½cise au client.
	                    ...
	                    SocketManager.GAME_SEND_ON_EQUIP_ITEM(this.player.getCurMap(), this.player); // Si l'obvi àÂ¯àÂ¿àÂ½tait cape ou coiffe : packet au client
	                    ...
	                    if (object.getQuantity() > 1) {
	                        ...
	                        if (object.getQuantity() - quantity > 0)//Si il en reste
	                        {
	                            ...
	                            Main.world.addGameObject(newItem, true);
	                            object.setQuantity(quantity, _perso);
	                            SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(this.player, object);
	                        }
	                    } else {
	                        Main.world.removeGameObject(object.getGuid());
	                    ...
	                    return; // on s'arràÂ¯àÂ¿àÂ½te làÂ¯àÂ¿àÂ½ pour l'obvi
	                } // FIN DU CODE OBVI

	                if (exObj != null)//S'il y avait dàÂ¯àÂ¿àÂ½ja un objet sur cette place on dàÂ¯àÂ¿àÂ½sàÂ¯àÂ¿àÂ½quipe
	                {
	                    GameObject obj2;
	                    if ((obj2 = this.player.getSimilarItem(exObj)) != null)//On le possàÂ¯àÂ¿àÂ½de deja
	                    {
	                        obj2.setQuantity(obj2.getQuantity()
	                                + exObj.getQuantity(), _perso);
	                    ...
	                    //On ne le possàÂ¯àÂ¿àÂ½de pas
	                    {
	                    ...
	                    if ((obj2 = this.player.getSimilarItem(object)) != null) {
	                        ...
	                        obj2.setQuantity(obj2.getQuantity() + quantity, _perso);
	                        SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(this.player, obj2);

	                        if (object.getQuantity() - quantity > 0)//Si il en reste
	                        {
	                            object.setQuantity(object.getQuantity() - quantity, _perso);
	                            SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(this.player, object);
	                        } else
	                        //Sinon on supprime
	                        {
	                ...
	                if (position == Constant.ITEM_POS_ARME) {
	                    ...
	                    //Incarnation
	                    {
	                        case 9544: // Tourmenteur tàÆàÂ©nebres
	                ...
	                } else {// Tourmenteur ; on dàÂ¯àÂ¿àÂ½morphe
	                    if (Constant.isIncarnationWeapon(object.getTemplate().getId()))
	                        this.player.unsetFullMorph();
	                }
	                String stats = object.parseStatsString();
	                int idSpell = Integer.parseInt(stats.split("#")[3], 16);
	                ...
	                if (object.getTemplate().getId() == 2157) {
	                    if (position == Constant.ITEM_POS_COIFFE) {
	                        ...
	                        SocketManager.GAME_SEND_MESSAGE(this.player, "Vous avez àÆàÂ©tàÆàÂ© transformàÆàÂ© en mercenaire.");
	                    } else if (position == Constant.ITEM_POS_NO_EQUIPED) {
	                        this.player.setGfxId(this.player.getClasse() * 10 + this.player.getSexe());
	                        SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this.player.getCurMap(), this.player.getId());
	                        SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(this.player.getCurMap(), this.player);
	                        SocketManager.GAME_SEND_MESSAGE(this.player, "Vous n'àÆàÂªtes plus mercenaire.");
	                    }
	                }
	                ...
	                if (position == Constant.ITEM_POS_FAMILIER && this.player.isOnMount())
	                    this.player.toogleOnMount();
	                //Verif pour les thisils de màÂ¯àÂ¿àÂ½tier
	                ...
	                if (objTemplate.getType() == 123 || objTemplate.getType() == 122) {
    ...
    private String objeto_String_Set_Equipado() {
		...
		Constant.ITEM_POS_AMULETTE, Constant.ITEM_POS_COIFFE, Constant.ITEM_POS_CAPE, Constant.ITEM_POS_ARME};
        for (byte i : orden) {
            GameObject obj = player.getObjetByPos(i);
            if (obj == null) {
                continue;
            }
  ...
  private void getGifts(String packet) {
      ...
      if (!gifts.isEmpty()) {
          ...
          for (String object : gifts.split(";")) {
              ...
              if (data.isEmpty()) data = "1~" + Integer.toString(id, 16) + "~" + Integer.toString(qua, 16) + "~~" + Main.world.getObjTemplate(id).getStrTemplate();
              else data += ";1~" + Integer.toString(id, 16) + "~" + Integer.toString(qua, 16) + "~~" + Main.world.getObjTemplate(id).getStrTemplate();
              if (item == -1) item = id;
          }
              ...
              if (qua == 1) {
                  ...
                  if (player.addObjet(obj, true))
                	  Main.world.addGameObject(obj, true);
                  		obj.addTxtStat(Constant.STATS_LIE_COMPTE, "");
        ...
        if (this.account == null) {
            if (this.chek <= 10) {
                ...
                TimerWaiterPlus.addNext(() -> {
                    ...
                    sendTicket(packet);
                }, 1000, TimerWaiterPlus.DataType.CLIENT);
            } else {
...
*/
        ...
        this.account.setSubscriber(Database.getStatics().getAccountData().load_subscribe(this.account.getId()));
         this.account.setTime_dj(Database.getStatics().getAccountData().load_dj(this.account.getId()));
         this.account.setTime_pxp(Database.getStatics().getAccountData().load_pxp(this.account.getId())); // Potion XP x2
         this.account.setTime_pdrop(Database.getStatics().getAccountData().load_pdrop(this.account.getId())); // Potion XP x2
  ...
  private void authorisedCommand(String packet) 
  {
   ...
   this.sendDiscord(Constant.WEBHOOK_STAFF, "Staff: " + this.player.getName() + " a utilisé la commande "+ packet.substring(2) + " Map : " + this.player.getCurMap().getId() + " Date : " + format.format(calendar.getTime())); 
  ...
  private void tchat(String packet)
  {
    ...
    switch(packet.charAt(2))
    {
      case '*'://Defaut
        ...
        if(Config.singleton.serverId == 2 || Config.singleton.serverId == 1 || Config.singleton.serverId == 3
        		|| Config.singleton.serverId == 4
        		|| Config.singleton.serverId == 5)
        {
        	 if(CommandePlayerAnemys.analyse(this.player,msg))
                 return;	
        }
        ...
        if(this.player.getGroupe()!=null && this.player.getFight() == null && this.player.getGroupe().getId() != 10) {
            SocketManager.GAME_SEND_cMK_PACKET_TO_MAP(this.player.getCurMap(), "", this.player.getId(), this.player.getName(),"<font color='#B41C1C'>"+ msg +" </font>");
return;
        }
      ...
      case '$'://Canal groupe
        ...
        if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF)!=null)
          if(this.player.getObjetByPos(Constant.ITEM_POS_ROLEPLAY_BUFF).getTemplate().getId()==10844)
            msg=Formulas.translateMsg(msg);
        if(this.player.getGroupe()!=null && this.player.getGroupe().getId() != 10) {

            SocketManager.GAME_SEND_cMK_PACKET_TO_GROUP(this.player.getParty(), "$", this.player.getId(), this.player.getName(),"<font color='#B41C1C'>"+ msg +" </font>");
			return;
		}
      ...
      case ':'://Canal commerce
        ...
        else
        {
          if(this.player.getGroupe()==null)
          {
            ...
            {
              l=(50000-l)/1000;//On calcul la diffà Â¯àÂ¿àÂ½rence en secondes
          ...
          if(this.player.getGroupe()!=null && this.player.getGroupe().getId() != 10) {
              ...
              SocketManager.GAME_SEND_cMK_PACKET_TO_ALL(this.player, ":", this.player.getId(), this.player.getName(),"<font color='#B41C1C'>"+ msg +" </font>");
  			return;
  		}
      ...
      case '?'://Canal recrutement
        ...
        else
        {
          if(this.player.getGroupe()==null)
          {
            ...
            {
              j=(40000-j)/1000;//On calcul la diffà Â¯àÂ¿àÂ½rence en secondes
      ...
      case '%'://Canal guilde
        ...
        if(this.player.getGroupe()!=null && this.player.getGroupe().getId() != 10) {
            ...
            SocketManager.GAME_SEND_cMK_PACKET_TO_GUILD(this.player.get_guild(), "%", this.player.getId(), this.player.getName(),"<font color='#B41C1C'>"+ msg +" </font>");
			return;
		}
      ...
      case '!'://Alignement
        ...
        {
          k=(30000-k)/1000;//On calcul la diffà Â¯àÂ¿àÂ½rence en secondes
      ...
      default:
        ...
        if(this.player.getGroupe()==null&&target.isInvisible()) // Alors pas connectà Â¯àÂ¿àÂ½
        {
          SocketManager.GAME_SEND_CHAT_ERROR_PACKET(this,nom);
          return;
        }
  ...
  // Tà Â¯àÂ¿àÂ½là Â¯àÂ¿àÂ½portation de MJ
  private void goToMap(String packet)
  {
    if(this.player.getGroupe()==null)
      return;
    if (this.player.getGroupe().getId() <= 1) 
    	return;
  ...
  private void prismFight(String packet)
  {
    switch(packet.charAt(2))
    {
      case 'J':
        ...
        },2000, TimerWaiterPlus.DataType.CLIENT);
  ...
  private void objeto_Desasociar_Mimobionte(final String packet) {
      try {
          final GameObject objeto = player.getObjet((Integer.parseInt(packet.substring(2))));
          if (objeto == null) {
              SocketManager.GAME_SEND_BN(player);
              return;
          }
			int regresar = Integer.parseInt(objeto.getParamStatTexto(Constant.STATS_MIMI_ID, 3));
          ObjectTemplate t = Main.world.getObjTemplate(regresar);
          GameObject obj = t.createNewItem(1, true);
          if (player.addObjet(obj, true)) {
        	  Main.world.addGameObject(obj, true);
          }
          SocketManager.GAME_SEND_Ow_PACKET(player);
          objeto.addStatTexto(Constant.STATS_MIMI_ID, "", false);
          objeto.addStatTexto(Constant.STATS_MIMI_NOM, "", false);
          SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, objeto);
          SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(player, objeto);
          this.player.refreshStats();
          SocketManager.GAME_SEND_ON_EQUIP_ITEM(this.player.getCurMap(), this.player);
          SocketManager.GAME_SEND_STATS_PACKET(this.player);
          Database.getDynamics().getObjectData().update(objeto);
      } catch (Exception e) {
      }
  }
  private void VerouilleObject(final String packet) {
      try {
          final GameObject objeto = player.getObjet((Integer.parseInt(packet.substring(2))));
          if (objeto == null) {
              SocketManager.GAME_SEND_BN(player);
          ...
          if (objeto.getLock() == 1) {
              objeto.addStatTexto(Constant.STATS_VERROUILLER, "", false);
              objeto.setLock(0);
              SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, objeto);
             // SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(player, objeto);
              this.player.refreshStats();
             ...
             // SocketManager.GAME_SEND_STATS_PACKET(this.player);
              Database.getDynamics().getObjectData().update(objeto);
              //SocketManager.GAME_SEND_MESSAGE(player,"Test "+(objeto.getGuid())+""+objeto.getLock()+"","FF0000");
              return;
          }
         
		
         // SocketManager.GAME_SEND_Ow_PACKET(player);
          //SocketManager.GAME_SEND_MESSAGE(player,"Test avant "+(objeto.getGuid())+""+objeto.getLock()+"","FF0000");
          objeto.addTxtStat(Constant.STATS_VERROUILLER,"");
  ...
  private void parseDialogPacket(String packet)
  {
    ...
    switch(packet.charAt(1))
    {
      ...
      case 'R'://Rà Â¯àÂ¿àÂ½ponse du joueur
        response(packet);
        if(party!=null&&this.player.getFight()==null&&party.getMaster()!=null&&party.getMaster().getName().equals(this.player.getName()))
        {
  ...
  private void create(String packet)
  {
    try
    {
      ...
      if(npc==null)
        return;
      SocketManager.GAME_SEND_DCK_PACKET(this,id);
     // this.player.confirmarLogrosNPC(npc.getTemplate().getId() + "");
      ...
      if (npc.getTemplate().getId() == EventoPortales.idPortal
				&& this.player.getCurMap().getId() == (short) 8271) {
			question = Main.world.getNPCQuestion(8001);
		}
  ...
  private void response(String packet)
  {
    ...
    try
    {
      ...
      if(answerId==6604||answerId==6605)
      {
        ...
        {
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_FECA_FREEDJ != false && player.getClasse() == Constant.CLASS_FECA) {
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_OSAMODAS_FREEDJ != false && player.getClasse() == Constant.CLASS_OSAMODAS) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_ENUTROF_FREEDJ != false && player.getClasse() == Constant.CLASS_ENUTROF) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_SRAM_FREEDJ != false && player.getClasse() == Constant.CLASS_SRAM) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_XELOR_FREEDJ != false && player.getClasse() == Constant.CLASS_XELOR) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_ECAFLIP_FREEDJ != false && player.getClasse() == Constant.CLASS_ECAFLIP) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_ENIRIPSA_FREEDJ != false && player.getClasse() == Constant.CLASS_ENIRIPSA) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_IOP_FREEDJ != false && player.getClasse() == Constant.CLASS_IOP) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_CRA_FREEDJ != false && player.getClasse() == Constant.CLASS_CRA) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_SADIDA_FREEDJ != false && player.getClasse() == Constant.CLASS_SADIDA) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_SACRIEUR_FREEDJ != false && player.getClasse() == Constant.CLASS_SACRIEUR) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
        	 ...
        	 if(player.getAccount().getTime_dj() == 0L && Config.singleton.CLASS_PANDAWA_FREEDJ != false && player.getClasse() == Constant.CLASS_PANDAWA) {
                 StringBuilder newStats=new StringBuilder();
                 for(String i : stats.split(","))
                   if(!i.equals(statsReplace))
                     newStats.append((newStats.toString().isEmpty() ? i : ","+i));
  ...
  private  void parseExchangePacket(String packet)
  {
    ...
    switch(packet.charAt(1))
    {
      case 'A'://Accepter demande d'à Â¯àÂ¿àÂ½change
        accept();
        break;
      case 'B'://Achat
        buy(packet);
        break;
      case 'H'://Demande prix moyen + catà Â¯àÂ¿àÂ½gorie
        bigStore(packet);
        break;
      case 'K'://Ok
        ready();
        break;
      case 'L'://jobAction : Refaire le craft prà Â¯àÂ¿àÂ½cedent
        replayCraft();
        break;
      case 'M'://Move (Ajthiser//retirer un objet a l'à Â¯àÂ¿àÂ½change)
    	   /* if(this.player.getGroupe() != null)
      ...
      case 'q'://Mode marchand (demande de la taxe)
        askOfflineExchange();
        break;
      case 'Q'://Mode marchand (Si valider aprà Â¯àÂ¿àÂ½s la taxe)
        offlineExchange();
        break;
      ...
      case 'W'://Metier public
        ...
        break;
      case 'V'://Fin de l'à Â¯àÂ¿àÂ½change
  ...
  private void accept()
  {
    ...
    switch(type)
    {
      case 1: // Echange PlayerVsPlayer
        ...
        break;
      case 12: // Craft sà ÆàÂ©ucirsà ÆàÂ©
      case 13:
  ...
  private void buy(String packet)
  {
    ...
    if(player.boutique)
    {
      ...
      try
      {
        ...
        if(Config.singleton.serverId == 6) {
        	...
        	if(Config.singleton.serverId == 222 || Config.singleton.serverId == 613 || Config.singleton.serverId == 22)
            if(!Config.getInstance().boutique.contains(","+Integer.parseInt(infos[0])+",")) {
            	SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
            	return;
            }
        ...
        newObj.getTxtStat().put(Constant.STATS_OBJ_BOUTIQUE,Integer.toHexString(1));
        ...
        SocketManager.GAME_SEND_STATS_PACKET(player);
        SocketManager.GAME_SEND_Ow_PACKET(player);
        SocketManager.GAME_SEND_MESSAGE(player,"Vous venez d'acheter "+(quantity>1 ? "l'items" : "l'item")+": <b>"+template.getName()+"</b> pour <b>"+value+"</b> points!","000000");
        SocketManager.GAME_SEND_MESSAGE(this.player,"Il te reste : " + this.account.getPoints() + " points boutique !");
    Database.getStatics().getPlayerData().shop_item(this.player.getName(), this.player.getAccID(), value, template.getId(), quantity);
    ...
    else
    {
      ...
      try
      {
        ...
        if (npcTemplate.getId() ==  32245) { // DUO SORT SPECIAL
 			final int value = template.getPrice() * qua;
 			if (!player.hasItemTemplate(53078, value)) {
 				SocketManager.GAME_SEND_MESSAGE(this.player,
 						"Vous n'avez pas de Fiole Prestigieuse pour acheter cet article.");
 				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
 				return;
 			}
 			player.removeByTemplateID(53078, value);
 			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
 	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
 	          {
 	            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
        ...
        if (npcTemplate.getId() ==  32215) { // ST VALENTIN
			final int value = template.getPrice() * qua;
			if (!player.hasItemTemplate(49550, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"Vous n'avez pas assez de Coeurs Perdus pour acheter cet article.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
			player.removeByTemplateID(49550, value);
			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
	            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
       ...
       /* if (npcTemplate.getId() ==  30026) {
        	if (Config.getInstance().beta) {
			final int value = template.getPrice() * qua;
			player.setKamas(player.getKamas()-value);
			GameObject object=template.createNewItem(qua,true);
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
	            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
	            object.clearStats();
        ...
        if (npcTemplate.getId() ==  30075) {
			final int value = template.getQuete() * qua;
			if (!player.hasItemTemplate(15000, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"Vous n'avez pas assez de Jetons de quête pour acheter cet article.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
			player.removeByTemplateID(15000, value);
			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
			//object.getTxtStat().put(Constant.STATS_OBJ_JETON,Integer.toHexString(1));
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
	            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
	            object.clearStats();
        ...
        if (npcTemplate.getId() ==  31001) {
			final int value = template.getRambo() * qua;
			if (!player.hasItemTemplate(8575, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"Vous n'avez pas de Ramboton pour acheter cet article.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
			player.removeByTemplateID(8575, value);
			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
			//object.getTxtStat().put(Constant.STATS_OBJ_ECHANGEUR,Integer.toHexString(1));
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
	            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
	            object.clearStats();
        ...
        if (npcTemplate.getId() ==  32262) {
			final int value = template.getKrom() * qua;
			if (!player.hasItemTemplate(19502, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"Vous n'avez pas de <b>Jeton Kromatique</b> pour acheter cet article.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
			player.removeByTemplateID(19502, value);
			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
			//object.getTxtStat().put(Constant.STATS_OBJ_ECHANGEUR,Integer.toHexString(1));
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
        ...
        if (npcTemplate.getId() ==  402) {
			final int value = template.getFoire() * qua;
			if (!player.hasItemTemplate(1749, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"Vous n'avez pas de <b>Jeton de la foire du Trool</b> pour acheter cet article.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
			player.removeByTemplateID(1749, value);
			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
			//object.getTxtStat().put(Constant.STATS_OBJ_ECHANGEUR,Integer.toHexString(1));
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
        ...
        if (npcTemplate.getId() ==  30024) {
			final int value = template.getPrice() * qua;
			if (!player.hasItemTemplate(10313, value)) {
				SocketManager.GAME_SEND_MESSAGE(this.player,
						"Vous n'avez pas assez de doplons pour acheter cet article.");
				SocketManager.GAME_SEND_BUY_ERROR_PACKET(this);
				return;
			}
			player.removeByTemplateID(10313, value);
			GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
			//object.getTxtStat().put(Constant.STATS_OBJ_DOPLON,Integer.toHexString(1));
	          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
	          {
	            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
	            object.clearStats();
        ...
        if(template.getPoints()>0&&(npcTemplate.getInformations()&0x4)==4)
        {
          ...
          GameObject object=template.createNewItem(qua,(npcTemplate.getInformations()&0x1)==1);
			object.getTxtStat().put(Constant.STATS_OBJ_BOUTIQUE,Integer.toHexString(1));
          if(template.getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
          {
            Mount mount=new Mount(Constant.getMountColorByParchoTemplate(object.getTemplate().getId()),this.getPlayer().getId(),false);
            object.clearStats();
        ...
        Database.getStatics().getPlayerData().shop_item(this.player.getName(), this.player.getAccID(), value, template.getId(),qua);
        if(player.getKamas() < 5000)
			SocketManager.GAME_SEND_STATS_BOUTIQUE_PACKET(player);
    ...
    switch(packet.charAt(2))
    {
      ...
      //  if(player.getWorldMarket())
       ...
       // else
        ...
        if(hE==null)
        {
          // Intervient lorsque un client achà Â¯àÂ¿àÂ½te plusieurs fois la mà Â¯àÂ¿àÂ½me ressource.
          // Par exemple une pyrute à Â¯àÂ¿àÂ½ 45'000k trois fois. Au bout d'un moment elle monte à Â¯àÂ¿àÂ½ 100'000k, mais le client
          // voit toujours 45'000k. Il doit il y avoir un manque de paquet envoyà Â¯àÂ¿àÂ½. La 4à Â¯àÂ¿àÂ½me avait buggà Â¯àÂ¿àÂ½.
        ...
        if(curHdv.buyItem(ligneID,amount,Integer.parseInt(info[2]),this.player))
        {
          ...
          if(curHdv.getLine(ligneID)!=null&&!curHdv.getLine(ligneID).isEmpty())
            SocketManager.GAME_SEND_EHm_PACKET(this.player,"+",curHdv.getLine(ligneID).parseToEHm());//Rà Â¯àÂ¿àÂ½ajthise la ligne si elle n'est pas vide
          this.player.refreshStats();
          SocketManager.GAME_SEND_Ow_PACKET(this.player);
          SocketManager.GAME_SEND_Im_PACKET(this.player,"068");//Envoie le message "Lot achetà Â¯àÂ¿àÂ½"
        }
        else
        {
      ...
      case 'P'://Demande des prix moyen
        ...
        break;
      case 'T'://Demande des template de la catà Â¯àÂ¿àÂ½gorie
        ...
        for(int k=couple.getRight();k>0;k--)
        {
          ...
          for(Map.Entry<Integer, Integer> entry1 : object.getStats().getMap().entrySet())
          {
            ...
            for(Rune rune : Rune.runes)
            {
              ...
              if(entry1.getKey()==characteristic && rune.getTemplateId() != 78988)
              {
                if(rune.getTemplateId()==1557||rune.getTemplateId()==1558||rune.getTemplateId()==7438)
                {
                  double puissance=1.3*(Math.pow(object.getTemplate().getLevel(),3.7)/Math.pow(rune.getPower(),(5.0/4.0)))+((jet-1)/rune.getPower())*(66.66-1.3*(Math.pow(object.getTemplate().getLevel(),2.0)/Math.pow(rune.getPower(),(55.0/4.0))));
            ...
            if(packet.charAt(3)=='+')
            {
              ...
              try
              {
                ...
                if(obj.getTemplate().getType() == 24)
             	   return;
                if(obj.getTemplate().getType() == 172)
              	   return;
                if(obj.getTemplate().getId() == 22005)
              	   return;
               /* if(obj.getTemplate().getId() == 53347)
               	   return;*/
                if(obj.getTemplate().getId() == 15000)
                	   return;
                if(obj.getTxtStat().containsKey(Constant.STATS_LIE_COMPTE))
              	   return;
                if(obj.getTxtStat().containsKey(Constant.STATS_VERROUILLER) || obj.getLock() == 1) {
              	  this.player.sendMessage("(<b>Infos</b>) : Veuillez déverouiller votre item avant.");   
              	   return;
              }
                if(obj.getTxtStat().containsKey(Constant.STATS_MIMI) || obj.getTxtStat().containsKey(Constant.STATS_MIMI_NOM) || obj.getTxtStat().containsKey(Constant.STATS_MIMI_ID)) {
                	  this.player.sendMessage("(<b>Infos</b>) : Veuillez retirer l'object d'apparence avant.");   
                	   return;
                }
                if(obj.getTxtStat().containsKey(Constant.STATS_OBJ_LIEPERSO))
             	   return;
          ...
          if(packet.charAt(2)=='O')
          {
            if(packet.charAt(3)=='+')
            {
              ...
              try
              {
                ...
                if(object.getTemplate().getType() == 24 || object.getTemplate().getType() == 78 || object.getTemplate().getType() == 87 ||
              		  object.getTemplate().getType() == 12 || object.getTemplate().getType() == 89 || object.getTemplate().getType() == 18
              		  || object.getTemplate().getType() == 69 || object.getTemplate().getType() == 76 || object.getTemplate().getType() == 42 || object.getTemplate().getType() == 23
              		  || object.getTemplate().getType() == 94 || object.getTemplate().getType() == 93 || object.getTemplate().getType() == 75
              		  || object.getTemplate().getType() == 47 || object.getTemplate().getType() == 15 || object.getTemplate().getType() == 51
              		  || object.getTemplate().getType() == 107 || object.getTemplate().getType() == 36 || object.getTemplate().getType() == 106
              		  || object.getTemplate().getType() == 111 || object.getTemplate().getType() == 104 || object.getTemplate().getType() == 50
              		  || object.getTemplate().getType() == 48 || object.getTemplate().getType() == 58 || object.getTemplate().getType() == 65
              		  || object.getTemplate().getType() == 15 || object.getTemplate().getType() == 103 || object.getTemplate().getType() == 54
              		  || object.getTemplate().getType() == 55)
             	   return;
                if(object.getTxtStat().containsKey(Constant.STATS_VERROUILLER) || object.getLock() == 1) {
              	  player.sendMessage("Vous avez tenté de briser un item vérouillé, cet item n'a pas été brisé.");   
                return;
                }
                if(object.getTxtStat().containsKey(Constant.STATS_MIMI) || object.getTxtStat().containsKey(Constant.STATS_MIMI_NOM) || object.getTxtStat().containsKey(Constant.STATS_MIMI_ID)) {
              	  this.player.sendMessage("(<b>Infos</b>) : Veuillez retirer l'object d'apparence avant.");   
              	   return;
              }

               /* if (object.getTemplate().getType() != 1 ||object.getTemplate().getType() != 6 || object.getTemplate().getType() != 2 ||object.getTemplate().getType() != 5 || object.getTemplate().getType() != 9 || object.getTemplate().getType() != 11 || object.getTemplate().getType() != 3 ||
              		  object.getTemplate().getType() != 4 || object.getTemplate().getType() != 7 || object.getTemplate().getType() != 8 ||object.getTemplate().getType() != 10 ||
              		  object.getTemplate().getType() != 19 || object.getTemplate().getType() != 17 || object.getTemplate().getType() != 16
              		  )
              	  return;*/
                if(qua<1)
                  return;
        ...
        for(String s : packet.substring(4).split("\\-"))
         ...
         String[] infos=s.split("\\|");
          ...
          case '+'://Mettre un objet en vente
            ...
            {
              SocketManager.GAME_SEND_MESSAGE(this.player,"Vous avez déjà Â  atteint le nombre maximum d'articles que vous pouvez vendre à Â  la fois.");
              return;
            }
            ...
            {
              Main.world.logger.error("Error Echange HDV '"+packet+"' => "+e.getMessage());
              // Arrive quand price n'est pas dans le pacquet. C'est que le joueur ne veut pas mettre dans un hdv, mais dans autre chose ... Un paquet qui est MO+itmID|qtà Â¯àÂ¿àÂ½
              // Peeut-à Â¯àÂ¿àÂ½tre aprà Â¯àÂ¿àÂ½sa voir utilisà Â¯àÂ¿àÂ½ le concasseur ...
              e.printStackTrace();
              SocketManager.GAME_SEND_MESSAGE(this.player,"Une erreur s'est produite lors de la vente de votre article. Veuillez vous reconnecter pour résoudre le problà Â¨me.");
              return;
            }
            if(amount > 3 )
                return;
           ...
           //   curHdv=Main.world.getHdv(Math.abs((Integer)this.player.getExchangeAction().getValue()));
            ...
            if(!this.player.hasItemGuid(itmID))//Và Â¯àÂ¿àÂ½rifie si le this.playernnage a bien l'item spà Â¯àÂ¿àÂ½cifià Â¯àÂ¿àÂ½ et l'argent pour payer la taxe
              return;
            ...
            GameObject obj=World.getGameObject(itmID);//Rà Â¯àÂ¿àÂ½cupà Â¯àÂ¿àÂ½re l'item
            ...
            if(qua>obj.getQuantity())//S'il veut mettre plus de cette objet en vente que ce qu'il possà Â¯àÂ¿àÂ½de
              return;
            int rAmount=(int)(Math.pow(10,amount)/10);
            int newQua=(obj.getQuantity()-rAmount);

            if(newQua<=0)//Si c'est plusieurs objets ensemble enleve seulement la quantità Â¯àÂ¿àÂ½ de mise en vente
            {
              this.player.removeItem(itmID);//Enlà Â¯àÂ¿àÂ½ve l'item de l'inventaire du this.playernnage
            ...
            if (Config.getInstance().serverId == 1 ) 
              ...
              if (Config.getInstance().serverId == 5) 
               ...
               if (Config.getInstance().serverId == 4) 
                 	 arg = "Duo-Compte";
               toAdd.sendDiscord(Constant.WEBHOOK_HDVVENTE, "**" + toAdd.getGameObject().getTemplate().getName() + "** a été mis en vente x"+ toAdd.getGameObject().getQuantity() + " à  l'HDV général au prix de **" + toAdd.getPrice() + " Kamas**."); 
      ...
      case ExchangeAction.CRAFTING:
            	 ...
            	 if(exchangeAction.getType()==ExchangeAction.CRAFTING&&value instanceof JobAction)
            	    {
            	      if(((JobAction)value).isCraft())
            	      {
            	    	  if (((JobAction)value).getId() == 119 || ((JobAction)value).getId() == 1 || ((JobAction)value).getId() == 113 || ((JobAction)value).getId() == 115 || ((JobAction)value).getId() == 116 || ((JobAction)value).getId() == 117 || ((JobAction)value).getId() == 118 || ((JobAction)value).getId() == 120){
            	    		  player.sendMessage("Les mêtiers de FM Corp à corp ont été désactivés de cette option, veuillez utilsier le 1 par 1.");
            	          	GameClient.leaveExchange(this.player);
            	    		  return;
            	    	  }
            	    	  if ((((JobAction)value).getId() >= 163 && ((JobAction)value).getId() <= 169)) {
          ...
          for(String part : packet.split(";"))
          {
            try
            {
              ...
              if(c=='+')
              {
                ...
                if (this.player.getCurMap().getId() != 437){
                    if (obj.getTemplate().getType() != 78 && obj.getTemplate().getType() != 26 && obj.getTemplate().getType() != 66 && obj.getTemplate().getType() != 15 && obj.getTemplate().getType() != 104 && obj.getTemplate().getType() != 105 && obj.getTemplate().getType() != 47 && obj.getTemplate().getType() != 35 && obj.getTemplate().getType() != 53 && obj.getTemplate().getType() != 111 && obj.getTemplate().getId() != 7508)
                        if (!JobAction.isAvailableObject(((JobAction) this.player.getExchangeAction().getValue()).getJobStat().getTemplate().getId(), obj.getTemplate().getType())) {
          			      SocketManager.PACKET_POPUP_DEPART(this.getPlayer(), "Faille détectée, vous avez été banni pour cheat.");
        			      TimerWaiterPlus.addNext(() -> {
        	  	        	if(this != null)
        	  	        		this.kick(); 
        	  	        	this.getAccount().setBanned(true);
        	  		    	this.sendDiscord(Constant.WEBHOOK_WPE, "Le joueur "+this.getPlayer().getName()+" a utilisé WPE et a été ban, merci de ne pas le débannir. @everyone");
        	 			 }, 5, TimeUnit.SECONDS, TimerWaiterPlus.DataType.CLIENT);	
        			      return;
                        }
                if (obj.getTemplate().getType() != 78 && obj.getTemplate().getType() != 66 && obj.getTemplate().getType() != 15 && obj.getTemplate().getType() != 104 && obj.getTemplate().getType() != 105 && obj.getTemplate().getType() != 47 && obj.getTemplate().getType() != 35 && obj.getTemplate().getType() != 53 && obj.getTemplate().getType() != 26 && obj.getTemplate().getType() != 111 && obj.getTemplate().getId() != 7508)
                    if (JobAction.GETFMJobs(((JobAction) this.player.getExchangeAction().getValue()).getJobStat().getTemplate().getId()))
                        if (((JobAction) this.player.getExchangeAction().getValue()).getJobStat().get_lvl() < (int) Math.floor(obj.getTemplate().getLevel() / 2) &&  ((JobAction) this.player.getExchangeAction().getValue()).getJobStat().get_lvl() != 100) {
                        	 SocketManager.PACKET_POPUP_DEPART(this.getPlayer(), "Veuillez vérifier le niveaux de votre mêtier.");
           			      TimerWaiterPlus.addNext(() -> {
           	  	        	if(this != null)
           	  	        		this.kick(); 
           	  	        	//this.getAccount().setBanned(true);
           	  		    	//this.sendDiscord(Constant.WEBHOOK_WPE, "Le joueur "+this.getPlayer().getName()+" a utilisé un bug d'items levels + mêtiers.");
           	 			 }, 1, TimeUnit.SECONDS, TimerWaiterPlus.DataType.CLIENT);	
           			      return;
                        }
            }
        ...
        if(close) {
        	GameClient.leaveExchange(this.player);
	    		...
	    		SocketManager.PACKET_POPUP_DEPART(this.player, "Tout vos crafts ont été fabriqué");
          ...
          case 'O'://Objet
            ...
            if(packet.contains("-"))
                ...
                if(object!=null)
                {
                  ...
                  if(object.getTxtStat().containsKey(Constant.STATS_STAFF))
                  {
                    Player player=Main.world.getPlayerByName(object.getTxtStat().get(Constant.STATS_STAFF));
                    if(player!=null)
                    {
                      if(!player.getName().equals(this.player.getName()))
                        return;
                  ...
                  if(object.getTxtStat().containsKey(Constant.STATS_OBJ_LIEPERSO))
                  {
                    Player player=Main.world.getPlayerByName(object.getTxtStat().get(Constant.STATS_OBJ_LIEPERSO));
                    if(player!=null)
                    {
                      if(!player.getName().equals(this.player.getName()))
                        return;
          ...
          case 'O'://Objet
            ...
            switch(packet.charAt(3))
            {
              case '+'://Ajthiser a la banque
                  GameObject object = soufix.game.World.getGameObject(guid);
                  	if (object.getTemplate().getId() == 15000) 
                  		return;
                    if(object==null)
                        return;
                    /* if(object.getTemplate().getId() == 53347)
                	   return;*/
                      if(object.getTemplate().getId() == 15000)
                   	   return;
                      if(object.getTxtStat().containsKey(Constant.STATS_LIE_COMPTE))
                    	   return;
                     if(object.getTxtStat().containsKey(Constant.STATS_OBJ_LIEPERSO))
                   	   return;
                     if(object.getTxtStat().containsKey(Constant.STATS_VERROUILLER) || object.getLock() == 1) {
        ...
        switch(packet.charAt(2))
        {
         ...
         /* case 'S'://Kamas
              try {
                  ...
                  if (numb < 0)
                      return;
                  if (this.player.getAccount().getPoints() < numb)
                      numb = this.player.getAccount().getPoints();
                  if (numb < 0)
                      return;
                  ((PlayerExchange) this.player.getExchangeAction().getValue()).setOgrinas(this.player.getId(), numb);
              } catch (NumberFormatException e) {
                  Main.world.logger.error("Error Echange PvP '" + packet + "' => " + e.getMessage());
                  e.printStackTrace();
                  return;
  ...
  private void askOfflineExchange()
  {
    ...
    if(this.player.getCurMap().getId()==10111||this.player.getCurMap().getId()==38||this.player.getCurMap().getId()==4601||this.player.getCurMap().getId()==4259||this.player.getCurMap().getId()==8036||this.player.getCurMap().getId()==10301)
    {
	...
	if(Config.singleton.serverId == 2 && this.player.getCurMap().getId()!=10111) {
		SocketManager.GAME_SEND_Im_PACKET(this.player,"125;0");
		 return;
	}
	if(Config.singleton.serverId == 1 && this.player.getCurMap().getId()!=10111) {
		SocketManager.GAME_SEND_Im_PACKET(this.player,"125;0");
		 return;
	}
	if(Config.singleton.serverId == 4 && this.player.getCurMap().getId()!=10111) {
		SocketManager.GAME_SEND_Im_PACKET(this.player,"125;0");
		 return;
	}
	if(Config.singleton.serverId == 8 && this.player.getCurMap().getId()!=10111) {
		SocketManager.GAME_SEND_Im_PACKET(this.player,"125;0");
		 return;
	}
	if(Config.singleton.serverId == 7 && this.player.getCurMap().getId()!=10111) {
		SocketManager.GAME_SEND_Im_PACKET(this.player,"125;0");
		 return;
	}
  ...
  private void offlineExchange()
  {
	...
	//player.sendDiscord(Constant.WEBHOOK_MARCHAND, "Le joueur **" + player.getName() + "** vient de se mettre en mode marchand en "+"[" + "**" + player.getCurMap().getX() + "," + player.getCurMap().getY()+"**" + "]" + " ! ");
  ...
  private void request(String packet)
  {
    ...
    { // Craft sà Â¯àÂ¿àÂ½curisà Â¯àÂ¿àÂ½ : celui qui n'a pas le job ( this.player ) souhaite invità Â¯àÂ¿àÂ½ player
      try
      {
        ...
        if (player.getWorldMarket() == true) {
        	player.sendMessage("Action impossible.");
        	return;
        }
    ...
    { // Craft sà Â¯àÂ¿àÂ½curisà Â¯àÂ¿àÂ½ : celui qui à Â¯àÂ¿àÂ½ le job ( this.player ) souhaite invità Â¯àÂ¿àÂ½ player
      try
      {
        String[] split=packet.split("\\|");
        int id=Integer.parseInt(split[1]);
        ...
        player.getCurMap();
		if(GameMap.isInCraftMap(player.getCurMap().getId()))
        {
	    	this.player.sendMessage("Cette fonctionnalité a été désactivé à  cet endroit.");
    ...
    ///  else
     //   hdv=Main.world.getHdv(this.player.getCurMap().getId());
      ...
      if(hdv!=null)
      {
        ...
        ExchangeAction<Integer> exchangeAction=new ExchangeAction<>(ExchangeAction.AUCTION_HOUSE_BUYING,-this.player.getCurMap().getId()); //Rà Â¯àÂ¿àÂ½cupà Â¯àÂ¿àÂ½re l'ID de la map et rend cette valeur nà Â¯àÂ¿àÂ½gative
        this.player.setExchangeAction(exchangeAction);
      }
      return;
    }
    ...
    switch(packet.charAt(2))
    {
      case '0'://Si NPC
        ...
        if(npc!=null)
        {
            ...
            if(npc.getTemplate().getId()==32215) {
          	  SocketManager.PACKET_POPUP_DEPART(this.player, "Les achats s'effectuent avec des Coeurs Perdus !");  
            }
            if(npc.getTemplate().getId()==30026) {
          	  SocketManager.PACKET_POPUP_DEPART(this.player, "Tout les items sont JP !");  
            }
            if(npc.getTemplate().getId()==30075) {
          	  SocketManager.PACKET_POPUP_DEPART(this.player, "Les achats s'effectuent avec les Jetons de quêtes  !");  
            }
            if(npc.getTemplate().getId()==31001) {
            	  SocketManager.PACKET_POPUP_DEPART(this.player, "Les achats s'effectuent avec le Ramboton  !");  
              }
            if(npc.getTemplate().getId()==30024) {
            	  SocketManager.PACKET_POPUP_DEPART(this.player, "Les achats s'effectuent avec des Doplons !");  
              }
            if (player.getWorldMarket() == true) {
            	player.sendMessage("Action impossible.");
            	return;
      ...
      case '1'://Si joueur
        try
        {
          ...
          if (target.getWorldMarket() == true) {
          	target.sendMessage("Action impossible.");
          	return;
          }
      ...
      case '4'://StorePlayer
        ...
        if(seller==null||!seller.isShowSeller()||seller.getCurMap()!=this.player.getCurMap())
          return;
        if (seller.getWorldMarket() == true) {
        	seller.sendMessage("Action impossible.");
        	return;
        }
      ...
      case '8'://Si Collector
        ...
        break;
      case '9'://Dà Â¯àÂ¿àÂ½posà Â¯àÂ¿àÂ½/Retirà Â¯àÂ¿àÂ½ un familier
        id=Integer.parseInt(packet.substring(4));
        if (player.getWorldMarket() == true) {
        	player.sendMessage("Action impossible.");
        	return;
    ...
    try
    {
      ...
      for(String s : sellAllSplit)
      {
        ...
        if(!this.player.hasItemGuid(id))
        {
          ...
          SocketManager.GAME_SEND_MESSAGE(this.player,"[<b>Alerte</b>] : Vous ne pouvez pas vendre vos items à  celui-ci, allez voir Josh Het' Tou, il pourra sans doute vous aider.");
          return;
        }
        Npc npc=this.player.getCurMap().getNpc(id);
        if(npc.getTemplate().getId() != 30025) {
            SocketManager.GAME_SEND_MESSAGE(this.player,"[<b>Alerte</b>] : Vous ne pouvez pas vendre vos items à  celui-ci, allez voir Josh Het' Tou, il pourra sans doute vous aider.");
        	return;
        }
      ...
      case 'F':
        ...
        for(Player artissant : Main.world.getOnlinePlayers())
        {
         ...
         // if(artissant.getGroupe() != null)
          ...
          inJob=(map==8731||map==8732 || map == 18500) ? 1 : 0;
          }
  ...
  private void useEmote(String packet)
  {
    ...
    if(this.player==null)
      return;
    if(this.player.getFight()!=null)
      return;//Pas d'à Â¯àÂ¿àÂ½mote en combat
    ...
    switch(emote)
    {
      case 20://Tabouret hà Â¯àÂ¿àÂ½hà Â¯àÂ¿àÂ½ ( rime )
      case 19://s'allonger
      case 1:// s'asseoir
  ...
  private void joinWife(String packet)
  {
    ...
    switch(packet.charAt(2))
    {
      case 'S'://Teleportation
        ...
        {
          SocketManager.GAME_SEND_MESSAGE(this.player,"Une barrià Â¨re magique vous empà Âªche de rejoindre votre conjoint.");
          return;
        }
    ...
    try
    {
      ...
      case 'D'://Dà¯Â¿Â½tails d'un combat (liste des combats)
        ...
        case 'N'://Bloquer le combat
          if(this.player.getFight()==null)
            return;
          if (this.player.getCurMap().getId() == 16072 || this.player.getCurMap().getId() == 16073 || this.player.getCurMap().getId() == 16074) 
          { 
        	  this.player.sendMessage("(<b>Évent</b>) : Impossible sur cette map !");
        	  return;
      }
  ...
  private void parseGamePacket(String packet)
  {
    switch(packet.charAt(1))
    {
     ...
     /* case 'D':
      ...
      case 't':
          if (this.player.getFight() != null) {
              Fighter fighter = this.player.getFight().getFighterByPerso(this.player);
              if(fighter != null) {
                  if(this.player.cinvoc) {
                      if(this.player.getFight().getFighterByOrdreJeu() != null)
                          if(this.player.getFight().getFighterByOrdreJeu().getInvocator() != null)
                              if(this.player.getFight().getFighterByOrdreJeu().getInvocator() == fighter)
                                  fighter = this.player.getFight().getFighterByOrdreJeu();
                  }

                  fighter.getFight().playerPass(fighter);
              }
          }
        default:
        if(this.player == null)
    ...
    switch(actionID)
    {
      case 1://Deplacement
        ...
        if(party!=null&&this.player.getFight()==null&&party.getMaster()!=null&&party.getMaster().getName().equals(this.player.getName()))
        {
          ...
          }),500, TimerWaiterPlus.DataType.CLIENT);
        }
        }
        break;
      ...
      case 507://Panneau intà Â¯àÂ¿àÂ½rieur de la maison
        houseAction(packet);
        break;
  ...
  private void gameParseDeplacementPacket(GameAction GA)
  {
    ...
    if(this.player.getFight()==null)
    {
      ...
      if (player.getKamas() < 0) {
    	  SocketManager.PACKET_POPUP_DEPART(player, "Faille Kamas/Bug Kamas détecté, vous êtes actuellement en Négatif, veuillez faire un ticket.");
	      TimerWaiterPlus.addNext(() -> {
	        	if(this != null)
  	        		this.kick(); 
  	        	this.getAccount().setBanned(true);
		    	this.sendDiscord(Constant.WEBHOOK_USEBUG, "Le joueur "+player.getName()+" a des kamas en négatif (Quantité : "+player.getKamas()+").");
			 }, 5, TimeUnit.SECONDS, TimerWaiterPlus.DataType.CLIENT);	
      }
      // Nowel
      if(player.getCurMap().getSubArea().getArea().getId() == 42) {
    	  player.teleport((short)7411, 311);
    	  player.sendMessage("(<b>Nowel</b>) : L'île est désormais fermé.");
      ...
      //Si dà Â¯àÂ¿àÂ½placement inutile
     ...
     // player.sendMessage(""+this.depla+"");
      if(this.player.walkFast || this.player.isInvisible())
      {
        ...
        //On dà Â¯àÂ¿àÂ½finie la case et on ajthise le this.playernnage sur la case
    	...
    	
    {
    	        Fighter fighter = this.player.getFight().getFighterByPerso(this.player);
    	          if (fighter != null) {
    	           if (this.player.cinvoc && 
    	           this.player.getFight().getFighterByOrdreJeu() != null && 
    	             this.player.getFight().getFighterByOrdreJeu().getInvocator() != null && 
    	           this.player.getFight().getFighterByOrdreJeu().getInvocator() == fighter) {
    	              fighter = this.player.getFight().getFighterByOrdreJeu();
    	         }
    	            
    	           if (fighter.getFight().cancast(fighter)) {
    	             GA.args = path;
    	              SocketManager.GAME_SEND_GAS_PACKET_TO_FIGHT(fighter.getFight(), 7, fighter.getId());
    	              this.player.getFight().onFighterDeplace(fighter, GA);
    	            SocketManager.GAME_SEND_GAF_PACKET_TO_FIGHT(fighter.getFight(), 7, 0, fighter.getId());
    	            } 
    	       } 
    		...
    		if(this.player.getParty().getMaster().getFight().getId() == this.getPlayer().getFight().getId())
    	{
    		...
    		final Fighter fighter1=this.getPlayer().getParty().getOne_windows().getFight().getFighterByPerso(this.getPlayer().getParty().getOne_windows());
    	      if(fighter1!=null)
    	      {
    	        GA.args=path;
    	        this.getPlayer().getParty().getOne_windows().getFight().cast(this.getPlayer().getParty().getOne_windows().getFight().getFighterByPerso(this.getPlayer().getParty().getOne_windows()),() -> this.getPlayer().getParty().getOne_windows().getFight().onFighterDeplace(fighter1,GA));
    	         return;
    	      }
    	} 
      final Fighter fighter1=this.player.getFight().getFighterByPerso(this.player);
      if(fighter1!=null)
      {
        GA.args=path;
        this.player.getFight().cast(this.player.getFight().getFighterByPerso(this.player),() -> this.player.getFight().onFighterDeplace(fighter1,GA));
      }
  ...
  private void gameCheckSign(String packet)
  {
    ...
    if(qp==null)
      quete.applyQuest(this.player); // S'il n'a pas la quà Â¯àÂ¿àÂ½te
    else
      SocketManager.GAME_SEND_MESSAGE(this.player,"Vous avez déjà Â  commencé cette quà Âªte.");
  ...
  private void gameTryCastSpell(String packet) {
      try {
          ...
          if (fight != null) {
              Fighter fighter = this.player.getFight().getFighterByPerso(this.player);
              Fighter current = this.player.getFight().getFighterByOrdreJeu();

              if (fighter != null ) {
                  if(this.player.cinvoc && current.getMob() != null) {
                      if(this.player.getFight().getFighterByOrdreJeu() != null)
                          if(current.getInvocator() != null)
                              if(current.getInvocator().getPersonnage() != null)
                                  if(current.getInvocator().getPersonnage()  == this.player)
                                      fighter = current;
                  }
            	  if(this.player.getParty() != null && this.player.getParty().getMaster() != null)
              		if(this.player.getParty().getMaster().isOne_windows())
              	...
              	if(this.player.getParty().getMaster().getId() == this.getPlayer().getId()) {
              	   ...
              	   if (SS == null) {
            	      ...
            	      if (Config.getInstance().serverId == 4) 
            	        	 arg = "Duo-Compte";
      					player.sendMessage("<b>[Bug Detected]</b> : Le joueur "+player.getName()+" a tenté d'utiliser un sort qu'il n'avait pas et a joué son tour trop vite. Afin d'éviter ce genre de bug, veuillez ne pas utiliser les raccourcis avant le début de votre tour.");
      					   this.sendDiscord(Constant.WEBHOOK_STAFFBUG, "**(" + arg + ")** : UseBug: Le joueur "+player.getName()+" a tenté d'utiliser un sort qu'il n'avait pas et a joué son tour trop vite ( Possible use bug ? ) à la  Map : " + this.player.getCurMap().getId() + ""); 
      						SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(player.getFight(), 7, 302,
      								player.getId() + "",9904+ "");// Echec
      															// Critique
      															// Cac
      						SocketManager.GAME_SEND_GAF_PACKET_TO_FIGHT(player.getFight(), 7, 0,
      								player.getId());// Fin
      													// de
      													// l'action
              		   return;
                   ...
                   if(SS!=null)
                     if(!this.player.getParty().getOne_windows().getFight().isCurAction() && !this.player.getFight().isTraped()) {
                  ...
                  /**  if(this.player.getFight().getCurAction().isEmpty())**/ {
                      ...
                      if(fighter.getPersonnage() != null)
                          SS = this.player.getSortStatBySortIfHas(id);
                      else
                          SS = fighter.getMob().getSortStatBySortIfHas(id);
                      ...
                      /**  if(this.player.getFight().getCurAction().isEmpty())**/ {
                          ...
                          SocketManager.GAME_SEND_GAS_PACKET_TO_FIGHT(fighter.getFight(), 7, fighter.getId());
                          fight.tryCastSpell(current, SS, cellId);

                          SocketManager.GAME_SEND_GAF_PACKET_TO_FIGHT(fighter.getFight(), 7, 0, fighter.getId());


                      }
                      if (id==432 && fighter.getPersonnage().getClasse() == Constant.CLASS_SACRIEUR){
          				fighter.addBuff(Constant.STATS_ADD_PERDOM, 30, 2, 2, false, 4001, "2,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_PERDOM, fighter.getId() + "", fighter.getId() + "," + 30 + "," + 2);  
          	       }
          	       if (id==184 && fighter.getPersonnage().getClasse() == Constant.CLASS_SADIDA){
          				fighter.addBuff(Constant.STATS_ADD_PERDOM, 30,1, 1, false, 4001, "1,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_PERDOM, fighter.getId() + "", fighter.getId() + "," + 30 + "," + 1);  
        
          	       }
          	       if (id==183 && fighter.getPersonnage().getClasse() == Constant.CLASS_SADIDA){
          				fighter.addBuff(Constant.STATS_ADD_PERDOM, 30,1, 1, false, 4001, "1,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_PERDOM, fighter.getId() + "", fighter.getId() + "," + 30 + "," + 1);  
        
          	       }
          	       if (id==188 && fighter.getPersonnage().getClasse() == Constant.CLASS_SADIDA){
          				fighter.addBuff(Constant.STATS_ADD_PM, 2,3, 1, false, 4001, "3,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_PM, fighter.getId() + "", fighter.getId() + "," + 2 + "," + 3);  
         
          	       }
          	       ///////////////////// Feca
          	       
          	       if (id==14 && fighter.getPersonnage().getClasse() == Constant.CLASS_FECA){
          				fighter.addBuff(Constant.STATS_ADD_RP_AIR, 10,3, 1, false, 4001, "3,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_RP_AIR, fighter.getId() + "", fighter.getId() + "," + 10 + "," + 3);  
         
          	       }
          	       if (id==6 && fighter.getPersonnage().getClasse() == Constant.CLASS_FECA){
          				fighter.addBuff(Constant.STATS_ADD_RP_TER, 10,3, 1, false, 4001, "3,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_RP_TER, fighter.getId() + "", fighter.getId() + "," + 10 + "," + 3);  
         
          	       }
          	       if (id==1 && fighter.getPersonnage().getClasse() == Constant.CLASS_FECA){
          				fighter.addBuff(Constant.STATS_ADD_RP_FEU, 10,3, 1, false, 4001, "3,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_RP_FEU, fighter.getId() + "", fighter.getId() + "," + 10 + "," + 3);  
         
          	       }
          	       if (id==18 && fighter.getPersonnage().getClasse() == Constant.CLASS_FECA){
          				fighter.addBuff(Constant.STATS_ADD_RP_EAU, 10,3, 1, false, 4001, "3,null,null,0,null", fighter, false);
      					SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(fight, 7, Constant.STATS_ADD_RP_EAU, fighter.getId() + "", fighter.getId() + "," + 10 + "," + 3);  
  	...
  	if(this.player.getParty().getMaster().getId() == this.getPlayer().getId()) 
    	...
    	if(this.player.getParty().getMaster().getId() == this.getPlayer().getId()) {
    		   ...
    		   final int cell=Integer.parseInt(packet.substring(5));
    		      if(this.player.getParty().getOne_windows().getFight()!=null&& !this.player.getParty().getOne_windows().getFight().isCurAction() && !this.player.getFight().isTraped()) {
  ...
  private void houseAction(String packet)
  {
    ...
    switch(actionID)
    {
      case 81://Và Â¯àÂ¿àÂ½rouiller maison
        h.lock(this.player);
        break;
  ...
  private void gameAggro(String packet)
  {
    try
    {
      ...
      if(this.player.cantAgro())
        return;
      if (this.player.getCurMap().isInArenaMap(this.player.getCurMap().getId())) 
    	  return;
      
      if(Config.singleton.serverId == 2000) {
      String map_pvp = ";7423;952;";
      if(!map_pvp.contains(";"+this.player.getCurMap().getId()+";"))
      {
   SocketManager.GAME_SEND_MESSAGE(this.player,"Merci d'utiliser la map PvP autorisé sur Naeris.");
      return;	  
      }
      }
      int id=Integer.parseInt(packet.substring(5));
     ...
     if(target == null || target.getGroupe() != null)
    	  return;
      if(Config.singleton.serverId == 2 && target.get_align() == 0)
    	  return;
      if(Config.singleton.serverId == 4 && target.get_align() == 0)
    	  return;
      ...
      if(Config.singleton.serverId == 2 || Config.singleton.serverId == 1 || Config.singleton.serverId == 4)
      if((Math.abs(target.getLevel() - this.player.getLevel())) >= 20){
    	  SocketManager.GAME_SEND_MESSAGE(this.player,"Vous devez agresser un personnage égal à votre niveau ou +- de 20 levels.");
    	    
    return;	  
      }
      if(Config.singleton.serverId == 2 || Config.singleton.serverId == 1 || Config.singleton.serverId == 3 || Config.singleton.serverId == 4)
          if((Math.abs(target.getPrestige() - this.player.getPrestige())) >= 4){
        	  SocketManager.GAME_SEND_MESSAGE(this.player,"Vous devez agresser un personnage égal à votre prestige ou +- de 3 prestiges de différences.");
     ...
     if(target.getAccount().getCurrentIp().compareTo(player.getAccount().getCurrentIp())==0)
      ...
      SocketManager.
      GAME_SEND_GA_PACKET_TO_MAP(this.player.getCurMap(),"",906,this.player.getId()+"",id+"");
  ...
  private void gameCollector(String packet)
  {
    try
    {
      ...
      Database.getStatics().getPlayerData().logs_perco(this.player.getName(), target.getGuildId());
  ...
  private void sendExtraInformations()
  {
    try
    {
      ...
      Main.world.showPrismes(this.player);
		...
		//Système incarnam
		if(player.getCurMap().getId() == 16080){
			soufix.other.tuto.tutorialIncarnam(player);
		}
		if(GameMap.isInStartMap(player.getCurMap().getId())){
    		TimerWaiterPlus.addNext(() -> {
    		this.send("kP");
    	 },500, TimerWaiterPlus.DataType.CLIENT);
    	}
			//Système songe
     // if(player.getCurMap().getId() == 16002){
		//soufix.other.tuto.tutoSonge(player);
	//}
      //Fin système songe
      if(player.getCurMap().getId() == 16081){
		soufix.other.tuto.tutorialIncarnam2(player);
  	  SocketManager.PACKET_POPUP_DEPART(this.player, "Afin de récupérer le Glouton, veuillez suivre le tutoriel !");  

	}
      if(player.getCurMap().getId() == 14056){
  		soufix.other.tuto.tutorialDuo(player);

  	}
      if(player.getCurMap().getId() == 16083){
		soufix.other.tuto.tutorialIncarnam3(player);
	}
     if(player.getCurMap().getId() == 16082){
	soufix.other.tuto.tutorialIncarnam4(player);
	}
 	World.get_Succes(player.getId()).Check(player, 24);


      //Fin système incarnam
      for(Player player : this.player.getCurMap().getPlayers())
      {
    	  if(player == null)
    		  continue;
      ...
      this.player.afterFight=false;
		...
		if(this.player.getCurMap().getId() == 10700) {
			  if(this.player.relique_paase)
				Main.world.getMap((short)10700).openKrala05();	
              ...
              if (isOk) {
            	 ...
            	 /* final Party party = this.player.getParty();
                  ...
                  if(party!=null&&this.player.getFight()==null&&party.getMaster()!=null&&party.getMaster().getName().equals(this.player.getName()))
                  {
                      ...
                      .forEach(follower -> follower.getGameClient().actionAck(packet)
                    );
                  }*/
      ...
      case 1://Deplacement
        if(isOk)
        {
          if(this.player.getFight()==null)
          {
            ...
            //On dà Â¯àÂ¿àÂ½finie la case et on ajoute le personnage sur la case
            if(nextCell!=null)
            {
              this.player.setCurCell(nextCell);
              this.player.setOldCell(nextCell.getId());
  ...
  private void leaveFight(String packet) {
      ...
      if (id > 0) {
          Player target = Main.world.getPlayer(id);
          //On ne quitte pas un joueur qui : est null, ne combat pas, n'est pas de Ã©a team.
  ...
  public void readyFight(String packet) {
      ...
      if(party!=null && this.player.isReadyFarm() && this.player.getParty().isChief(this.player.getId()))
      {
      	TimerWaiterPlus.addNext(() -> party.getPlayers().stream().forEach(follower -> follower.getGameClient().readyFight(packet)),700, TimerWaiterPlus.DataType.MAP);
      }
  }
  ...
  private void parseGuildPacket(String packet)
  {
    switch(packet.charAt(1))
    {
      ...
      case 'C'://Creation
        createGuild(packet);
        break;
      case 'f'://Tà Â¯àÂ¿àÂ½là Â¯àÂ¿àÂ½portation enclo de guilde
        teleportToGuildFarm(packet.substring(2));
        break;
      case 'F'://Retirer Collector
        removeTaxCollector(packet.substring(2));
        break;
      case 'h'://Tà Â¯àÂ¿àÂ½là Â¯àÂ¿àÂ½portation maison de guilde
      ...
      case 'T'://attaque sur Collector
        ...
        break;
      case 'V'://Ferme le panneau de crà Â¯àÂ¿àÂ½ation de guilde
  ...
  private void createGuild(String packet)
  {
    ...
    try
    {
      ...
      boolean isValid=true;
      //Và Â¯àÂ¿àÂ½rifie d'abord si il contient des termes dà Â¯àÂ¿àÂ½finit
      if(tempName.length()>30||tempName.contains("mj")||tempName.contains("modo")||tempName.contains("fuck")||tempName.contains("admin"))
      {
        isValid=false;
      }
		final String abcMin = "abcdefghijklmnopqrstuvwxyz- '";
      ...
      if(this.player.get_guild()!=null||this.player.getGuildMember()!=null)
      {
    	...
    	SocketManager.GAME_SEND_ERASE_ON_MAP_TO_MAP(this.player.getCurMap(), this.player.getId());
        SocketManager.GAME_SEND_ADD_PLAYER_TO_MAP(this.player.getCurMap(), this.player);
        SocketManager.GAME_SEND_MESSAGE(this.player,"(<b>Infos</b>) : Le nom de votre guilde a bien été changé en "+player.get_guild().getName()+" !");
      ...
      if(this.player.getCurMap().getId()==2196)//Temple de crà Â¯àÂ¿àÂ½ation de guilde
      {
  ...
  private void placeTaxCollector()
  {
    ...
    if(Config.singleton.serverId == 622222) {
        String map_pvp = ";7423;952;";
        if(!map_pvp.contains(";"+this.player.getCurMap().getId()+";"))
        {
    ...
    {//La Map possà Â¯àÂ¿àÂ½de un Collector
    ...
    {//La map ne possà Â¯àÂ¿àÂ½de pas de "places"
      SocketManager.GAME_SEND_Im_PACKET(this.player,"113");
      return;
    }
    ...
    switch(packet.charAt(0))
    {
      ...
      case 'E'://ou Refus
        if(packet.substring(1).equalsIgnoreCase(this.player.getInvitation()+""))
        {
          ...
          if(p==null)
            return;//Pas censà Â¯àÂ¿àÂ½ arriver
  ...
  private void banToGuild(String name)
  {
    ...
    else
    {
      ...
      {
        toRemGuild=Main.world.getGuild(this.player.get_guild().getId());//On prend la guilde du this.player qui l'à Â¯àÂ¿àÂ½jecte
      }
      ...
      if(toRemMember.getGuild().getId()!=this.player.get_guild().getId())
        return;//Si guilde diffà Â¯àÂ¿àÂ½rente
    }
    ...
    //S'il n'a pas le droit de kick, et que ce n'est pas lui mà Â¯àÂ¿àÂ½me la cible
  ...
  private void changeMemberProfil(String packet)
  {
	  if(this.player == null)
		  return;
    if(this.player.get_guild()==null)
      return; //Si le this.playernnage envoyeur n'a mà Â¯àÂ¿àÂ½me pas de guilde
    ...
    Player p=Main.world.getPlayer(guid); //Cherche le this.playernnage a qui l'on change les droits dans la mà Â¯àÂ¿àÂ½moire
    GuildMember toChange;
    GuildMember changer=this.player.getGuildMember();
    //Rà Â¯àÂ¿àÂ½cupà Â¯àÂ¿àÂ½ration du this.playernnage à Â¯àÂ¿àÂ½ changer, et verification de quelques conditions de base
    if(p==null) //Arrive lorsque le this.playernnage n'est pas chargà Â¯àÂ¿àÂ½ dans la mà Â¯àÂ¿àÂ½moire
    {
      int guildId=Database.getDynamics().getGuildMemberData().isPersoInGuild(guid); //Rà Â¯àÂ¿àÂ½cupà Â¯àÂ¿àÂ½re l'id de la guilde du this.playernnage qui n'est pas dans la mà Â¯àÂ¿àÂ½moire

      if(guildId<0)
        return; //Si le this.playernnage à Â¯àÂ¿àÂ½ qui les droits doivent à Â¯àÂ¿àÂ½tre modifià Â¯àÂ¿àÂ½ n'existe pas ou n'a pas de guilde

      if(guildId!=this.player.get_guild().getId()) //Si ils ne sont pas dans la mà Â¯àÂ¿àÂ½me guilde
      {
        SocketManager.GAME_SEND_gK_PACKET(this.player,"Ed");
        return;
      }
    ...
    {
      if(p.get_guild()==null)
        return; //Si la this.playernne à Â¯àÂ¿àÂ½ qui changer les droits n'a pas de guilde
    ...
    //Và Â¯àÂ¿àÂ½rifie ce que le this.playernnage changeur à Â¯àÂ¿àÂ½ le droit de faire

    if(changer.getRank()==1) //Si c'est le meneur
    {
      if(changer.getGuid()==toChange.getGuid()) //Si il se modifie lui mà Â¯àÂ¿àÂ½me, reset tthis sauf l'XP
      {
        rank=-1;
        right=-1;
      }
      ...
      //Si il modifie un autre membre
      {
        if(rank==1) //Si il met un autre membre "Meneur"
        {
          ...
          //Dà Â¯àÂ¿àÂ½fini les droits à Â¯àÂ¿àÂ½ mettre au nouveau meneur
          rank=1;
          xpGive=-1;
          right=1;
        }
  ...
  private void parseObjectPacket(String packet)
  {
    switch(packet.charAt(1))
    {
      ...
      case 'D':
    	  ...
    	  this.player.sendMessage("Erreur, il est impossible de jeter des objects au sol."); 
        //dropObject(packet);
        break;
      case 'M':
      ...
      case 'm':// disasociar mimobionte
    	  ...
    	  //this.player.sendMessage("Action Impossible: Merci de contacter un staff afin d'échanger votre mimibiote.");
          break;
      case 'L':
    	  VerouilleObject(packet);
...
*/
  ...
  { 
    ...
    try
    {
	  ...
	  if (player == null)
	    ...
	    if (player.getExchangeAction() != null) 
	    	return;
      /*if (this.player.getCurMap().getSong() == 0)
          if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
        		  || position==Constant.OBJETO_POS_GEMA_3|| position==Constant.OBJETO_POS_GEMA_4 || position==Constant.OBJETO_POS_GEMA_5
        		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
            	this.player.sendMessage("Vous devez être en songe pour équiper cette gemme.");
        	  return;
          }*/
      //// Position Dofus NEW QWAK /////////
    /*  if (player.getLevel() <= 219)
          if(object.getTemplate().getType() == 23 && object.containtTxtStats(Constant.ITEM_RARE) ) {
            	this.player.sendMessage("(<b>Infos</b>) : Vous devez être Oméga 20 afin d'équiper des Dofus Rares.");
        	  return;
          }
      if (player.getLevel() <= 229)
          if(object.getTemplate().getType() == 23 && object.containtTxtStats(Constant.ITEM_LEGENDAIRE) ) {
            	this.player.sendMessage("(<b>Infos</b>) : Vous devez être Oméga 30 afin d'équiper des Dofus Légendaires.");
        	  return;
          }
      if (player.getLevel() <= 249)
          if(object.getTemplate().getType() == 23 && object.containtTxtStats(Constant.ITEM_DIVIN) ) {
            	this.player.sendMessage("(<b>Infos</b>) : Vous devez être Oméga 50 afin d'équiper des Dofus Divin.");
        	  return;
          }*/
      if (player.getPrestige() <= 5) 
      if(position==Constant.ITEM_POS_DOFUS8) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 6 pour débloquer ce nouvel emplacement.");
    	  return;
      }
      
      if (player.getLevel() <= 209)

      if(position==Constant.ITEM_POS_DOFUS9) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être Oméga 10 pour débloquer ce nouvel emplacement.");
    	  return;
      }
      if (player.getLevel() <= 219) 
      if(position==Constant.ITEM_POS_DOFUS10) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être Oméga 20 pour débloquer ce nouvel emplacement.");
    	  return;
      }
      if (player.getLevel() <= 224) 
    	  if(position==Constant.ITEM_POS_DOFUS11) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être Oméga 25 pour débloquer ce nouvel emplacement.");
    	  return;
      }
      if (player.getLevel() <= 199)
    	  if(position==Constant.ITEM_POS_DOFUS7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être levels 200 pour débloquer ce nouvel emplacement.");
    	  return;
      }
      ...
      /////////////////////////////////
      if (this.player.getPrestige() != 1 && object.getTemplate().getId() == 101293 && this.player.getPrestige() != 2
    		  && this.player.getPrestige() != 3 && this.player.getPrestige() != 4 && this.player.getPrestige() != 5 && this.player.getPrestige() != 6
    		  && this.player.getPrestige() != 7 && this.player.getPrestige() != 8 && this.player.getPrestige() != 9 && this.player.getPrestige() != 10) // 100529 Prestige 2 
    	  // 100530 Prestige 3
    	  // 100531 Prestige 4
    	  // 100532 Prestige 5
      if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
    		  || position==Constant.OBJETO_POS_GEMA_3
    		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 1 pour équiper cette gemme.");
    	  return;
      }
      if (this.player.getPrestige() != 2 && object.getTemplate().getId() == 100529 && this.player.getPrestige() != 3
    		  && this.player.getPrestige() != 4 && this.player.getPrestige() != 5 && this.player.getPrestige() != 6
    		  && this.player.getPrestige() != 7 && this.player.getPrestige() != 8 && this.player.getPrestige() != 9 && this.player.getPrestige() != 10) // 100529 Prestige 2 
    	  // 100530 Prestige 3
    	  // 100531 Prestige 4
    	  // 100532 Prestige 5
      if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
    		  || position==Constant.OBJETO_POS_GEMA_3
    		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 2 pour équiper cette gemme.");
    	  return;
      }
      if (this.player.getPrestige() != 3 && object.getTemplate().getId() == 100530 && this.player.getPrestige() != 4
    		  && this.player.getPrestige() != 5 && this.player.getPrestige() != 6
    		  && this.player.getPrestige() != 7 && this.player.getPrestige() != 8 && this.player.getPrestige() != 9 && this.player.getPrestige() != 10) // 100529 Prestige 2 
    	  // 100530 Prestige 3
    	  // 100531 Prestige 4
    	  // 100532 Prestige 5
      if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
    		  || position==Constant.OBJETO_POS_GEMA_3 
    		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 3 pour équiper cette gemme.");
    	  return;
      }
      if (this.player.getPrestige() != 4 && object.getTemplate().getId() == 100531 && this.player.getPrestige() != 5 && this.player.getPrestige() != 6
    		  && this.player.getPrestige() != 7 && this.player.getPrestige() != 8 && this.player.getPrestige() != 9 && this.player.getPrestige() != 10) // 100529 Prestige 2 
    	  // 100530 Prestige 3
    	  // 100531 Prestige 4
    	  // 100532 Prestige 5
      if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
    		  || position==Constant.OBJETO_POS_GEMA_3
    		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 4 pour équiper cette gemme.");
    	  return;
      }
      if (this.player.getPrestige() != 5 && this.player.getPrestige() != 6
    		  && this.player.getPrestige() != 7 && this.player.getPrestige() != 8 && this.player.getPrestige() != 9 && this.player.getPrestige() != 10 && object.getTemplate().getId() == 100532) // 100529 Prestige 2 
    	  // 100530 Prestige 3
    	  // 100531 Prestige 4
    	  // 100532 Prestige 5
      if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
    		  || position==Constant.OBJETO_POS_GEMA_3
    		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 5 pour équiper cette gemme.");
    	  return;
      }
      if (this.player.getPrestige() != 8 && this.player.getPrestige() != 9 && this.player.getPrestige() != 10 && object.getTemplate().getId() == 100535) // 100529 Prestige 8
    	  // 100530 Prestige 3
    	  // 100531 Prestige 4
    	  // 100532 Prestige 5
      if(position==Constant.OBJETO_POS_GEMA_1 || position==Constant.OBJETO_POS_GEMA_2
    		  || position==Constant.OBJETO_POS_GEMA_3
    		  || position==Constant.OBJETO_POS_GEMA_6|| position==Constant.OBJETO_POS_GEMA_7) {
        	this.player.sendMessage("(<b>Infos</b>) : Vous devez être prestige 8 pour équiper cette gemme.");
    	  return;
      }
     // if (object.getTemplate().getId() >= 10922 && object.getTemplate().getId() <= 10954) {
      if (object.getTemplate().getTitre() >= 1) {
    	  //player.set_title(object.getTemplate().getTitre());
    	  player.set_title(object.getTemplate().getTitre());
    	  player.setAllTitle(String.valueOf(object.getTemplate().getTitre()));
      ...
      else
      {
       ...
       /* if(!object.getTemplate().getConditions().contains("PJ"))
        ...
        //On ne peut à Â¯àÂ¿àÂ½quiper 2 items de panoplies identiques, ou 2 Dofus identiques
        if(position!=Constant.ITEM_POS_NO_EQUIPED&&(object.getTemplate().getPanoId()!=-1||object.getTemplate().getType()==Constant.ITEM_TYPE_DOFUS||object.getTemplate().getType()==Constant.ITEM_TYPE_RELIQUE ||object.getTemplate().getType()==Constant.OBJETO_TIPO_GEMAS ||object.getTemplate().getType()==Constant.OBJETO_TIPO_CARTA1)&&this.player.hasEquiped(object.getTemplate().getId()))
        {
          return;
        }
        ...
        int objGUID=object.getTemplate().getId();
        //MIMO
        if (objTemplate.esMimo()) {
            objeto_Equipar_MIMO1(object, position);
            return;
        }
       //MIMO
        if (objTemplate.getType() == 176) {
        	//this.player.sendMessage("Test");
            objeto_Equipar_FM(object, position);
            return;
        }
        // CODE OBVI
        if(object.getTemplate().getType()==Constant.ITEM_TYPE_OBJET_VIVANT)
        {
         ...
         /* if (exObj.containtTxtStats(1016) || exObj.containtTxtStats(1017)){
        	  this.player.sendMessage("Désactivé pour les objects Légendaire/Rare, veuillez utiliser un object d'apparence.");
      		return;
      	}*/
          exObj.setObvijevanPos(object.getObvijevanPos()); // L'objet qui à Â¯àÂ¿àÂ½tait en place a maintenant un obvi
          Database.getDynamics().getObvejivanData().add(object,exObj);
        ...
        if(exObj!=null)//S'il y avait dà Â¯àÂ¿àÂ½ja un objet sur cette place on dà Â¯àÂ¿àÂ½sà Â¯àÂ¿àÂ½quipe
        {
          ...
          //On ne le possà Â¯àÂ¿àÂ½de pas
          {
              ...
              if ((idSetExObj >= 81 && idSetExObj <= 92) || (idSetExObj >= 201 && idSetExObj <= 212)) {
                  ...
                  for (String stat : stats) {
                      String[] val = stat.split("#");
                      String modifi = Integer.parseInt(val[0], 16) + ";" + Integer.parseInt(val[1], 16)
                              + ";0";
                      SocketManager.SEND_SB_SPELL_BOOST(this.player, modifi);
                      this.player.removeItemClasseSpell(Integer.parseInt(val[1], 16));
                  }
        ...
        {// Tourmenteur ; on dà Â¯àÂ¿àÂ½morphe
          if(Constant.isIncarnationWeapon(object.getTemplate().getId()))
            this.player.unsetFullMorph();
        }
        ...
        if (object.getTemplate().getId()==42185&&position==Constant.ITEM_POS_ARME)
        	SocketManager.GAME_SEND_OT_PACKET(this,42185);        	
        //Si objet de panoplie
        if(object.getTemplate().getPanoId()>0)
          SocketManager.GAME_SEND_OS_PACKET(this.player,object.getTemplate().getPanoId());
        if(this.player.getFight()!=null)
	...
	private void remover_Objeto_Aparencia(final GameObject item){
		try{
			int aparenciaOficial = Integer.parseInt(item.getParamStatTexto(Constant.STAT_ID_ITEM_REAL, 3));
			ObjectTemplate obj = Main.world.getObjTemplate(aparenciaOficial);
			if(obj == null)
				return;
			item.addStatTexto(Constant.STAT_APARENCIA_ITEM, "", false);
			item.addStatTexto(Constant.STAT_ID_ITEM_REAL, "", false);
			SocketManager.ENVIAR_OCK_ACTUALIZA_OBJETO(player, item);
			final GameObject obj1 = obj.createNewItem(1, true);
			player.addObjet(obj1, true);
            SocketManager.GAME_SEND_Ow_PACKET(player);
            SocketManager.GAME_SEND_ON_EQUIP_ITEM(this.player.getCurMap(), this.player);
		}catch(Exception e){
			e.printStackTrace();
  ...
  private void remove_mimi(String id) {
		//SocketManager.GAME_SEND_MESSAGE(player,"Cette fonction a été désactivé suite à  un bug.");	
			//return;
	  int guid;
	    try
	    {
	...
	GameObject obj=World.getGameObject(guid);
	 ...
	 if (obj.getPosition() != -1) {
			SocketManager.GAME_SEND_MESSAGE(player,"Veuillez retirer votre famillier avant.");
		 return;
	 }
  ...
  private void useObject(String packet)
  {
    ...
    if(!this.player.hasItemGuid(guid)||this.player.isAway()||this.player.getFight() != null)
      return;
    ...
    if(T.getType()==Constant.ITEM_TYPE_PAIN||T.getType()==Constant.ITEM_TYPE_VIANDE_COMESTIBLE || T.getId() == 994 || T.getId() == 995 || T.getId() == 993 || T.getType() == Constant.ITEM_TYPE_COMESTI_POISSON ) {
		if(player.getParty() != null && player.getParty().isChief(player.getId()) && player.getParty().getMaster() != null){
			for (final Player z :player.getParty().getPlayers()) {
				if (z.getGameClient() == null) {
					continue;
				}
				if(z.getFight() != null)
					continue;
					if(!z.getAccount().getCurrentIp().equals(player.getAccount().getCurrentIp()))
					continue;
				if(z.getId() == player.getId())
					continue;
				   if(!this.player.hasItemGuid(guid)||this.player.isAway())
					      return;
			     z.getGameClient().useObject("OU"+player.getItemGuid(T.getId()).getGuid()+"|");
			       // SocketManager.GAME_SEND_eUK_PACKET_TO_MAP(z.getCurMap(),z.getId(),17);
			        T.applyAction(this.player,z,guid,cellID,quantity);
  ...
  private void dissociateObvi(String packet)
  {
    ...
    obV.addStatTexto(Constant.ITEM_LEGENDAIRE, "", false);
    obV.addStatTexto(Constant.ITEM_RARE, "", false);
    obV.addStatTexto(Constant.ITEM_DIVIN, "", false);
    obV.addStatTexto(Constant.ITEM_RUNE, "", false);


    SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, obV);
    SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(player, obV);
    this.player.refreshStats();
    SocketManager.GAME_SEND_STATS_PACKET(this.player);
  ...
  private void leaveParty(String packet)
  {
    ...
    if(g==null)
      return;
    if(packet.length()==2)//Si aucun guid est spà Â¯àÂ¿àÂ½cifià Â¯àÂ¿àÂ½, alors c'est que le joueur quitte
    {
      g.leave(this.player);
  ...
  private void parseMountPacket(String packet)
  {
    switch(packet.charAt(1))
    {
      ...
      case 'p'://Demande Decription en Enclo.
        ...
        break;
      case 'f'://Libà Â¯àÂ¿àÂ½re la monture
        killMount(packet);
        break;
  ...
  private void boostSpell(String packet)
  {
    try
    {
      ...
      {
	      SocketManager.PACKET_POPUP_DEPART(this.player, "Vous n'avez pas le sort de votre classe, veuillez le drop afin de pouvoir monter votre sort.");
      }
    }
    catch(NumberFormatException e)
    {
 ...
 /* private void forgetSpell(String packet) // Parchomancia
  {
    if(this.player.isForgetingSpell()==false)
      return;
    ...
    int id=Integer.parseInt(packet.substring(2));
    if (Constant.isStartSpell(id)) {
    	this.player.sendMessage("(<b>Actions</b>) : Les sorts de base de classe ne peuvent pas être supprimé de vos sorts.\n Votre sort a été remis au niveau 1.");
        this.player.forgetSpell(id);
        SocketManager.GAME_SEND_SPELL_UPGRADE_SUCCED(this,id,this.player.getSortStatBySortIfHas(id).getLevel());
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
        this.player.setisForgetingSpell(false);
    	return;
    ...
    if(id==-1)
      this.player.setExchangeAction(null);
    if (id >= 53300 && id <= 53350) {
    	this.player.unlearnSpell(id);
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
        this.player.setisForgetingSpell(false);

    }
    if (this.player.getKamas() <= 50000) {
    	this.player.sendMessage("(<b>Actions</b>): Vous n'avez pas assez de kamas.");
        this.player.setisForgetingSpell(false);
        SocketManager.GAME_SEND_SPELL_LIST(this.player);
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
       Database.getStatics().getPlayerData().update(this.player);
    ...
    if(this.player.forgetSpell(id))
    {
    	this.player.setKamas(this.player.getKamas() - 50000);
        this.player.sendMessage("(<b>Actions</b>): Vous avez oublié le sort : ("+this.player.getSortStatBySortIfHas(id).getSpell().getNombre()+")\n Vous avez perdu <b>50 000 Kamas</b> afin de désapprendre ce sort.");
    	this.player.unlearnSpell(id);
        SocketManager.GAME_SEND_SPELL_LIST(this.player);
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
       Database.getStatics().getPlayerData().update(this.player);
  ...
  private void moveToUsed(String packet)
  {
    try
    {
      ...
      if (Spell == null) {
          SocketManager.PACKET_POPUP_DEPART(this.player, "Vous n'avez pas le sort de votre classe, veuillez le drop afin de pouvoir l'utiliser.");
          SocketManager.GAME_SEND_STATS_PACKET(this.player);
          SocketManager.GAME_SEND_SPELL_LIST(player);
          return;
      }
...
public void disconnect()
{
  ...
  public  void removeAction(GameAction GA)
  {
    ...
    if(actions.get(-1)!=null&&GA.actionId==1)//Si la queue est pas vide
    {
      ...
      //Risquà Â¯àÂ¿àÂ½ mais bon pas le choix si on veut pas à Â¯àÂ¿àÂ½tre emmerder avec les blà Â¯àÂ¿àÂ½s. Parser le bon type ?
...
Logging.getInstance().write("DDOS","IP Same Packet 500 out "+packet+" "+ip);
  ...
  private void recursiveBreakingObject(BreakingObject breakingObject, final int i, int count)
  {
    ...
    },200, TimerWaiterPlus.DataType.CLIENT);
  }
	  ...
	  switch(packet.charAt(2))
	    {
	      ...
	      case 'i': //Changer de morph
	    	 ...
	    	 player.sendMessage("Système désactivé.");
			break;
	    }
  }
  private void parseMimibiotePacket(String packet) {
      boolean debug = true;
      int price = 150;
      if (debug) {
          if (!packet.contains("XMN")) return;
          if (player == null || player.getAccount() == null || player.getFight() != null)
              return;

          String[] infos = packet.substring(3).split(Pattern.quote(";"));

          GameObject objStats = player.getItemTemplate(Integer.parseInt(infos[0]));
          GameObject objSkin = player.getItemTemplate(Integer.parseInt(infos[1]));

          if (objStats == null || objSkin == null) {
              SocketManager.GAME_SEND_POPUP(player, "Une erreur s'est produite !");
              return;
          }

          if (objStats.getTemplate().getType() != objSkin.getTemplate().getType()) {
              SocketManager.GAME_SEND_POPUP(player, "Vous devez prendre deux objet de type différents !");
              return;
          }

          if (objStats.getTemplate().getId() == objSkin.getTemplate().getId()) {
              SocketManager.GAME_SEND_POPUP(player, "Vous devez prendre deux objet différents !");
              return;
          }

          if (objStats.containtTxtStats(985)) {
              SocketManager.GAME_SEND_POPUP(player, "Vous ne pouvez encore changer cet item !");
              return;
          }

          if (objSkin == null || objSkin.getTemplate() == null
                  || objStats.getTemplate().getLevel() < objSkin.getTemplate().getLevel()
                  || objStats.getTemplate().getType() != objSkin.getTemplate().getType()
                  || !Constant.mimibioteItem(objSkin.getTemplate().getType())
                  || !Constant.mimibioteItem(objStats.getTemplate().getType())) {
              SocketManager.GAME_SEND_POPUP(player, "Ces deux objet ne peuvent pas être mimibiotés !");
              return;
          } else if (!player.getItems().containsValue(objSkin) || !player.getItems().containsValue(objStats)) {
              SocketManager.GAME_SEND_POPUP(player, "Une erreur s'est produite ! MISSITEM");
              return;
          }

          if (player.getAccount().getPoints() >= price) {
              player.getAccount().setPoints(player.getAccount().getPoints() - price);
              GameObject nuevo = GameObject.getCloneObjet(objSkin, 1);
              nuevo.setStats(objStats.getStats());
              nuevo.addTxtStat(985, objStats.getTemplate().getName());
              if (player.addObjet(nuevo, true)) {
                  Main.world.addGameObject(nuevo, true);
                  SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, nuevo);
                  SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(player, nuevo);
                  SocketManager.GAME_SEND_Ow_PACKET(player);
              }
              player.removeItem(Integer.parseInt(infos[0]), 1, true, true);
              player.removeItem(Integer.parseInt(infos[1]), 1, true, true);
              player.sendMessage("Vous venez de créer l'objet : <b>" + nuevo.getTemplate().getName() + "</b>");
              player.sendMessage("Vous avez perdu <b>" + price + "</b> points boutique.");
              return;
          } else {
              player.sendMessage("Il vous faut au minimum " + price + " points boutique pour faire un mimibiote.");
              return;
          }

      } else {
          player.sendMessage("Bientà´t !");
          return;
      }
  }
  ...
  public void changeName(String packet , boolean new_core)
  {
    ...
    if(name.length()>20||name.length()<2||name.contains("mod")||name.contains("admin")||name.contains("putain")||name.contains("administrator")||name.contains("puta")||name.contains("staff")||name.contains("owner")
    		||name.contains("Fondateur")||name.contains("Modérateur")||name.contains("Administrateur")||name.contains("Animateur")||name.contains("Modératrice")||name.contains("Pute")||name.contains("GameHelper"))
      isValid=false;
    if(isValid)
    {
      int tiretCount=0;
    ...
    {
      this.player.sendMessage("Le nom saisi est déjà Â  utilisé ou n'est pas autorisé.");
      return;
    }
...
private void Core(String packet)
{
  switch(packet.charAt(2)) {
    ...
    case '1':
    CommandePlayerAnemys.analyse(this.player,".maitre");
      break;
    case '2':
    CommandePlayerAnemys.analyse(this.player,".tp");
     break;
    case '3':
    	      String cell = "GDZ";
    	      String plus_ou_moin = "";
      ...
      break;
    case '4':
    	CommandePlayerAnemys.analyse(this.player,".help");
    break;
    case '5':
       CommandePlayerAnemys.analyse(this.player,".noall");
    break;
    case '6':
    	CommandePlayerAnemys.analyse(this.player,".boutique");
    break;
    case '7':
    	CommandePlayerAnemys.analyse(this.player,".hdv");
    break;
    case '8':
    	CommandePlayerAnemys.analyse(this.player,".banque");
    break;
    case '9':
    	CommandePlayerAnemys.analyse(this.player,".window");
    break;
    case 'Z':
    	CommandePlayerAnemys.analyse(this.player,".pcore");
    break;
    case 'P':
    	CommandePlayerAnemys.analyse(this.player,".pass");
    break;
    case 'O':
    	CommandePlayerAnemys.analyse(this.player,".movemule");
    break;
    case 'W':
    	CommandePlayerAnemys.analyse(this.player,".mcinvoc");
    break;
    case 'I':
    	CommandePlayerAnemys.analyse(this.player,".djmodu");
    break;
    case 'E':
    	CommandePlayerAnemys.analyse(this.player,".farm");
    break;
    case 'T':
    	CommandePlayerAnemys.analyse(this.player,".ipkamas");
    break;
    case 'R':
    	CommandePlayerAnemys.analyse(this.player,".ipdrop");
    break;
    case 'J':
    	CommandePlayerAnemys.analyse(this.player,".hard");
    break;
    case 'K':
    	CommandePlayerAnemys.analyse(this.player,".zaap");
    break;
    case 'L':
    	CommandePlayerAnemys.analyse(this.player,".tptuto");
    break;
    case 'o':
        if(player.songe_reset) {
	    	 SocketManager.GAME_SEND_MESSAGE(player,"(<b>Infos</b>) : Limitation à une fois par jour."); 
	    	 return;
	     }
	    player.songe_reset = true;
	    player.Song = 0;
	    SocketManager.GAME_SEND_MESSAGE(player,"(<b>Infos</b>) : Vos songe ont été remis à zéro !");
	  if(player.getLevel()<49)
     {
       SocketManager.GAME_SEND_MESSAGE(player,"Malheureusement vous ne pouvez pas passer dans le côté obscur dû à votre niveau... revenez me voir quand vous passerez levels 50.");
       return;
     }
	player.teleport((short) ((16003+player.Song)),339);
	 SocketManager.GAME_SEND_MESSAGE(player,"Vous êtes au <b>Songe Ultime</b> "+player.Song, "008000");
    	break;
    case 'l':
    	if (player.Song > 0 && player.Song <= 18) {
          	player.teleport((short) ((16003+player.Song)),339);
       	// SocketManager.GAME_SEND_MESSAGE(player,"Vous êtes au <b>Songe Ultime</b> "+player.Song, "008000");
       	 return;
    	  }
    	  if (player.Song >= 19) {
            	player.teleport((short) ((16066+player.Song)),339);
         	 //SocketManager.GAME_SEND_MESSAGE(player,"Vous êtes au <b>Songe Ultime</b> "+player.Song, "008000");
         	 return;
      	  }

          if(!player.hasItemTemplate(11560,1)) {
              SocketManager.GAME_SEND_MESSAGE(player,"Malheureusement vous ne pouvez pas passer dans le côté obscur... revenez me voir quand vous aurez la clef des songes.");
              return;
          }
    	  if(player.Song < 0)
    		  player.Song = 0;
    	  ...
    	  SocketManager.GAME_SEND_Im_PACKET(player,"022;"+1+"~"+11560);
          player.removeByTemplateID(11560,1);
    	player.teleport((short) ((16003+player.Song)),339);
    	 SocketManager.GAME_SEND_MESSAGE(player,"Vous êtes au <b>Songe Ultime</b> "+player.Song, "008000");
    	 break;
  }
...
/*private void MorphitemChange(final String packet){
	...
	SocketManager.GAME_SEND_ASK(this.player.getAccount().getGameClient(), this.player);
	SocketManager.PACKET_POPUP_DEPART(this.player, "Votre objet à  bien été transformé !");
...
private void objeto_Equipar_MIMO1(final GameObject objSkin, final int position) {

    GameObject objStats = player.getObjetByPos(position);

    if (objStats == null) {
        player.sendMessage("(<b>Infos</b>) : Vous devez équiper cet objet d'apparence sur un autre objet.");
        return;
    }
    if (objStats.getTemplate().getType() != objSkin.getTemplate().getType()) {
        SocketManager.GAME_SEND_POPUP(player, "Vous devez prendre deux objet de type différents !");
    ...
    if (objStats.getObvijevanPos() != 0) {
        SocketManager.GAME_SEND_POPUP(player, "Veuillez dissocier votre obvijevan avant de mettre votre skin d'apparence!");
        return;
    }
    ...
    if (objStats.containtTxtStats(Constant.STATS_MIMI_ID)) {
        SocketManager.GAME_SEND_POPUP(player, "Vous ne pouvez encore changer cet item !");
        return;
    }
	//ObjectTemplate T = World.getGameObject(objSkin).getTemplate().getId();
    if (objSkin == null || objSkin.getTemplate() == null
            || objStats.getTemplate().getType() != objSkin.getTemplate().getType()
            || !Constant.mimibioteItem(objSkin.getTemplate().getType())
            || !Constant.mimibioteItem(objStats.getTemplate().getType())) {
    ...
    if (objStats.getTemplate().getType() == 18) {
       // objStats.addStatTexto(Constant.STATS_MIMI, Integer.toHexString(objSkin
         //       .getTemplateID()), false);
        objStats.addTxtStat(Constant.STATS_MIMI_NOM, objSkin.getTemplate().getName());
        objStats.addTxtStat(Constant.STATS_MIMI_ID, objSkin.getTemplate().getId()+"");
        player.removeItem(objSkin.getGuid(), 1, true, true);
        this.player.refreshStats();
        SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, objStats);
        SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(player, objStats);
        SocketManager.GAME_SEND_ON_EQUIP_ITEM(this.player.getCurMap(), this.player);
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
        player.sendMessage("Vous venez de créer l'objet : <b>" + objStats.getTemplate().getName() + "</b>");
        Database.getDynamics().getObjectData().update(objStats);
        Database.getStatics().getPlayerData().update(this.player);
    } else {
        GameObject nuevo = GameObject.getCloneObjet(objStats, 1);
        nuevo.setStats(objStats.getStats());
      //  nuevo.addStatTexto(Constant.STATS_MIMI, Integer.toHexString(objSkin
                //.getTemplateID()), false);
        nuevo.addTxtStat(Constant.STATS_MIMI_NOM, objSkin.getTemplate().getName());
        nuevo.addTxtStat(Constant.STATS_MIMI_ID, objSkin.getTemplate().getId()+"");
        if (player.addObjet(nuevo, true)) {
            Main.world.addGameObject(nuevo, true);
            player.removeItem(objStats.getGuid(), 1, true, true);
            player.removeItem(objSkin.getGuid(), 1, true, true);
        }
        nuevo.setPosition(position);
        this.player.refreshStats();
        SocketManager.GAME_SEND_UPDATE_OBJECT_DISPLAY_PACKET(player, nuevo);
        SocketManager.GAME_SEND_OBJECT_QUANTITY_PACKET(player, nuevo);
        SocketManager.GAME_SEND_ON_EQUIP_ITEM(this.player.getCurMap(), this.player);
        SocketManager.GAME_SEND_STATS_PACKET(this.player);
        player.sendMessage("Vous venez de créer l'objet : <b>" + nuevo.getTemplate().getName() + "</b>");
        Database.getDynamics().getObjectData().update(nuevo);
        Database.getStatics().getPlayerData().update(this.player);
    }
}
...
private void Aventure(String packet)
{
     this.send("bd" + World.get_Aventure(this.getPlayer().getId()).getPassed() + "|" + World.get_Aventure(this.getPlayer().getId()).getPoints() + "|" + Main.world.Aventure_packet); 
...
private void succes(String packet)
{
     ...
     if(Config.singleton.serverId == 62222) {
     SocketManager.GAME_SEND_MESSAGE(this.player,""
    	 ...
    	 SocketManager.GAME_SEND_MESSAGE(this.player,"\nVos Statistiques :"
    	    		 ...
    	    		 +"\n</b>. Points boutique dépensés : <b>"+World.get_Succes(this.getPlayer().getId()).getBoutique()
    	    		 ...
    	    		 +"\n</b>. Objets Brisés : <b>"+World.get_Succes(this.getPlayer().getId()).getBrisage()+"</b>"
    	    		 ...
    	    		 +"\n</b>. Étages Songe Ultime : <b>"+this.getPlayer().Song+"</b>"
    	    		 +"\n</b>. Élevage Bébés : <b>"+World.get_Succes(this.getPlayer().getId()).getMount()+"</b>"
		    	     +"\n</b>. Quêtes Quotidiennes : <b>"+this.getPlayer().getNObjDiarios()+"</b>"
				    	     +"\n</b>. Dofus Drops : <b>"+this.getPlayer().getNbDofus()+"</b>"
...
private void batlle_pass () {
	 /*if(Config.singleton.serverId == 6) {
	  ...
	  if (player == null) {
			SocketManager.GAME_SEND_MESSAGE(player,"Action impossible, veuillez contacter un membre de l'équipe si le bug continue.");
	        return;
	  }
	    if (player.getGameClient() == null)  {
			SocketManager.GAME_SEND_MESSAGE(player,"Action impossible, veuillez contacter un membre de l'équipe si le bug continue.");
	...
	if(succes >= 400) {
		if(!ravens.contains("."+400+".")) {
			...
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+400+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(5,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
		        SocketManager.GAME_SEND_Im_PACKET(this.getPlayer(),"021;"+5+"~"+iditem);
		}
	}
	if(succes >= 500) {
		percent = 20;
		if(!ravens.contains("."+500+".")) {
			...
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+500+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(2,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
		        SocketManager.GAME_SEND_Im_PACKET(this.getPlayer(),"021;"+2+"~"+iditem);
		}
	}
	if(succes >= 600) {
		percent = 73;
	...
	this.send("wo1;item;0;400;0;42186;1;5,"
			+ "2;item;0;500;0;22006;0;2,"
			+ "3;item;0;600;0;9643;0;1,"
			+ "4;item;0;700;0;9642;0;1,"
			+ "5;item;0;800;0;45002;0;1,"
			+ "6;item;0;900;0;41035;1;1,"
			+ "7;item;0;1100;0;43006;0;1,"
			+ "8;item;0;1300;0;45000;1;1"
			+ ",info;"+percent+";1;0;0;"+World.get_Succes(this.getPlayer().getId()).getPoints());

}
private void batlle_pass2() {
	 /*if(Config.singleton.serverId == 6) {
		  this.player.sendMessage("indisponible pour le moment");
		  return;  
	  }*/
	...
	if(succes >= 2200) {
		percent = 410;
		if(!ravens.contains("."+2200+".")) {
			iditem = 25593;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+2200+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(1,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
	...
	if(succes >= 2500) {
		percent = 460;
		if(!ravens.contains("."+2500+".")) {
			iditem = 8988;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+2500+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(1,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
	...
	if(succes >= 2900) {
		percent = 460;
		if(!ravens.contains("."+2900+".")) {
			iditem = 6500005;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+2900+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(1,false);
			  if(obj.getTemplate().getType()==Constant.ITEM_TYPE_CERTIF_MONTURE)
		        {
	...
	if(succes >= 3300) {
		percent = 550;
		if(!ravens.contains("."+3300+".")) {
			iditem =  684;
			//iditem = 6500005;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+3300+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(10,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
		        SocketManager.GAME_SEND_Im_PACKET(this.getPlayer(),"021;"+10+"~"+iditem);
		}
	}
	if(succes >= 3600) {
		percent = 600;
		if(!ravens.contains("."+3600+".")) {
			iditem = 53347;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+3600+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(30,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
		        SocketManager.GAME_SEND_Im_PACKET(this.getPlayer(),"021;"+30+"~"+iditem);
		}
	}
	if(succes >= 4000) {
		percent = 630;
		if(!ravens.contains("."+4000+".")) {
			iditem = 59167;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+4000+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(1,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
	...
	if(succes >= 5000) {
		percent = 660;
		if(!ravens.contains("."+5000+".")) {
			iditem = 43000;	
			World.get_Succes(this.getPlayer().getId()).Ravens +=  "."+5000+".";
			 GameObject obj=Main.world.getObjTemplate(iditem).createNewItem(1,false);
		        if(this.getPlayer().addObjet(obj,true))
		          World.addGameObject(obj,true);
		        SocketManager.GAME_SEND_Ow_PACKET(this.getPlayer());
	...
	this.send("wo1;item;0;1800;0;45001;1;1,"
			//+ ",1;item;0;1800;0;22010;1,"
			+ "2;item;0;2200;0;25593;0;1,"
			+ "3;item;0;2500;0;8988;0;1,"
			+ "4;item;0;2900;0;6500005;0;1,"
			+ "5;item;0;3300;0;684;0;10,"
			+ "6;item;0;3600;0;53347;0;30,"
			+ "7;item;0;4000;0;59167;0;1,"
			+ "8;item;0;5000;0;43000;0;1"
			+ ",info;"+percent+";1;0;0;"+World.get_Succes(this.getPlayer().getId()).getPoints());
...
public void Prestige_send_information() {
	  
	  this.send("ki"+this.getPlayer().getPrestige()+"");
  }
public void PanelMaitre_send_information() {
	  if (player == null) {
			SocketManager.GAME_SEND_MESSAGE(player,"Action impossible, veuillez contacter un membre de l'équipe si le bug continue.");
	        return;
	  }
	  ...
	  this.send("kS"+this.getPlayer().getPrestige()+"");
}
public void PanelSonge_send_information() {
	  if (player == null) {
			SocketManager.GAME_SEND_MESSAGE(player,"Action impossible, veuillez contacter un membre de l'équipe si le bug continue.");
	        return;
	  }
	  ...
	  this.send("ku"+this.getPlayer().getNbItemTemplate(11502)+"");
}
public void PanelSongeID_send_information() {
	  if (player == null) {
			SocketManager.GAME_SEND_MESSAGE(player,"Action impossible, veuillez contacter un membre de l'équipe si le bug continue.");
	        return;
	  }
	  ...
	  this.send("ku"+this.player.Song+"");
}
public void PrestigeP_send_information() {
	  
	  this.send("kH"+this.getPlayer().getPrestige()+"");
}
public void PrestigeP1_send_information() {
	  
	  this.send("kU"+this.getPlayer().getPrestige()+"");
}
  ...
  public void Koli_add() {
	  ...
	  { 
	   SocketManager.GAME_SEND_MESSAGE(this.player,"Vous vous êtes inscrit au Kolizeum", "008000");
		SocketManager.GAME_SEND_MESSAGE_TO_ALL("(<b>Kolizeum</b>) : Le joueur <b>"+player.getName()+"</b> vient de s'inscrire en <b>Kolizeum</b> !", "581193");
	  }
	  else
	  {
		  ...
		  if(Liste.getValue() == null || Liste.getValue().statu != 0)
		   ...
		   if(Main.world.getPlayer(Liste.getValue().id_perso).getKamas() < Liste.getValue().kamas) {
			 ...
			 Database.getDynamics().getboursekamasData().delete_bourse(Liste.getValue().id);
			  Database.getDynamics().getboursekamasData().add_bourse_logs(0, Liste.getValue().id_perso, " auto delete pas assez de kamas offre id "+Liste.getValue().id+" kamas "+Liste.getValue().kamas+" points " +Liste.getValue().points+" taux "+Liste.getValue().taux);
  ...
  public void Bourse_add (String packet) {
	 /* if(this.player.getGroupe() != null) {
	  ...
	  leaveExchange(this.player);
		...
		if(have) {
			this.player.sendMessage("Vous possèdez déjà  une offre en cours");
			return;
		}
...
public void setPersonaje(Player perso) {
	this.player = perso;
}
	public PrintWriter get_out() {
		return _out;
	}
    private Player saveplayer = null;

	 public void setPlayer(Player perso) {
		 /*  107 */     this.player = perso;
		 /*      */   }
