...
public class Fight
{
  ...
  /*      */   private boolean modulaire = false;
  private boolean collectorProtect=false;
 // private boolean STAFF=false;
  private boolean MONO=false;
  /*   76 */   public int dgt = 1;
  /*   77 */   public int cacs = 1;
  ...
  public Fight(int type, int id, GameMap map, Player perso, Player init2)
  {
    ...
    setType(type); // 0: DÃ¯Â¿Â½fie (4: Pvm) 1:PVP (5:Perco)
    setId(id);
    ...
    // on desactive le timer de regen cotÃ¯Â¿Â½ client
  ...
  public Fight(int id, GameMap map, Player perso, MobGroup group, boolean mode)
  {
      if (perso == null) {
          return;
      } 
    ...
    setMobGroup(group);
    demorph(perso);
    setType(Constant.FIGHT_TYPE_PVM); // (0: DÃ¯Â¿Â½fie) 4: Pvm (1:PVP) (5:Perco)
    setId(id);
    setMap(map.getMapCopy());
    setMapOld(map);
    setInit0(new Fighter(this,perso));
    ...
    getTeam0().put(perso.getId(),getInit0());
     ...
     if (this.modulaire) {
    	     ...
    	     for (Map.Entry<Integer, MobGrade> entry : (Iterable<Map.Entry<Integer, MobGrade>>)group.getMobsSorted().entrySet()) {
    	        if (i < 4 && !getTeam1().containsKey(entry.getKey())) {
    	          ((MobGrade)entry.getValue()).setInFightID(((Integer)entry.getKey()).intValue());
    ...
    int c = PathFinding.getNearestCellAround(getInit0().getPersonnage().getCurMap(),
            getInit0().getPersonnage().getCurCell().getId(), group.getCellId(), new ArrayList<>());
    if (c < 0) {
        c = getInit0().getPersonnage().getCurCell().getId();
    }

    SocketManager.GAME_SEND_GAME_ADDFLAG_PACKET_TO_MAP(getInit0().getPersonnage().getCurMap(), 4,
            getInit0().getId(), group.getId(), c, "0;-1", group.getCellId(), "1;-1");
    SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(getInit0().getPersonnage().getCurMap(), getInit0().getId(),
            getInit0());
    ...
    if (perso != null) {
        if (perso.oldcell_fight != 0) {
            this.exchangePlace(perso, perso.oldcell_fight);
        }
    }
}
  ...
  public Fight(int id, GameMap map, Player perso, MobGroup group, int type,boolean protecteur)
  {
    ...
    setInit0(new Fighter(this,perso));
    getTeam0().put(perso.getId(),getInit0());
       this.modulaire = perso.DonjonModulaire;
  ...
  public Fight(int id, GameMap map, Player perso, Collector perco)
  {
    ...
    setType(Constant.FIGHT_TYPE_PVT); // (0: DÃ¯Â¿Â½fie) (4: Pvm) (1:PVP) 5:Perco
    setId(id);
    setMap(map.getMapCopy());
    setMapOld(map);
    setInit0(new Fighter(this,perso));
 ...
 /* public ArrayList<Pair<Integer, Fighter>> getDeadList()
  ...
  void scheduleTimer(int time)
  {
    ...
    },time*1000, TimerWaiterPlus.DataType.CLIENT);
  }
  ...
  public void startFight()
  {
    ...
    if(this.collector!=null&&!this.collectorProtect)
    {
      ...
      for(Player player : protectors)
      {
        if(player.getFight()==null&&!player.isAway())
        {
          ...
          TimerWaiterPlus.addNext(() -> this.joinCollectorFight(player,collector.getId()),time, TimerWaiterPlus.DataType.FIGHT);
        }
        else
        {
    ...
    if(getType()==Constant.FIGHT_TYPE_PVM||getType()==Constant.FIGHT_TYPE_DOPEUL)
    {
      ...
      String boss=".58.85.86.107.113.121.147.173.180.225.226.230.232.251.252.257.289.295.374.375.377.382.404.423.430.457.478.568.605.612.669.670.673.675.677.681.780.792.797.799.800.827.854.926.939.940.943.1015.1027.1045.1051.1071.1072.1085.1086.1087.1159.1184.1185.1186.1187.1188.519.1170.1195.";
try {
        ...
        if(fighter.getMob()!=null)
        {
          if(fighter.getMob().getTemplate()!=null)
          {
            ...
            if(Main.world.archi.contains("."+fighter.getMob().getTemplate().getId()+"."))
        		this.archi = true;
            if(fighter.getMob().getTemplate().getId() == 1072)
      ...
      String challenges=Main.world.getChallengeFromConditions(severalEnnemies,severalAllies,bothSexes,EvenEnnemies,MoreEnnemies,hasCawotte,hasChafer,hasRoulette,hasArakne,hasBoss,ecartLvlPlayer,hasArround,hasDisciple,getTeam0().size());
      String[] chalInfo;
      ...
      if(player!=null)
      {
    	 ...
    	 if(player.getSpioned_by().getCurMap().getId() == player.getCurMap().getId()) {
    		 if(player.getSpioned_by().getGameClient() != null) {
    			 TimerWaiterPlus.addNext(() ->   player.getSpioned_by().getGameClient().gameJoinFight("GA903"+this.id),900, TimerWaiterPlus.DataType.CLIENT);
    ...
    /* 1240 */     String IPS = "";
    /* 1241 */     int bonus = 1;
    /* 1242 */     for (Fighter f : getTeam0().values()) {
    /* 1243 */       Player player = f.getPersonnage();
    /* 1244 */       if (player != null && 
    /* 1245 */         !IPS.contains(player.getAccount().getCurrentIp())) {
    /* 1246 */         if (!IPS.isEmpty())
    /* 1247 */           bonus++; 
    /* 1248 */         IPS = IPS + player.getAccount().getCurrentIp() + ",";
    /*      */       } 
    /*      */     } 
    /*      */     
    /* 1252 */     if (bonus > 1) {
    /* 1253 */       for (Fighter f : getTeam0().values()) {
    /* 1254 */         Player player = f.getPersonnage();
    /* 1255 */         if (player != null) {
    /* 1256 */           f.bonusip = bonus;
    /* 1257 */          // player.sendMessage("Vous étes <b>" + bonus + "</b> personnes dans ce combat, vous bénéficicez donc d'un bonus Xp/Drop de <b>" + Formulas.GetBonusXP(bonus) + "</b>.");
    /*      */         } 
    /*      */       } 
    /*      */     }
    ...
    for(Fighter F : getFighters(3))
    {
          ...
          {
          /*
           * if (F.getPersonnage() != null) { if (F.getPersonnage().getClasse() ==
           * Constant.CLASS_PANDAWA && F.haveState(Constant.STATE_SOBER)) F.addBuff(138,
           * 85, 100, 126, false, 837, "85,null,null,100,0,0d0+85", F, true);
           *
           * }
           */
          // dark buff begin fight
          /* if (Config.singleton.CLASS_OSAMODAS_STATS != false && player.getClasse() == Constant.CLASS_OSAMODAS) {

    			// Buffos
    		
    			int inte = F.getTotalStats().get(Constant.STATS_ADD_INTE);
    			int fuerza = F.getTotalStats().get(Constant.STATS_ADD_FORC);
    			int agi = F.getTotalStats().get(Constant.STATS_ADD_AGIL);
    			int suerte = F.getTotalStats().get(Constant.STATS_ADD_CHAN);
    			int in = Constant.STATS_ADD_INTE;
    			int fo = Constant.STATS_ADD_FORC;
    			int ag = Constant.STATS_ADD_AGIL;
    			int su = Constant.STATS_ADD_CHAN;
    			F.addBuff(in, (int) (inte * 0.25), 200, 4013,
    					true, 0, (int) (inte * 0.25) + ",null,null,0,null", F, false);
    			F.addBuff(fo, (int) (fuerza * 0.25), 200, 4013,
    					true, 0, (int) (fuerza * 0.25) + ",null,null,0,null", F, false);
    			F.addBuff(ag, (int) (agi * 0.25), 200, 4013,
    					true, 0, (int) (agi * 0.25) + ",null,null,0,null", F, false);
    			F.addBuff(su, (int) (suerte * 0.25), 200, 4013,
    					true, 0, (int) (suerte * 0.25) + ",null,null,0,null", F, false);
    			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, in, F.getId() + "", F.getId() + "," + inte * 0.25 + "," + 999);
    			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, fo, F.getId() + "", F.getId() + "," + fuerza * 0.25 + "," + 999);
    			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, ag, F.getId() + "", F.getId() + "," + agi * 0.25 + "," + 999);
    			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, su, F.getId() + "", F.getId() + "," + suerte * 0.25 + "," + 999);


    			
          }
            if (Config.singleton.CLASS_ENUTROF_STATS != false && player.getClasse() == Constant.CLASS_ENUTROF) {
            ...
            if (Config.singleton.CLASS_SRAM_STATS != false && player.getClasse() == Constant.CLASS_SRAM) {
            ...
            if (Config.singleton.CLASS_XELOR_STATS != false && player.getClasse() == Constant.CLASS_XELOR) {
            ...
            if (Config.singleton.CLASS_ECAFLIP_STATS != false && player.getClasse() == Constant.CLASS_ECAFLIP) {
            ...
            if (Config.singleton.CLASS_ENIRIPSA_STATS != false && player.getClasse() == Constant.CLASS_ENIRIPSA) {
            ...
            if (Config.singleton.CLASS_CRA_STATS != false && player.getClasse() == Constant.CLASS_CRA) {
            ...
            if (Config.singleton.CLASS_FECA_STATS != false && player.getClasse() == Constant.CLASS_FECA) {
            ...
            if (Config.singleton.CLASS_SADIDA_STATS != false && player.getClasse() == Constant.CLASS_SADIDA) {
            ...
            if (Config.singleton.CLASS_SACRIEUR_STATS != false && player.getClasse() == Constant.CLASS_SACRIEUR) {
            ...
            if (Config.singleton.CLASS_PANDAWA_STATS != false && player.getClasse() == Constant.CLASS_PANDAWA ) {
        ...
        if (player.getClasse() == Constant.CLASS_IOP && player != null) {

  			// Buffos
  			F.addBuff(Constant.STATS_ADD_PDOM, 15, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PA, 1, 4, 2, false, 4001, "4,null,null,0,null", F, false);
  			int inte = F.getTotalStats().get(Constant.STATS_ADD_INTE);
  			int fuerza = F.getTotalStats().get(Constant.STATS_ADD_FORC);
  			int agi = F.getTotalStats().get(Constant.STATS_ADD_AGIL);
  			int suerte = F.getTotalStats().get(Constant.STATS_ADD_CHAN);
  			...
  			int ag = Constant.STATS_ADD_AGIL;
  			int su = Constant.STATS_ADD_CHAN;
  			F.addBuff(in, (int) (inte * 0.3), 200, 4013,
  					true, 0, (int) (inte * 0.3) + ",null,null,0,null", F, false);
  			F.addBuff(fo, (int) (fuerza * 0.3), 200, 4013,
  					true, 0, (int) (fuerza * 0.3) + ",null,null,0,null", F, false);
  			F.addBuff(ag, (int) (agi * 0.3), 200, 4013,
  					true, 0, (int) (agi * 0.3) + ",null,null,0,null", F, false);
  			F.addBuff(su, (int) (suerte * 0.3), 200, 4013,
  					true, 0, (int) (suerte * 0.3) + ",null,null,0,null", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, in, F.getId() + "", F.getId() + "," + inte * 0.3 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, fo, F.getId() + "", F.getId() + "," + fuerza * 0.3 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, ag, F.getId() + "", F.getId() + "," + agi * 0.3 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, su, F.getId() + "", F.getId() + "," + suerte * 0.3 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PDOM, F.getId() + "", F.getId() + "," + 15 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PA, F.getId() + "", F.getId() + "," + 1 + "," + 4);

  		

  			
        }
        if (player.getClasse() == Constant.CLASS_ENUTROF  && player != null) {

  			// Buffos
  			F.addBuff(Constant.STATS_ADD_PROS, 30, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_CHAN, 100, 100,2, false, 4001, "", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PROS, F.getId() + "", F.getId() + "," + 30 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_CHAN, F.getId() + "", F.getId() + "," + 100 + "," + 999);


  			
        }
        if (player.getClasse() == Constant.CLASS_ECAFLIP  && player != null) {
     	   int CC = F.getTotalStats().getEffect(Constant.STATS_ADD_CC) * 25 / 100;
     	   if (CC <= 5) {
     		   CC = 5;
     	   }
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_CC, CC, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PO, 1, 100, 2, false, 4001, "", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_CC, F.getId() + "", F.getId() + "," + CC + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PO, F.getId() + "", F.getId() + "," + 1 + "," + 999);


  			
       }
        if (player.getClasse() == Constant.CLASS_FECA  && player != null) {
     	   int CC = F.getTotalStats().getEffect(Constant.STATS_ADD_CC);
  			// Buffos
  			F.addBuff(Constant.STATS_RETDOM, 25, 4, 2, false, 4001, "4,null,null,0,null", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_AIR, 10, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_EAU, 10, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_TER, 10, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_FEU, 10, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_NEU, 10, 100, 2, false, 4001, "", F, false);
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_PDOM, 15, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PA, 1, 4, 2, false, 4001, "4,null,null,0,null", F, false);
  			int inte = F.getTotalStats().get(Constant.STATS_ADD_RP_FEU);
  			int fuerza = F.getTotalStats().get(Constant.STATS_ADD_RP_TER);
  			int agi = F.getTotalStats().get(Constant.STATS_ADD_RP_AIR);
  			int suerte = F.getTotalStats().get(Constant.STATS_ADD_RP_EAU);
  			int in = Constant.STATS_ADD_R_FEU;
  			int fo = Constant.STATS_ADD_R_TER;
  			int ag = Constant.STATS_ADD_R_AIR;
  			int su = Constant.STATS_ADD_R_EAU;
  			F.addBuff(in, 4, 200, 4013,
  					true, 0, 4 + ",null,null,0,null", F, false);
  			F.addBuff(fo, 4, 200, 4013,
  					true, 0, 4 + ",null,null,0,null", F, false);
  			F.addBuff(ag, 4, 200, 4013,
  					true, 0, 4 + ",null,null,0,null", F, false);
  			F.addBuff(su, 4, 200, 4013,
  					true, 0, 4 + ",null,null,0,null", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, in, F.getId() + "", F.getId() + "," + inte * 4 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, fo, F.getId() + "", F.getId() + "," + fuerza * 4  + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, ag, F.getId() + "", F.getId() + "," + agi * 4  + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, su, F.getId() + "", F.getId() + "," + suerte * 4 + "," + 999);

  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_RETDOM, F.getId() + "", F.getId() + "," + 25 + "," + 4);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_AIR, F.getId() + "", F.getId() + "," + 10 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_EAU, F.getId() + "", F.getId() + "," + 10 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_TER, F.getId() + "", F.getId() + "," + 10 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_FEU, F.getId() + "", F.getId() + "," + 10 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_NEU, F.getId() + "", F.getId() + "," + 10 + "," + 999);
        ...
        if (player.getClasse() == Constant.CLASS_PANDAWA  && player != null) {
     	   int CC = F.getTotalStats().getEffect(Constant.STATS_ADD_CC);
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_PM, 1, 4, 2, false, 4001, "4,null,null,0,null", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_AIR, 20, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_EAU, 20, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_TER, 20, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_FEU, 20, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_RP_NEU, 20, 100, 2, false, 4001, "", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PM, F.getId() + "", F.getId() + "," + 1 + "," + 4);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_AIR, F.getId() + "", F.getId() + "," + 20 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_EAU, F.getId() + "", F.getId() + "," + 20 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_TER, F.getId() + "", F.getId() + "," + 20 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_FEU, F.getId() + "", F.getId() + "," + 20 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_RP_NEU, F.getId() + "", F.getId() + "," + 20 + "," + 999);


  			
       }
        if (player.getClasse() == Constant.CLASS_OSAMODAS  && player != null) {
     	  
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_DOMA, 35, 100, 2, false, 4001, "", F, false);
  			//F.addBuff(Constant.STATS_ADD_PERDOM, 15, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PA, 1, 4, 2, false, 4001, "4,null,null,0,null", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_DOMA, F.getId() + "", F.getId() + "," + 35 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PA, F.getId() + "", F.getId() + "," + 1 + "," + 4);

  			
       }
        if (player.getClasse() == Constant.CLASS_ENIRIPSA  && player != null) {
     	   int Soin = (int) (F.getTotalStats().getEffect(Constant.STATS_ADD_SOIN) * 0.25);
     	   
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_SOIN, 25, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PERDOM, 15, 100, 2, false, 4001, "", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_SOIN, F.getId() + "", F.getId() + "," + 25 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PERDOM, F.getId() + "", F.getId() + "," + 15 + "," + 999);

  			
       }
        if (player.getClasse() == Constant.CLASS_SADIDA  && player != null) {
      		F.addBuff(Constant.STATS_ADD_PDOM, 15, 100, 2, false, 4001, "", F, false);
      		F.addBuff(Constant.STATS_ADD_SUM, 1, 100, 2, false, 4001, "", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PDOM, F.getId() + "", F.getId() + "," + 15 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_SUM, F.getId() + "", F.getId() + "," + 1 + "," + 999);

  		
        }
        if (player.getClasse() == Constant.CLASS_SACRIEUR  && player != null) {
     	   int Vita = F.getTotalStats().getEffect(Constant.STATS_ADD_VITA) / 100 * 20;
     	   if (Vita == 0) {
     		   Vita = 100;
     	   }    	   
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_VITA, Vita, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PERDOM, 10, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_REM_RP_AIR, 5, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_REM_RP_EAU, 5, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_REM_RP_TER, 5, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_REM_RP_FEU, 5, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_REM_RP_NEU, 5, 100, 2, false, 4001, "", F, false);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_VITA, F.getId() + "", F.getId() + "," + Vita + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_ADD_PERDOM, F.getId() + "", F.getId() + "," + 10 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_REM_RP_AIR, F.getId() + "", F.getId() + "," + 5 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_REM_RP_EAU, F.getId() + "", F.getId() + "," + 5 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_REM_RP_TER, F.getId() + "", F.getId() + "," + 5 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_REM_RP_FEU, F.getId() + "", F.getId() + "," + 5 + "," + 999);
  			SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this, 7, Constant.STATS_REM_RP_NEU, F.getId() + "", F.getId() + "," + 5 + "," + 999);

  			
       }
        if (player.getClasse() == Constant.CLASS_CRA  && player != null) {
     	   int Vie = (int) (F.getTotalStats().getEffect(Constant.STATS_ADD_VITA) * 0.2);
     	   
  			// Buffos
  			F.addBuff(Constant.STATS_ADD_PO, 1, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_DOMA,10, 100, 2, false, 4001, "", F, false);
  		      SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PO,F.getId()+"",F.getId()+","+1+","+999);
  		      SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_DOMA,F.getId()+"",F.getId()+","+10+","+999);

  		
  			
  			
       }
        if (player.getClasse() == Constant.CLASS_XELOR  && player != null) {
     	   //int Soin = F.getTotalStats().getEffect(Constant.STATS_ADD_SAGE) / 100 * 25;
     	
  			// Buffos
  			//F.addBuff(Constant.STATS_ADD_SAGE, Soin, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PA, 2, 3, 2, false, 4001, "3,null,null,0,null", F, false);
  			 int Soin = 25;
   		 	  
  	   		 	
  				// Buffos
  	 	   if(F.getPersonnage().getBuffsStats().get(Constant.STATS_ADD_SAGE) < 300) {
  	 	  F.addBuff(Constant.STATS_ADD_SAGE, Soin, 100, 2, false, 4001, "", F, false);
  	 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_SAGE,F.getId()+"",F.getId()+","+Soin+","+999);

  	 	   }
  	      SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PA,F.getId()+"",F.getId()+","+2+","+3);

  	
  			
  			
       }
        if (player.getClasse() == Constant.CLASS_SRAM  && player != null) {
     	  
  			// Buffos
  			

  			F.addBuff(Constant.STATS_ADD_CC, 15, 100, 2, false, 4001, "", F, false);
  			F.addBuff(Constant.STATS_ADD_PM, 1, 100, 2, false, 4001, "", F, false);
  		      SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_CC,F.getId()+"",F.getId()+","+15+","+999);
  		      SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PM,F.getId()+"",F.getId()+","+1+","+999);
              	     ...
              	     /* if(total == 2) {
              	      ...
              	      if(player!=null)
              	      {
              	       ...
              	       //	SocketManager.GAME_SEND_MESSAGE(player,"Bonus VIP 25% activé.","008000");
              	         // player.sendMessage("CELL : "+player.lastfightcell.getId()+" Map : "+player.lastfightmap+"");
              	    	if(bonus > 1)
              	        SocketManager.GAME_SEND_MESSAGE(player,"Vous étes <b>" + bonus + "</b> personnes dans ce combat, vous bénéficicez donc d'un bonus Xp/Drop de <b>" + Formulas.GetBonusXP(bonus) + "</b>","008000");
              	    	if(MONO)
              	        SocketManager.GAME_SEND_MESSAGE(player,"<b>[Mono-Compte</b>] : Vous étes <b>" + bonus + "</b> joueur dans ce combat, vous bénéficiez donc d'un bonus Xp/Drop de <b>" + Formulas.GetBonusXPMono(bonus) + "</b>.","008000");
              	    	/*if(DUO)
              	            SocketManager.GAME_SEND_MESSAGE(player,"<b>[DUO-Compte</b>] : Vous étes <b>" + bonus + "</b> joueur dans ce combat, vous bénéficiez donc d'un bonus Xp/Drop de <b>" + Formulas.GetBonusXPMono(bonus) + "</b>.","008000");
              	          */
              	    	if (Config.singleton.EVENTXP != false) {
              	           player.sendMessage("(<b>Évènement</b>) : Bonus XP activé, vous avez reçu un bonus de 25%.");

              	       }
              	    	/*if (Config.singleton.CLASS_FECA != false && player.getClasse() == Constant.CLASS_FECA) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_OSAMODAS != false && player.getClasse() == Constant.CLASS_OSAMODAS) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_CRA != false && player.getClasse() == Constant.CLASS_CRA) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_IOP != false && player.getClasse() == Constant.CLASS_IOP) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_ENUTROF != false && player.getClasse() == Constant.CLASS_ENUTROF) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_PANDAWA != false && player.getClasse() == Constant.CLASS_PANDAWA) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_SACRIEUR != false && player.getClasse() == Constant.CLASS_SACRIEUR) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_ENIRIPSA != false && player.getClasse() == Constant.CLASS_ENIRIPSA) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_ECAFLIP != false && player.getClasse() == Constant.CLASS_ECAFLIP) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_XELOR != false && player.getClasse() == Constant.CLASS_XELOR) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        } if (Config.singleton.CLASS_SADIDA != false && player.getClasse() == Constant.CLASS_SADIDA) {
              	      	   player.sendMessage("(<b>Roulette Classe</b>) : Vous avez été sélectionné aujourd'hui, vous gagnez 15% XP/Drop en plus !");

              	        }*/
              	    	if (player.hardlevel == 1 && player.isHardcore()) {
              	            player.sendMessage("(<b>HARDCORE 1</b>) : Bonus HARDCORE 1 activé, vous avez reçu un bonus de <b>50% Xp/Drop</b> pour votre combat.");

              	        }
              	    	if (player.hardlevel == 2 && player.isHardcore()) {
              	            player.sendMessage("(<b>HARDCORE 2</b>) : Bonus HARDCORE 2 activé, vous avez reçu un bonus de <b>90% Xp/Drop</b> pour votre combat.");

              	        }
              	    	if (player.hardlevel == 3 && player.isHardcore()) {
              	            player.sendMessage("(<b>HARDCORE 3</b>) : Bonus HARDCORE 3 activé, vous avez reçu un bonus de <b>125% Xp/Drop</b> pour votre combat.");

              	        }
              	    	if (player.isDuo()) {
              	            player.sendMessage("(<b>DUO</b>) : Bonus DUO activé, vous avez reçu un bonus de <b>25% Xp/Drop</b> pour votre combat.");

              	        }

              	       if (Config.getInstance().EVENTDROP != false) {
              	           player.sendMessage("(<b>Évènement</b>) : Bonus Drops activé, vous avez reçu un bonus de 25%.");
              	       ...
              	       /*
              	       if (Config.getInstance().EVENTMETIER != false) {
              	           player.sendMessage("(<b>événement Metier</b>) : Bonus XP activé, vous recevrez 50% de plus.");
              	    	...
              	    	/*Calendar c = Calendar.getInstance();
              	                       ...
              	                       // case 3: // Mardi
              	                        ...
              	                        //case 5: // Jeudi
              	                       //case 6:// Vendredi
              	                           // xpPlayer = (long)(xpPlayer * 1.5D);
              	                          player.sendMessage("(<b>événement</b>) : Bonus XP/Drop activé, vous recevrez 35% de plus.");
              	                           break;
              	                        }*/
              	   	/*	if (EventoInvasion.bendicionCaza == true) {
              			luchador.addBuff(Constant.STATS_ADD_INTE,
              					luchador.getTotalStats().getEffect(Constant.STATS_ADD_INTE) * 3, 100, 2, false, 3000,
              					"", luchador, false);
              			luchador.addBuff(Constant.STATS_ADD_FUERZA,
              					luchador.getTotalStats().getEffect(Constant.STATS_ADD_FUERZA) * 3, 100, 2, false, 3000,
              					"", luchador, false);
              			luchador.addBuff(Constant.STATS_ADD_SUERTE,
              					luchador.getTotalStats().getEffect(Constant.STATS_ADD_SUERTE) * 3, 100, 2, false, 3000,
              					"", luchador, false);
              			luchador.addBuff(Constant.STATS_ADD_AGIL,
              					luchador.getTotalStats().getEffect(Constant.STATS_ADD_AGIL) * 3, 100, 2, false, 3000,
              					"", luchador, false);

              		}*/
              	       
              				
                         player.oldcell_fight = player.getFight().getFighterByPerso(player).getCell().getId();

              			for (Fighter mob : getTeam1().values()) {
              			// Bendidció® ²
              			//  if (EventoInvasion.bendicionCaza == true && EventoInvasion.idBendicién == 2) {
              			//	int vidaMob = mob.getPdvMax() * 20 / 100;
              			//	mob.addBuff(153, vidaMob, 100, 2, false, 3000, "", mob, false);
              			//	mob.removePdv(mob, vidaMob);
              			//}
              			...
              			// MOdular
              				...
              				if (player.hardlevel == 1 && player.isHardcore()) {
              		   			int cantidad = 4;
              					int vidaMob = mob.getPdvMax() * cantidad / 25;
              					int dano = 0 + (10 * cantidad);
              					int stat = 0 + (45 * cantidad * 2);
              					// Buffos
              					mob.addBuff(Constant.STATS_ADD_VITA, vidaMob, 100, 2, false, 3001, "", mob, false);
              					mob.addBuff(Constant.STATS_ADD_PERDOM, dano, 100, 2, false, 3001, "", mob, false);
              					mob.addBuff(Constant.STATS_ADD_DOMA, dano, 100, 2, false, 3001, "", mob, false);
              					mob.addBuff(Constant.STATS_ADD_FORC, stat, 100, 2, false, 3001, "", mob, false);
              					mob.addBuff(Constant.STATS_ADD_INTE, stat, 100, 2, false, 3001, "", mob, false);
              					mob.addBuff(Constant.STATS_ADD_AGIL, stat, 100, 2, false, 3001, "", mob, false);
              					mob.addBuff(Constant.STATS_ADD_SAGE, stat, 100, 2, false, 3001, "", mob, false);
  ...
  public void leftFight(Player playerCaster, Player playerTarget)
  {
	    if(playerCaster==null)
	      ...
	      if (playerCaster.getCurMap().getId() == 16072 || playerCaster.getCurMap().getId() == 16073 || playerCaster.getCurMap().getId() == 16074) {
	     	 playerCaster.sendMessage("(<b>Évent</b>) : Impossible sur cette map !");
	     	 return;
	      } 
    if(caster!=null)
    {
      ...
      case Constant.FIGHT_TYPE_KOLI:
        ...
        case Constant.FIGHT_TYPE_DOPEUL:
          ...
          else if(getState()==Constant.FIGHT_STATE_PLACE)
          {
            ...
            {// Il leave de son plein grÃ¯Â¿Â½ donc (target = null)
              ...
              {// Soit il a lancer le combat => annulation du combat
                        ...
                        if(getType()==Constant.FIGHT_TYPE_AGRESSION)
                        {
                          for(Fighter enemy : (this.getTeam1().containsValue(caster) ? this.getTeam0() : this.getTeam1()).values())
                          {
                            if(enemy.getPersonnage()!=null)
                            {
                              ...
                              {
                            	  TimerWaiterPlus.addNext(() ->  player.teleportFaction(enemy.getPersonnage().get_align()),1500, TimerWaiterPlus.DataType.MAP);
                                break;
                              }
                            }
                          }
                      ...
                      else
                      {
                        ...
                        else
                        {
                          if(player.isOnline())
                          {
                            ...
                            TimerWaiterPlus.addNext(() ->  player.teleport(Short.parseShort(split[0]),Integer.parseInt(split[1])),1500, TimerWaiterPlus.DataType.MAP);
                          }
                          else
                          {
                ...
                this.setState(4);// Nous assure de ne pas dÃ¯Â¿Â½marrer le combat
              ...
              {// Soit il a rejoin le combat => Left de lui seul
                ...
                if(getType()==Constant.FIGHT_TYPE_AGRESSION||getType()==Constant.FIGHT_TYPE_PVM||getType()==Constant.FIGHT_TYPE_PVT||getType()==Constant.FIGHT_TYPE_CONQUETE||getType()==Constant.FIGHT_TYPE_DOPEUL)
                {
                  ...
                  else
                  {
                    ...
                    player.setPdv(player.getCurPdv());
                  }
                }
...
public void Anti_bug () {
 ...
 // ONE WINDOWS
    ...
    /* 1901 */     if (current.getPersonnage() != null && 
    		/* 1902 */       (current.getPersonnage()).cinvoc) {
		if (!current.isDouble()) {
    		/* 1903 */       current.getPersonnage().send(current.getPersonnage().getAsPacket(false, false));
    		/* 1904 */       current.getPersonnage().send("kI" + current.getId());
    		/* 1905 */       current.getPersonnage().send(current.getPersonnage().parseSpellList());
    		/* 1906 */       SocketManager.GAME_SEND_GAMETURNSTART_PACKET_LEADER(current, current.getPersonnage());
    		/* 1907 */       SocketManager.GAME_SEND_GAMETURNSTART_PACKET(current.getPersonnage(), current.getId(), 40000);
    		/* 1908 */       SocketManager.GAME_SEND_SB_PACKET(current.getPersonnage(), current.getPersonnage().getTotalBoostSpellStats(), true, null);
    		/* 1909 */       SocketManager.SEND_COOLDOWN(current, current.getPersonnage());
    		/*      */       
    		/*      */       return;
    		/*      */     } 
    }
    		/*      */     
    		/* 1914 */     if (current.isInvocation() && current.getInvocator() != null && current.getInvocator().getPersonnage() != null) {
    		/* 1915 */       Player Controller = current.getInvocator().getPersonnage();
    		/*      */ 
    		/*      */ 
    		/*      */ 
    		/*      */ 
    		...
    		/*      */ 
    		/*      */       
    		/* 1927 */       if (Controller.cinvoc) {
    		/* 1928 */         if (Controller.getFight() == null)
    		/* 1929 */           return; 
    							if (!current.isDouble()) {
    						if (Controller.getFight() == current.getFight()) {
    		/* 1930 */           Controller.send(current.getMob().getAsPacket(current));
    		/* 1931 */           Controller.send("kI" + current.getId());
    		/* 1932 */           Controller.send(current.getMob().parseSpellList(current));
    		/* 1933 */           SocketManager.GAME_SEND_ASK_LEADER_INVOC(Controller, current);
    		/* 1934 */           SocketManager.GAME_SEND_GAMETURNSTART_PACKET_LEADER(current, Controller);
    		/* 1935 */           SocketManager.GAME_SEND_GAMETURNSTART_PACKET(Controller, current.getId(), 40000);
    		/* 1936 */           SocketManager.SEND_COOLDOWN(current, Controller);
    		/*      */ 
    		/*      */           
    		/*      */           return;
    		/*      */         } 
    		...
    		/*      */     } 
    		if (current.getPersonnage() != null) {
          		 if (current.getPersonnage().getClasse() == Constant.CLASS_XELOR){
          		 	   int Soin = 5;
          		 	  
          		 	
          					// Buffos
          		 	   if(current.getPersonnage().getBuffsStats().get(Constant.STATS_ADD_SAGE) < 100) {
          		 	  current.addBuff(Constant.STATS_ADD_SAGE, Soin, 100, 2, false, 4001, "", current, false);
          		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_SAGE,current.getId()+"",current.getId()+","+Soin+","+999);

          		 	   }
          		   }
          		}
          		if (current.getPersonnage() != null) {
             		 if (current.getPersonnage().getClasse() == Constant.CLASS_SACRIEUR && current.getPorcPDV() < 51){
             		 	   int Soin = 5;
             		 	  
             		 	
             					// Buffos
             		 	 // current.addBuff(Constant.STATS_ADD_SAGE, Soin, 100, 2, false, 4001, "", current, false);
             		 	current.addBuff(Constant.STATS_ADD_PA, 4, 0, 4017, false, 0, "4,null,null,null,null",
      						current, false);
        		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PA,current.getId()+"",current.getId()+","+4+","+1);

             					
             		   }
             		}
          		// Cazador solitario
          		if (current.getPersonnage() != null && current.getPersonnage().getClasse() == Constant.CLASS_CRA) {
          			int alcance = current.getTotalStats().get(Constant.STATS_ADD_PO);
          			current.addBuff(Constant.STATS_ADD_PERDOM, alcance * 25, 0, 2, false, 4014, alcance * 25 + ",null,null,0,null", current, false);
          		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PERDOM,current.getId()+"",current.getId()+","+alcance * 25+","+1);

          		}
          		if (current.getPersonnage() != null && current.getPersonnage().getClasse() == Constant.CLASS_SADIDA) {
          			int alcance = current.getTotalStats().get(Constant.STATS_ADD_SUM);
          			current.addBuff(Constant.STATS_ADD_DOMA, alcance * 5, 0, 2, false, 4014, alcance * 5 + ",null,null,0,null", current, false);
          		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_DOMA,current.getId()+"",current.getId()+","+alcance * 5+","+1);
            			current.addBuff(Constant.STATS_ADD_MFLEE, alcance * 40, 0, 2, false, 4014, alcance * 40 + ",null,null,0,null", current, false);
        		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_MFLEE,current.getId()+"",current.getId()+","+alcance * 40+","+1);

          		}
          		if (current.getPersonnage() != null && current.getPersonnage().getClasse() == 5) { // Passif xel
          			if (current.pasiva9 > 0)
          				

          			current.addBuff(Constant.STATS_ADD_PERDOM, current.pasiva9, 0, 4009,
          					true, 0, current.pasiva9 + ",null,null,0,null", current, false);
          		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PERDOM,current.getId()+"",current.getId()+","+current.pasiva9+","+1);

          			current.pasiva9 = 0;

          		}
          		// Maestro invocador
          		if (current.getPersonnage() != null && current.getPersonnage().getClasse() == 2) {
          			int noInvo = current.nbInvocation();
          				current.addBuff(Constant.STATS_ADD_PROS, noInvo * 20, 2, 4004,
              					true, 0, noInvo * 20 + ",null,null,0,null", current, false);
          			current.addBuff(Constant.STATS_ADD_DOMA, noInvo * 10, 2, 4004,
          					true, 0, noInvo * 10 + ",null,null,0,null", current, false);
        		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PROS,current.getId()+"",current.getId()+","+noInvo * 20+","+2);
      		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_DOMA,current.getId()+"",current.getId()+","+noInvo * 10+","+2);

          		}
          		// Azote del destino
          		if (current.getPersonnage() != null && current.getPersonnage().getClasse() == 6) {
          			int rnd = Formulas.getRandomInt(0, 8);
          			getPasiva10(rnd);
          		}
          		// Cazador solitario
          		if (current.getPersonnage() != null && current.getPersonnage().getClasse() == 2) {
          			int noInvo = current.getTotalStats().get(Constant.STATS_ADD_SUM);
          			current.addBuff(Constant.STATS_ADD_PERDOM, noInvo * 20, 2, 4005,
          					true, 0, noInvo * 20 + ",null,null,0,null", current, false);
        		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PERDOM,current.getId()+"",current.getId()+","+noInvo * 20+","+2);
	...
	private void getPasiva10(int efecto) {
	    Fighter current=this.getFighterByOrdreJeu();
		...
		case 0:
			break;
		case 1:
			current.addBuff(Constant.STATS_ADD_PA, 4, 1, 4010,
					true, 0, 1 + "4,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PA,current.getId()+"",current.getId()+","+4+","+1);

			break;
		case 2:
		
			current.addBuff(Constant.STATS_ADD_PM, 4, 1, 4010,
					true, 0, 1 + "4,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PM,current.getId()+"",current.getId()+","+4+","+1);

			break;
		case 3:
			current.addBuff(Constant.STATS_ADD_PO, 5, 1, 4010,
					true, 0, 1 + "5,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PO,current.getId()+"",current.getId()+","+5+","+1);

			break;
		case 4:
			
			current.addBuff(Constant.STATS_ADD_CC, 15, 1, 4010,
					true, 0, 1 + "15,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_CC,current.getId()+"",current.getId()+","+15+","+1);

			break;
		case 5:
			current.setPdv(current.getPdv() + (current.getPdv() * 10 /100));
	          SocketManager.GAME_SEND_Im_PACKET_TO_FIGHT(this, 7, "A gagné" + current.getPdv() + (current.getPdv() * 10 / 100) + ".");
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_VITA,current.getId()+"",current.getId()+","+current.getPdv() + (current.getPdv() * 10 / 100)+","+0);

	          break;
		case 6:
			
			current.addBuff(Constant.STATS_ADD_DOMA, 50, 1, 4010,
					true, 0, 1 + "50,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_DOMA,current.getId()+"",current.getId()+","+50+","+1);

			break;
		case 7:
		
			current.addBuff(Constant.STATS_ADD_PERDOM, 250, 1, 4010,
					true, 0, 1 + "250,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_PERDOM,current.getId()+"",current.getId()+","+250+","+1);

			break;
		case 8:
			current.addBuff(Constant.STATS_ADD_AGIL, 300, 1, 4010,
					true, 0, 1 + "300,null,null,0,null", current, false);
			current.addBuff(Constant.STATS_ADD_FORC, 300, 1, 4010,
					true, 0, 1 + "300,null,null,0,null", current, false);
			current.addBuff(Constant.STATS_ADD_INTE, 300, 1, 4010,
					true, 0, 1 + "300,null,null,0,null", current, false);
			current.addBuff(Constant.STATS_ADD_SAGE, 300, 1, 4010,
					true, 0, 1 + "300,null,null,0,null", current, false);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_AGIL,current.getId()+"",current.getId()+","+300+","+1);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_FORC,current.getId()+"",current.getId()+","+300+","+1);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_INTE,current.getId()+"",current.getId()+","+300+","+1);
		 	  SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,Constant.STATS_ADD_SAGE,current.getId()+"",current.getId()+","+300+","+1);
  ...
  public void endTurn(final boolean onAction)
  {
    ...
    try
    {
      ...
      if (this.isCurAction() || this.isTraped()) {
    	 ...
    	 this.setCurAction(false);
    	 TimerWaiterPlus.addNext(() -> this.endTurn(onAction,current),100, TimerWaiterPlus.DataType.FIGHT);
        return;
      }
     /* if(current.getPersonnage() != null){
      ...
      if(onAction) {
    	  this.setCurAction(false);
    	  TimerWaiterPlus.addNext(() -> this.newTurn(current),300, TimerWaiterPlus.DataType.FIGHT);
      }
      else
      {
    	  this.setCurAction(false);
  ...
  private void doPoisons(Fighter current) //only for end-turn damage poisons
  {
    for(SpellEffect SE : current.getBuffsByEffectID(131))
    {
      ...
      /* 2030 */       if (current.getPa() == getCurFighterPa())
    	  /* 2031 */         nbr = 0; 
      ...
      if(SE.getSpell()==200) //Paralyzing poison
      {
       ...
       float dom=SE.getCaster().getTotalStats().getEffect(Constant.STATS_ADD_DOMA);
        if(pdom<0)
          pdom=0;
        dgt=(int)Math.floor(100 + inte + (pdom * 0.8) + dom) * dgt / 100;
      }
  ...
  private void newTurn(Fighter current)
  {
    ...
    // Timer d'une seconde Ã¯Â¿Â½ la fin du tour
    SocketManager.ENVIAR_GTM_INFO_STATS_TODO_LUCHADORES_A_TODOS(this, 7);
  ...
  public void joinFight(Player perso, int guid)
  {
    ...
    if(Config.singleton.serverId == 60)
    ...
    if(getTeam0().containsKey(guid))
    {
      ...
      if (Capture.isInEventMap(this.getMap().getId())) {
    	  int multiIp= 0;
          for(Fighter f : getTeam0().values())
            if(perso.getAccount().getCurrentIp().compareTo(f.getPersonnage().getAccount().getCurrentIp())==0)
          	  multiIp++;
          if(multiIp >= 1)
          {
            SocketManager.GAME_SEND_MESSAGE(perso,"Les combats en zone Évent sont limités à 1 IP par personnage.");
            return;
          }
      }
      if(isOnlyGroup0())
      ...
      if (getType()==Constant.FIGHT_TYPE_AGRESSION) {
          if(this.getTeam0().size()>=1||this.start0.size()==this.getTeam0().size()) 
        	     perso.sendMessage("Le PvP est seulement autorisé en 1V1.");
              return;
      }
      
      
    
      if (this.getMap().getId() == 19999) {
      ...
      if(this.getType() == Constant.FIGHT_TYPE_PVM)
    	  for(Fighter f : getTeam0().values()) {
              ...
              if (f.getPersonnage() != null && f.getPersonnage().isDuo())
             if(this.getTeam0().size()>=2||this.start0.size()==this.getTeam0().size())
                 return;
   	        } 
      if (this.getMap().getId() == 17502) {
      if(this.getTeam0().size()>=10||this.start0.size()==this.getTeam0().size())
          return;
      }
      ...
      if (this.getMap().getId() == 19999) {
      if(this.getTeam1().size()>=16||this.start1.size()==this.getTeam0().size())
          return;
      }
      if(this.getType() == Constant.FIGHT_TYPE_PVM)
          for(Fighter f : getTeam0().values()) {
             ...
             if (f.getPersonnage() != null && f.getPersonnage().isDuo())
             if(this.getTeam1().size()>=2||this.start1.size()==this.getTeam0().size())
                 return;
   	        } 

      if (this.getMap().getId() == 17502) {
      if(this.getTeam1().size()>=10||this.start1.size()==this.getTeam0().size())
          return;
      }
      if (getType()==Constant.FIGHT_TYPE_AGRESSION) {
          if(this.getTeam1().size()>=1||this.start1.size()==this.getTeam0().size()) 
        	     perso.sendMessage("Le PvP est seulement autorisé en 1V1.");
              return;
      }
    ...
    if(this.getType() == Constant.FIGHT_TYPE_PVM)
        for (Fighter fightere : getFighters(3)) {
      	  
           if (fightere.getPersonnage() != null && perso.hardlevel != 0)
 	          fightere.getPersonnage().sendMessage("(<b>Informations</b>) : Le joueur "+perso.getName()+" est en mode HARDCORE."); 
 	        } 
    if(getCollector()!=null)
    {
      for(Player z : Main.world.getGuild(getGuildId()).getOnlineMembers())
  ...
  void joinCollectorFight(final Player player, final int collector)
  {
    ...
    },700, TimerWaiterPlus.DataType.CLIENT);
  }
    ...
    if (getType() == Constant.FIGHT_TYPE_AGRESSION) {
        if (Cac_use == 2) {
			SocketManager.GAME_SEND_MESSAGE(perso,
					"(<b>Information</b>) : 2 coups de Corps à Corps par tour maximum", "FF0000");
  ...
  public  int tryCastSpell(Fighter fighter, SortStats spell, int cell) //0 = success, 5 = crit fail, 10 = error
  {
    ...
    SortStats Spell = spell;
    int caseID = cell;

    if (current.getMob() != null) {
        if (current.getMob().getTemplate().getId() == 605)
            if ((spell.getSpellID() == 142 || spell.getSpellID() == 1280) && current.haveState(Constant.ETAT_PESANTEUR))
                return 0;
    }
   ...
   // GameCase Cell=getMap().getCase(cell);
      GameCase targetCell = getMap().getCase(cell);
   // Player player=fighter.getPersonnage();
    if (current.getInvocator() != null && 
    		/* 2834 */       current.getInvocator().getPersonnage() != null && 
    		/* 2835 */       (current.getInvocator().getPersonnage()).cinvoc) {
    		/* 2836 */       player = current.getInvocator().getPersonnage();
    		/*      */     }
    ...
    //debut interdiction ligne de vue
    if (fighter.getMob() != null || fighter.isInvocation()) {
        switch (spell.getSpellID()) {
            ...
            case 329:
            case 331:
                cell = PathFinding.getcasebetwenEnemy(fighter.getCell().getId(), getMap(), this);
                break;
            case 2103:
            case 240:
                break;
            ...
            case 3627:
                ...
                if (numLunch - LaunchedSpell.getNbLaunch(fighter, spell.getSpellID()) <= 0 && numLunch > 0) {
                    return 66;
                }
                break;
            default:
                char dir = PathFinding.getDirBetweenTwoCase(fighter.getCell().getId(), caseID, getMap(), true);
                char diroposer = 0;
                switch (dir) {
                    case 'h':
                        diroposer = 'd';
                ...
                if (caseID != fighter.getCell().getId() && spell.hasLDV() && !PathFinding.checkLignedevue(getMap(), fighter.getCell().getId(), caseID, null))
                    return 0;
                //debut sï¿½curitï¿½
                boolean cac = PathFinding.getcaseCAC(caseID, getMap(), this, fighter);
                if (PathFinding.getcasebetwenangle(caseID, getMap(), this, dirs) == false && cac == false && diroposer != 0)
                    return 0;
                //fin sï¿½curitï¿½
                break;
        }
    ...
    if (current.getMob() != null) {
        boolean canlunch = true;
        for (SpellEffect spellEffect : spell.getEffects()) {
            switch (spellEffect.getEffectID()) {
                case 4:
                case 8:
                    if (current.haveState(Constant.ETAT_PESANTEUR) || current.haveState(Constant.ETAT_ENRACINE))
                        canlunch = false;
                    break;            
            }
        }
  ...
  public boolean onFighterDeplace(Fighter fighter, GameAction GA)
  {
    ...
    Player player = current.getPersonnage();
    /*      */     
    /* 3369 */     if (current.getInvocator() != null && 
    /* 3370 */       current.getInvocator().getPersonnage() != null && 
    /* 3371 */       (current.getInvocator().getPersonnage()).cinvoc) {
    /* 3372 */       player = current.getInvocator().getPersonnage();
    /*      */     }
    ...
    if(targetTacle!=null&&!fighter.haveState(Constant.ETAT_ENRACINE)&&!fighter.haveState(Constant.ETAT_PORTE)&&!fighter.isHide())
    {
      ...
      int esquiveTerre=Formulas.getTacleChanceTerre(fighter,targetTacle);
      int esquiveFeu=Formulas.getTacleChanceFeu(fighter,targetTacle);
      int esquiveEau=Formulas.getTacleChanceEau(fighter,targetTacle);

      int rand=Formulas.getRandomValue(0,99);
      if(targetTacle.haveState(Constant.STATE_UNDODGEABLE))
        rand=100;
           for (Fighter fightere : getFighters(3)) {
              if (fightere.getPersonnage() != null)
    	          fightere.getPersonnage().sendMessage("La chance d'esquiver le tacle est : <b>" + esquive + "%</b>."); 
    	        } 
          /* if(rand>esquiveTerre)
           {
             SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,GA.id,"104",fighter.getId()+";","");
             int pierdePA=getCurFighterPa()*esquiveTerre/100;
             if(pierdePA<0)
               pierdePA=-pierdePA;
           ...
           {
             SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,GA.id,"104",fighter.getId()+";","");
             int pierdePA=getCurFighterPa()*esquiveFeu/100;
             if(pierdePA<0)
               pierdePA=-pierdePA;
             if(getCurFighterPm()<0)
               setCurFighterPm(0);
           ...
           {
             SocketManager.GAME_SEND_GA_PACKET_TO_FIGHT(this,7,GA.id,"104",fighter.getId()+";","");
             int pierdePA=getCurFighterPa()*esquiveEau/100;
             if(pierdePA<0)
               pierdePA=-pierdePA;
             if(getCurFighterPm()<0)
               setCurFighterPm(0);
    ...
    else
    {
      if(current.getPersonnage()!=null)
      {
        // On envoie le path qu'au joueur qui se dÃ¯Â¿Â½place
    ...
    if(po!=null)// mÃ¯Â¿Â½me erreur que tantÃ¯Â¿Â½t, bug ou plus de fighter sur la
      // case
    ...
    {
      // on dÃ¯Â¿Â½place le portÃ¯Â¿Â½ sur la case
  ...
  public void onFighterDie(Fighter target, Fighter caster)
  {
    ...
    try
    {
      ...
      target.getCell().getFighters().clear();// Supprime tout causait bug si portÃ¯Â¿Â½/porteur
      if(target.haveState(Constant.ETAT_PORTEUR))
      {
        ...
        f.setState(Constant.ETAT_PORTE,0,caster.getId());// J'ajoute ceci quand mÃ¯Â¿Â½me pour signaler qu'ils ne sont plus en Ã¯Â¿Â½tat portÃ¯Â¿Â½/porteur
      ...
      if(target.haveState(Constant.ETAT_PORTE))
    	  TimerWaiterPlus.addNext(() -> removeCarry(target),3000, TimerWaiterPlus.DataType.CLIENT); //timer so mob only gets dropped after already despawning
      ...
      else if(target.getTeam()==1)
      {
        ...
        for(Fighter fighter : team.values())
        {
          ...
          {// si il a Ã¯Â¿Â½tÃ¯Â¿Â½ invoquÃ¯Â¿Â½ par le joueur mort
      ...
      if(target.getMob()!=null)
      {
        try
        {
          if(target.isInvocation()&&!target.isStatique)
          {
            ...
            // prÃ¯Â¿Â½cedent pour que le startTurn passe au suivant
            ...
            if(this.getOrderPlaying()!=null&&!this.getOrderPlaying().isEmpty())
            {
              ...
              // Si le joueur courant a un index plus Ã¯Â¿Â½levÃ¯Â¿Â½, on le
              // diminue pour Ã¯Â¿Â½viter le outOfBound
              if(index!=-1)
              {
  ...
  private GameCase getRandomCell(List<GameCase> cells)
  {
	...
	private GameCase getCeldaRandom(final List<GameCase> celdas) {
		if (celdas.isEmpty()) {
			return null;
		}
		final ArrayList<GameCase> celdas2 = new ArrayList<>();
		for (GameCase c : celdas) {
			if (c == null || c.getFighters() != null) {
				continue;
			}
			celdas2.add(c);
		}
  ...
  void addChevalier()
  {
	...
	void addPoup() {
		...
		for (Fighter F : getTeam0().values()) {
			...
			if (getTeam1().size() > getTeam0().size())
				continue;
			groupData = groupData + "394," + Constant.getLevelForChevalier(F.getPersonnage()) + ","
		...
		for (Entry<Integer, Fighter> entry : e) {
			...
			if (cell == null) {
				...
				f.getCell().addFighter(f);
				f.setTeam(1);
				SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(getInit0().getPersonnage().getCurMap(),
						getMobGroup().getId(), entry.getValue());
				SocketManager.GAME_SEND_ADD_IN_TEAM_PACKET_TO_MAP(getInit1().getPersonnage().getCurMap(),
						getMobGroup().getId(), entry.getValue());
			}
		}
		SocketManager.GAME_SEND_MAP_FIGHT_GMS_PACKETS_TO_FIGHT(this, 7, getMap());
  ...
  {
    // True si entre en mode déconnexion en combat, false sinon
  ...
  public boolean playerReconnect(final Player perso)
  {
    ...
    SocketManager.ENVIAR_GTM_INFO_STATS_TODO_LUCHADORES_A_TODOS(f.getFight(), 7);
      ...
      for(Fighter fighter : fighters)
      {
        ...
        if(!player.isFollowing)
        {
          if(Config.getInstance().teleports.containsKey((int)player.getCurMap().getId()))
          {
            ...
            TimerWaiterPlus.addNext(() ->  player.teleport(player.getCurMap(),teleCell),1500, TimerWaiterPlus.DataType.MAP);
          }
        }
...
/* if(player.getMetiers().size()>0)
 {
   ...
   if(obj!=null)
    ...
    for(Fighter f : looseTeam)
    {
      ...
      int id=f.getMob().getTemplate().getId();
        ...
        SocketManager.GAME_SEND_MESSAGE(player,"La relique du jour est <b>["+Main.relique_donjon+"]</b> pour <b>Antre du kralamour </b> est activé.","257C38");
        }
    
   /* int id2=f.getMob().getTemplate().getId();
    if(id2 == Main.id_mob_relique_djoff) {
    	player.relique_paase = false;
    SocketManager.GAME_SEND_MESSAGE(player,"La relique du jour a été désactivé, vous devez refaire le  <b>["+Main.relique_donjon+"]</b> pour ouvrir <b>Antre du kralamour </b>.","257C38");
    }*/
    }
    ...
    if(this.getType()==Constant.FIGHT_TYPE_KOLI) {
    	TimerWaiterPlus.addNext(() ->  player.teleportOld(),6000, TimerWaiterPlus.DataType.MAP);
    ...
    if(this.getType()==Constant.FIGHT_TYPE_PVM)
    {
    	...
    	GameObject obj2=Main.world.getObjTemplate(11502).createNewItem(1 * player.Song,false);
        if(player.addObjet(obj2,true))
          World.addGameObject(obj2,true);
        SocketManager.GAME_SEND_Ow_PACKET(player);
        SocketManager.GAME_SEND_Im_PACKET(player,"021;"+1 * player.Song+"~"+11502);
    	}
		//pjGanador.verificarPeleasLogros(_mobGrupo.getAlmasMobs(), this.getMapaReal().esMazmorra());
    	...
    	if (this.classement)
    		player.setBossClassement(player.getBossClassement() + 1);
    	World.get_Succes(player.getId()).level(player);

        player.verificarPeleasLogros(mobGroup.getAlmasMobs(), true);
    	World.get_Succes(player.getId()).Check(player, 25);

  		//player.confirmarLogrosPelea(mobGroup.getAlmasMobs());
  ...
  //ONE WINDOWS
    ...
    if(this.getType()==Constant.FIGHT_TYPE_KOLI) {
    	TimerWaiterPlus.addNext(() ->  player.teleportOld(),6000, TimerWaiterPlus.DataType.MAP);
    	 ...
    	 player.remHonor(150);
    	 if(player.get_honor()<0)
    	        player.set_honor(0);
    	 player.sendMessage("<b>(Koli)</b> : Vous avez perdus 150 honneurs pour votre défaite.");

    }
  if (this.getType() == Constant.FIGHT_TYPE_PVM) {
	   // Donjon.VerifyLastMapIfloose(player);
        ...
        if(energy<=0)
        {
          if(this.getType()==Constant.FIGHT_TYPE_AGRESSION&&fighter.getTraqued())
          {
            if(getTeam1().containsValue(fighter))
            	TimerWaiterPlus.addNext(() ->  player.teleportFaction(this.getAlignementOfTraquer(this.getTeam0().values(),player)),1500, TimerWaiterPlus.DataType.MAP);
            else
            	TimerWaiterPlus.addNext(() ->  player.teleportFaction(this.getAlignementOfTraquer(this.getTeam0().values(),player)),1500, TimerWaiterPlus.DataType.MAP);
            player.setEnergy(1);
          }
    ...
    if(this.getType()!=Constant.FIGHT_TYPE_CHALLENGE
    		&&this.getType()!=Constant.FIGHT_TYPE_KOLI && this.getType()!= Constant.FIGHT_TYPE_PVM)
    {
      int loose=Formulas.getLoosEnergy(player.getLevel(),getType()==1,getType()==5);
      int energy=player.getEnergy()-loose;
      ...
      try {
          ...
          for (Fighter fighter : loosers) {
              if (fighter.getMob() == null || fighter.getMob().getTemplate() == null
                      || !fighter.getMob().getTemplate().isCapturable()) {
                  mobCapturable = false;
              }
              if (fighter.getMob() != null && fighter.getMob().getTemplate() != null) {
          ...
          if (this.getMapOld().getSong() != 1) {
              if (mobCapturable && !Capture.isInArenaMap(this.getMapOld().getId())) {
                  ...
                  for (Fighter fighter : loosers) {
                      ...
                      if (fighter.getMob() != null) {
                          if (fighter.getMob().getTemplate().getId() == 499) {
                              mino = true;
                          }
                      }
                  }

                  if (Capture.IsLastMap(this.getMapOld().getId())) {
                      this.setFullSoul(new Capture(Database.getDynamics().getObjectData().getNextId(), 1, 10417,
                              Constant.ITEM_POS_NO_EQUIPED, stats)); // Crï¿½e la pierre d'ï¿½me
                  } else if (mobGroup.getIsExtraGroup() || mino) {
                      this.setFullSoul(new Capture(Database.getDynamics().getObjectData().getNextId(), 1, 10418,
                              Constant.ITEM_POS_NO_EQUIPED, stats)); // Crï¿½e la pierre d'ï¿½me
                  } else {
                      this.setFullSoul(new Capture(Database.getDynamics().getObjectData().getNextId(), 1, 7010,
                              Constant.ITEM_POS_NO_EQUIPED, stats)); // Crï¿½e la pierre d'ï¿½me
                  }

                  winners.stream().filter(F -> !F.isInvocation() && F.haveState(Constant.ETAT_CAPT_AME))
                          .forEach(F -> getCapturer().add(F));

                  if (this.getCapturer().size() > 0 && !Capture.isInArenaMap(this.getMapOld().getId())) // S'il y a
                  // des
                  // captureurs
                  {
                      for (int i = 0; i < this.getCapturer().size(); i++) {
                          try {
                              Fighter f = this.getCapturer()
                                      .get(Formulas.getRandomValue(0, this.getCapturer().size() - 1)); // Rï¿½cupï¿½re
                              // un
                              // captureur
                              // au hasard
                              // dans la
                              // liste
                              if (f != null && f.getPersonnage() != null) {
                                  if (f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME) == null
                                          || !(f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME).getTemplate()
                                          .getType() == Constant.ITEM_TYPE_PIERRE_AME)) {
                                      this.getCapturer().remove(f);
                                      continue;
                                  }
                                  Pair<Integer, Integer> playerSoulStone = Formulas
                                          .decompPierreAme(f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME));// Rï¿½cupï¿½re
                                  // les
                                  // stats
                                  // de
                                  // la
                                  ...
                                  if (playerSoulStone.getRight() < maxLvl) { // Si la pierre est trop faible
                                      this.getCapturer().remove(f);
                                      continue;
                                  }
                                  if (Formulas.getRandomValue(1, 100) <= Formulas
                                          .totalCaptChance(playerSoulStone.getLeft(), f.getPersonnage())) { // Si le
                                      // joueur
                                      // obtiens
                                      // la
                                      // capture
                                      ...
                                      // ce
                                      // changement
                                      int emptySoulStone = f.getPersonnage().getObjetByPos(Constant.ITEM_POS_ARME)
                                              .getGuid();
          ...
          if (this.getType() == Constant.FIGHT_TYPE_PVM || this.getType() == Constant.FIGHT_TYPE_DOPEUL) {
              for (Fighter fighter : winners) {
                  ...
                  player.verificarPeleasLogros(mobGroup.getAlmasMobs());
                  World.get_Succes(player.getId()).verifyXMob(player);
                  World.get_Succes(player.getId()).verifyDuoMob(player, winners.size(), mobGroup.getAlmasMobs());
                      ...
                      for (Fighter ennemy : loosers) {
                          ...
                          try {
                              for (QuestPlayer questP : player.getQuestPerso().values()) {
                                  ...
                                  quest.getQuestSteps().stream()
                                          .filter(qEtape -> !questP.isQuestStepIsValidate(qEtape)
                                                  && (qEtape.getType() == 0 || qEtape.getType() == 6))
                                          .filter(qEtape -> qEtape.getMonsterId() == ennemy.getMob().getTemplate()
                                                  .getId())
                                          .forEach(qEtape -> {
                                              try {
                                                  player.getQuestPersoByQuest(qEtape.getQuestData()).getMonsterKill()
          ...
          if (amande || rousse || doree) {
              winners.stream()
                      .filter(fighter -> !fighter.isInvocation() && fighter.haveState(Constant.ETAT_APPRIVOISEMENT))
                      .forEach(F -> getTrainer().add(F));
              if (getTrainer().size() > 0) {
                  for (int i = 0; i < getTrainer().size(); i++) {
                      try {
                          Fighter f = getTrainer().get(Formulas.getRandomValue(0, getTrainer().size() - 1)); // Rï¿½cupï¿½re
                          // un
                          // captureur
                          // au
                          // hasard
                          ...
                          if (player.getObjetByPos(Constant.ITEM_POS_ARME) == null
                                  || !(player.getObjetByPos(Constant.ITEM_POS_ARME).getTemplate()
                                  .getType() == Constant.ITEM_TYPE_FILET_CAPTURE)) {
                              getTrainer().remove(f);
                              continue;
                          }
          ...
          float starFactor = this.getMobGroup() != null
                  ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus())
                  : 0;

          // initialize PP variables
          for (Fighter fighter : winners) {
              if (fighter != null && !fighter.isDouble()) {
                  if (!fighter.isInvocation() || (fighter.getMob() != null && fighter.getMob().getTemplate() != null
                          && fighter.getMob().getTemplate().getId() == 285)) {
          ...
          while (temporary1.size() < winners.size()) {
              ...
              for (Fighter fighter : winners) {
                  if (fighter.getMob() != null && fighter.getMob().getTemplate() != null
                          && fighter.getMob().getTemplate().getId() == 285 && fighter.getInvocator() != null) {
                      if (fighter.getInvocator().getTotalStats()
                              .getEffectSummed(Constant.STATS_ADD_PROS) > currentProspecting
                              && !temporary1.contains(fighter)) {
                          higherFighter = fighter;
                          currentProspecting = fighter.getInvocator().getTotalStats()
                                  .getEffectSummed(Constant.STATS_ADD_PROS);
                      }
                  } else if (fighter.getTotalStats().getEffectSummed(Constant.STATS_ADD_PROS) > currentProspecting
                          && !temporary1.contains(fighter)) {
                      higherFighter = fighter;
                      currentProspecting = fighter.getTotalStats().getEffectSummed(Constant.STATS_ADD_PROS);
                  }
          ...
          if (this.getType() == Constant.FIGHT_TYPE_AGRESSION) {
              /*
               * int totalLoserRank=0; int losingPlayers=0; for(Fighter fighter : loosers)
               * if(fighter.getPersonnage()!=null) { losingPlayers++;
               * if(fighter.getPersonnage().get_align()!=0)
               * totalLoserRank+=fighter.getPersonnage().getGrade(); }
               *
               * int totalWinnerRank=0; int winningPlayers=0; for(Fighter fighter : winners)
               * if(fighter.getPersonnage()!=null) { winningPlayers++;
               * if(fighter.getPersonnage().get_align()!=0)
               * totalWinnerRank+=fighter.getPersonnage().getGrade(); }
               *
               * float averageLoserRank=totalLoserRank/losingPlayers;
               * if(totalWinnerRank<=totalLoserRank&&averageLoserRank>=2) { int
               * tokenReward=(int)Math.floor(4*totalLoserRank/winningPlayers); // for(Fighter
               * f : winners) // if(f.getPersonnage()!=null) //
               * f.getPersonnage().setNeededEndFightAction(new
               * Action(475,Integer.toString(tokenReward),"",null)); //token reward }
               */
              ...
              if (isAlone) {
                  for (Fighter fighter : loosers) {
                      if (!fighter.isInvocation() && curPlayer != null && curPlayer.get_traque() != null
                              && curPlayer.get_traque().getTraque() == fighter.getPersonnage()) {
                          SocketManager.GAME_SEND_MESSAGE(curPlayer,
                                  "Thomas Sacre : Le contrat a ete execute, venez me voir pour reclamer votre recompense.",
                                  "000000");
          ...
          /*
           * if(this.getType()==Constant.FIGHT_TYPE_CONQUETE) { int totalLoserRank=0; int
           * losingPlayers=0; for(Fighter fighter : loosers)
           * if(fighter.getPersonnage()!=null) { losingPlayers++;
           * if(fighter.getPersonnage().get_align()!=0)
           * totalLoserRank+=fighter.getPersonnage().getGrade(); }
           *
           ...
           * float averageLoserRank=totalLoserRank/losingPlayers;
           * if(totalWinnerRank<=totalLoserRank&&averageLoserRank>=2) { int
           * tokenReward=(int)Math.floor(4*totalLoserRank/winningPlayers); for(Fighter f :
           * winners) if(f.getPersonnage()!=null)
           * f.getPersonnage().setNeededEndFightAction(new
           * Action(475,Integer.toString(tokenReward),"",null)); //token reward } }
           */
                          ...
                          if (team) { // Players have loose the fight, mob win the fight
                              ...
                              if (group.isFix()) {
                                  ...
                                  if (RespawnGroup.fixMobGroupObjects.get(infos) != null) {
                                      ...
                                      RespawnGroup.fixMobGroupObjects.put(infos,
                                              new Pair<ArrayList<GameObject>, Integer>(objects, 0));
                                  } else {
                                      RespawnGroup.fixMobGroupObjects.put(infos,
                                              new Pair<ArrayList<GameObject>, Integer>(objects, 0));
                              ...
                              if (group.isFix()) {
                                  if (RespawnGroup.fixMobGroupObjects
                                          .get(this.getMapOld().getId() + "," + group.getCellId()) != null) {
                                      list = Fight.give(
                                              RespawnGroup.fixMobGroupObjects
                                                      .get(this.getMapOld().getId() + "," + group.getCellId()).getLeft(),
                                              winners);
                                      RespawnGroup.fixMobGroupObjects.get(this.getMapOld().getId() + ","
                                              + group.getCellId()).left = new ArrayList<GameObject>();
                                  }
                              }
                              if (!group.isFix()) {
                              ...
                              if (map.getId() != 8338 && map.getId() != 8340 && map.getId() != 8342 && map.getId() != 8344
                                      && map.getId() != 8345 && map.getId() != 8347) // firefoux
                              {
                                  if (!group.isFix()) {
                                      this.getMapOld().spawnAfterTimeGroup(this.getMapOld().getMinRespawnTime(),
                                              this.getMapOld().getMaxRespawnTime());
                                  }
                              }
                          }
                      } catch (Exception e) {
              ...
              if (this.getType() == Constant.FIGHT_TYPE_PVM) {
                  try {
                      ...
                      } else // players won, spawn new group
                      {
                          ...
                          // else
                          if (!group.isFix()) {
                              if (map.getId() != 8338 && map.getId() != 8340 && map.getId() != 8342
                                      && map.getId() != 8344 && map.getId() != 8345 && map.getId() != 8347) // firefoux
                              {
                                  this.getMapOld().spawnAfterTimeGroup(this.getMapOld().getMinRespawnTime(),
                                          this.getMapOld().getMaxRespawnTime());
                              }
          ...
          for (Fighter i : winners) {
              ...
              if (player != null && player.ipDrop) {
                  for (Fighter ipDropper : ipDroppers.keySet()) {
                      if (player.getAccount().getCurrentIp()
                              .compareTo(ipDropper.getPersonnage().getAccount().getCurrentIp()) == 0) // same IP of
                      // two
                      // IpDroppers,
                      // do not add
                      {
              ...
              if (getType() == Constant.FIGHT_TYPE_PVT || getType() == Constant.FIGHT_TYPE_PVM
                      || getType() == Constant.FIGHT_TYPE_CHALLENGE || getType() == Constant.FIGHT_TYPE_DOPEUL
                      || getType() == Constant.FIGHT_TYPE_KOLI) {
                  String drops = "";
                  ...
                  if (!this.STAFF) {
                      /** Xp,kamas **/
                      if (player != null) {
                          xpPlayer = Formulas.getXp(i, winners, totalXP, nbbonus,
                                  /*
                                   * this.getMobGroup()!=null ?
                                   * getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) : 0
                                   */0, challXp, lvlMax, lvlMin, lvlLoosers, lvlWinners,
                                  Main.world.getConquestBonus(player));
                          // atsuka xp difficulty
                          if (this.type == Constant.FIGHT_TYPE_PVM) {
                              if (getMobGroup() != null) {
                                  long starbonus = (1
                                          + (getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus()) / 100));
                                  player.sendMessage("xp prévu sans étoiles : " + xpPlayer);

                                  xpPlayer *= starbonus;
                                  player.sendMessage(
                                          "Bonus des étoiles = " + starbonus + " | XP finale : " + xpPlayer);
                              }
                              /*
                               * if(player.getAccount().getSubscribeRemaining() != 0L)xpPlayer = (long)
                               * (xpPlayer+(xpPlayer*0.25));
                               */
                          ...
                          if (this.getType() == Constant.FIGHT_TYPE_PVT && win == 1) {
                              ...
                              // player.setNeededEndFightAction(new Action(475,"20","",null)); //20 token
                              // reward
                          } else {
                  ...
                  winKamas = (int) ((this.getType() == Constant.FIGHT_TYPE_PVT && win == 1)
                          ? Math.floor(kamas.getLeft() / winners.size())
                          : Formulas.getKamasWin(i, winners, kamas.getLeft(), kamas.getRight()));
                  if (this.STAFF) {
                      winKamas = 0;
                  }
                  /** Xp,kamas **/
                  ...
                  if (!this.STAFF) {
                      if (this.getType() == Constant.FIGHT_TYPE_PVT && win == 1 && dropsCollector != null) {
                          ...
                          temporary3.addAll(Main.world
                                  .getEtherealWeapons(i.isInvocation() ? i.getInvocator().getLvl() : i.getLvl())
                                  .stream().map(objectTemplate -> new Drop(objectTemplate.getId(), 0.001, 0))
                          ...
                          for (Drop drop : temporary3) {
                              ...
                              if (this.type == Constant.FIGHT_TYPE_PVM) {
                                  if (Config.singleton.serverId == 3001 || Config.singleton.serverId == 3002) {
                                      // if(player!=null&&player.getAccount().getSubscribeRemaining() != 0L)charPros =
                                      // (int) (charPros+(charPros*0.25));
                                      if (MONO) {
                                          charPros = (int) (charPros + (charPros * 0.10));
                                      }
                                      if (DUO) {
                                          charPros = (int) (charPros + (charPros * 0.20));
                                      }
                                      if (TRIO) {
                                          charPros = (int) (charPros + (charPros * 0.23));
                                      }
                                      if (QUADRO) {
                                          charPros = (int) (charPros + (charPros * 0.27));
                                      }
                                      if (CINQO) {
                                          charPros = (int) (charPros + (charPros * 0.32));
                                      }
                                      if (SIXTO) {
                                          charPros = (int) (charPros + (charPros * 0.38));
                                      }
                                      if (SEVENTO) {
                                          charPros = (int) (charPros + (charPros * 0.45));
                                      }
                                      if (TEAM) {
                                          charPros = (int) (charPros + (charPros * 0.60));
                                      }
                                  }
                              }
                              float soloBonus = Formulas.getSoloBonus(winners);

                              final double jet = Double
                                      .parseDouble(formatter.format(Math.random() * 100).replace(',', '.'));
                              double chance = Formulas.dropChance((float) drop.getLocalPercent(), starFactor,
                                      challengeFactor, (float) Main.world.getConquestBonus(player), (float) charPros,
                                      soloBonus);

                              if (this.fightdifficulty == 0) {
                                  chance = chance * 1;
                                  // test drop jeton atsuka
                                  // if (this.gra
                              }
                              if (this.fightdifficulty == 1) {
                                  chance = chance * 1.25;
                              }
                              if (this.fightdifficulty == 2) {
                                  chance = chance * 1.40;
                              }

                              boolean ok = true;
                              if (drop.getObjectId() == 686) {
                              }
                              if (player != null) {
                                  switch (drop.getAction()) {
                                      case 4:
                                          ...
                                          break;
                                      case 12:
                                          if (!ConditionParser.validConditions(player, "PL>" + drop.getCondition())) {
                                              continue;
                                          }
                                  }
                              }
                              if (jet < chance && ok) // item gained // replace charria et par ou
                              {
                                  ...
                                  /*
                                   * if((objectTemplate.getType() >= 1 && objectTemplate.getType() <= 11) ||
                                   * (objectTemplate.getType() >= 16 && objectTemplate.getType() <= 23) ||
                                   * (objectTemplate.getType() >= 81 && objectTemplate.getType() <= 82) ||
                                   * objectTemplate.getType() == 85 || objectTemplate.getType() == 113 ||
                                   * objectTemplate.getType() == 84 || objectTemplate.getType() == 100 ||
                                   * objectTemplate.getType() == 73 || objectTemplate.getType() == 13 ||
                                   * objectTemplate.getType() == 41){ }else if(Config.getInstance().HEROIC) {
                                   * max_drop = max_drop*3; } else { max_drop =
                                   * max_drop*Config.getInstance().rateDrop; }
                                   */

                                  quantity = drop.getMinDrop()
                                          + (int) Math.rint(Math.random() * (max_drop - drop.getMinDrop()));
                                  boolean itsOk = false, unique = false;

                                  if (objectTemplate.getType() == 23 || objectTemplate.getId() == 12324
                                          || objectTemplate.getId() == 101620) {
                                  ...
                                  int droptest = drop.getObjectId();
                                  if (droptest == 71808) {
                                      System.out.println("drop ok");
                                  }
                                  System.out.println("id : " + drop.getObjectId() + " Action : " + drop.getAction()
                                          + "Condition : " + drop.getCondition());

                                  switch (drop.getAction())// 71808 1057
                                  {
                                      case -2:
                                          itsOk = true;
                                          break;
                                      ...
                                      case 8:// Win a specific quantity
                                          ...
                                          quantity = Formulas.getRandomValue(Integer.parseInt(split[0]),
                                      ...
                                      case 19:// monster level
                                          String lvlcond = drop.getCondition();
                                          int lvlcnd = Integer.parseInt(lvlcond);
                                          int levelMax = 0;

                                          for (Fighter F : getFighters(2)) {
                                              if (F.getMob().getLevel() >= lvlcnd) {
                                                  levelMax = F.getMob().getLevel();
                                              }
                                          }
                                          if (levelMax == 0) {
                                              break;
                                          ...
                                          break;
                                      case 13:// difficulty
                                          String diffcond = drop.getCondition();
                                          int diffcnd = 0;
                                          try {
                                              diffcnd = Integer.parseInt(diffcond);
                                          } catch (Exception e) {
                                              e.printStackTrace();
                                              System.out
                                                      .println("condition drop 13 non respecté check BDD pour l'objet : "
                                                              + drop.getObjectId());
                                          }
                                          if (diffcnd == 0) {
                                              break;
                                          ...
                                          for (Fighter F : getFighters(2)) {
                                              if (F.getFight().fightdifficulty == diffcnd) {
                                                  itsOk = true;
                                              }
                                              System.out.println("Drop condition difficulty OK for Diff : " + diffcnd);
                                          }
                                          break;
                                      case 15 : // diff 2 + level 180 +
                                          diffcond = drop.getCondition();
                                          diffcnd = 0;
                                          try {
                                              diffcnd = Integer.parseInt(diffcond);
                                          } catch (Exception e) {
                                              e.printStackTrace();
                                              System.out
                                                      .println("condition drop 15 non respecté check BDD pour l'objet : "
                                                              + drop.getObjectId());
                                          }
                                          if (diffcnd == 0) {
                                              break;
                                          ...
                                          for (Fighter F : getFighters(2)) {
                                              if ((F.getFight().fightdifficulty == diffcnd) && (F.getMob().getLevel() > 180)) {
                                                  itsOk = true;
                                              }
                                              System.out.println("Drop condition difficulty OK for Diff : " + diffcnd);
                                          }
                                      ...
                                      case 14: // combo 12&13 ( format 14|lvl,diff
                                          int cmpt = 0;
                                          String lvlcond1 = drop.getCondition().split(",")[0].trim();
                                          String dfcond = drop.getCondition().split(",")[1].trim();
                                          int lvl = Integer.parseInt(lvlcond1);
                                          int diff = Integer.parseInt(dfcond);

                                          int levelMax1 = 0;

                                          for (Fighter F : getFighters(2)) {
                                              if (F.getMob().getLevel() > lvl) {
                                                  levelMax1 = F.getMob().getLevel();
                                              }
                                          }
                                          if (levelMax1 == 0) {
                                              break;
                                          ...
                                          for (Fighter F : getFighters(2)) {
                                              if (F.getFight().fightdifficulty == diff) {
                                                  itsOk = true;
                                                  cmpt++;
                                              }
                                          }
                                  ...
                                  if (itsOk) {
                                      // mark dark generation drop
                                      objectsWon.put(objectTemplate.getId(),
                                              (objectsWon.get(objectTemplate.getId()) == null ? quantity
                                                      : (objectsWon.get(objectTemplate.getId())) + quantity));
                                      if (unique) {
                          ...
                          if (player != null) {
                              ...
                              GameObject weapon = player.getObjetByPos(Constant.ITEM_POS_ARME);
                              boolean ok = weapon != null && weapon.getStats().getEffectSummed(795) == 1;
                                  ...
                                  for (Drop drop : temporary) {
                                      final double jet = Double
                                              ...
                                              chance = Double.parseDouble(
                                                      formatter.format(drop.getLocalPercent() * (i.getPros() / 100.0))
                                                              .replace(',', '.'));

                                      if (jet < chance) {
                                          ObjectTemplate objectTemplate = Main.world
                                                  .getObjTemplate(drop.getObjectId());

                                          if (drop.getAction() == 1 && objectTemplate != null
                                                  && player.getMetierByID(41) != null
                                                  && player.getMetierByID(41).get_lvl() >= drop.getLevel()) {
                                              itemWon2.put(objectTemplate.getId(),
                                                      (itemWon2.get(objectTemplate.getId()) == null ? 0
                                                              : itemWon2.get(objectTemplate.getId())) + 5);
                                          }
                                      }
                                  }
                              }
                  ...
                  if (player != null || (i.getMob() != null && i.getMob().getTemplate().getId() == 285)) {
                      if (player != null) {
                          if (this.getTrainerWinner() != -1 && i.getId() == this.getTrainerWinner()
                      ...
                      for (Entry<Integer, Integer> entry : objectsWon.entrySet()) {
                          ...
                          // Charria med, no drop songe
                          if (Config.singleton.serverId == 3001 || Config.singleton.serverId == 3002
                                  || Config.singleton.serverId == 1|| Config.singleton.serverId == 2) {
                              if (this.getMap().getId() >= 31000 && this.getMap().getId() <= 31031) {
                                  playerTarget.setCanDrop(false);
                                  continue;
                              } else {
                                  playerTarget.setCanDrop(true); // 3ade zedte hadi bache nchoffe
                              }

                          }
                          if (playerTarget != null) {
                              if (objectTemplate.getType() == 23 && objectTemplate.getId() != 7113) {
                                  SocketManager.GAME_SEND_Im_PACKET_TO_ALL("116;" + "<b>Server</b>"
                                          + "~ON dirait que la chance sourit a [" + playerTarget.getName()
                                          + "] Il vient de drop [" + objectTemplate.getName() + "]");
                              }

                              switch (objectTemplate.getId()) {
                                  case 12324:
                                  case 101620:
                                  case 10166:
                                  case 10159:
                          ...
                          if (objectTemplate.getType() == 32 && player != null) {
                              ...
                              if (objectTemplate.getType() == Constant.ITEM_TYPE_FAMILIER) {
                                  ...
                                  newObj = objectTemplate.createNewItemWithoutDuplication(
                                          playerTarget.getItems().values(), entry.getValue(), false);
                                  if (objectTemplate.getType() == Constant.ITEM_TYPE_CERTIF_MONTURE) {
                                      // obj.setMountStats(this.getPlayer(), null);
                                      Mount mount = new Mount(
                                              Constant.getMountColorByParchoTemplate(newObj.getTemplate().getId()),
                                              playerTarget.getId(), false);
                                      newObj.clearStats();
                      ...
                      for (Entry<Integer, Integer> entry : itemWon2.entrySet()) {
                          ...
                          GameObject newObj = Main.world.getObjTemplate(objectTemplate.getId())
                                  .createNewItemWithoutDuplication(target.getItems().values(), entry.getValue(),
                                          false);
                      ...
                      if (this.getType() == Constant.FIGHT_TYPE_PVM && player != null) {
                           ...
                           * switch(mob.getTemplate().getId()) { case 793: bouftou++; break; case 794:
                           * tofu++; break; case 289: //if(player.getCurMap().getSubArea().getId()==211)
                           * // MobGroup.MAITRE_CORBAC.repop(player.getCurMap().getId()); break; } }
                           *
                           * if(Config.getInstance().HALLOWEEN) {
                           * if((bouftou>0||tofu>0)&&!player.hasEquiped(976)) { if(bouftou>tofu) {
                           * drops+=(drops.length()>0 ? "," : "")+"8169~1"; player.setMalediction(8169);
                           * player.setFullMorph(Formulas.getRandomValue(16,20),false,false); } else
                           * if(tofu>bouftou) { drops+=(drops.length()>0 ? "," : "")+"8170~1";
                           * player.setMalediction(8170);
                           * player.setFullMorph(Formulas.getRandomValue(21,25),false,false); } else {
                           * switch(Formulas.getRandomValue(1,2)) { case 1: drops+=(drops.length()>0 ? ","
                           * : "")+"8169~1"; player.setMalediction(8169);
                           * player.setFullMorph(Formulas.getRandomValue(16,20),false,false); break; case
                           * 2: drops+=(drops.length()>0 ? "," : "")+"8170~1";
                           * player.setMalediction(8170);
                           * player.setFullMorph(Formulas.getRandomValue(21,25),false,false); break; } } }
                           * }
                           */
                          ...
                          switch (player.getCurMap().getId()) {
                              case 8984:
                                  ...
                                  if (Main.world.getObjTemplate(8012).getType() == Constant.ITEM_TYPE_CERTIF_MONTURE) {
                                      // obj.setMountStats(this.getPlayer(), null);
                                      Mount mount = new Mount(
                                              Constant.getMountColorByParchoTemplate(obj.getTemplate().getId()),
                                              player.getId(), false);
                                      obj.clearStats();
                      ...
                      if (player != null) {
                          xpPlayer = XP.get();
                          if (xpPlayer != 0) {
                              if (Config.singleton.serverId == 3001) {
                                  // test xp degressif atsukaka
                                  if (player.getLevel() < 61) {
                                      xpPlayer -= (xpPlayer * 3) / 100;
                                  } else if (player.getLevel() < 101 && player.getLevel() > 60) {
                                      xpPlayer -= (xpPlayer * 6) / 100;
                                  } else if (player.getLevel() < 121 && player.getLevel() > 100) {
                                      xpPlayer -= (xpPlayer * 9) / 100;
                                  } else if (player.getLevel() < 151 && player.getLevel() > 120) {
                                      xpPlayer -= (xpPlayer * 12) / 100;
                                  } else if (player.getLevel() < 181 && player.getLevel() > 150) {
                                      xpPlayer -= (xpPlayer * 18) / 100;
                                  } else if (player.getLevel() < 191 && player.getLevel() > 180) {
                                      xpPlayer -= (xpPlayer * 30) / 100;
                                  } else if (player.getLevel() < 199 && player.getLevel() > 190) {
                                      xpPlayer -= (xpPlayer * 40) / 100;
                                  } else if (player.getLevel() < 200 && player.getLevel() > 198) {
                                      xpPlayer -= (xpPlayer * 50) / 100;
                                  }
                              }
                              // ici prestige7754
                              if (player.getPrestige() > 0) {
                                  switch (player.getPrestige()) {
                                      case 1: // prestige 1 tu continue jusqu'a 10
                                          xpPlayer -= (xpPlayer * 3) / 100; // Diminution de 3% premier prestige
                                          break;
                                      case 2:
                                          xpPlayer -= (xpPlayer * 6) / 100;
                                          break;
                                      ...
                                      case 4:
                                          ...
                                          break;
                                      case 5:
                                          xpPlayer -= (xpPlayer * 15) / 100;
                                          break;
                                      case 6:
                                          xpPlayer -= (xpPlayer * 18) / 100;
                                          break;
                                      case 7:
                                          xpPlayer -= (xpPlayer * 21) / 100;
                                          break;
                                      case 8:
                                          xpPlayer -= (xpPlayer * 24) / 100;
                                          break;
                                      case 9:
                                          xpPlayer -= (xpPlayer * 27) / 100;
                                          break;
                                      case 10:
                                          xpPlayer -= (xpPlayer * 30) / 100;
                                          break;
                                      case 11:
                                          xpPlayer -= (xpPlayer * 80) / 100;
                                          break;
                                      case 12:
                                          xpPlayer -= (xpPlayer * 85) / 100;
                                          break;
                                      case 13:
                                          xpPlayer -= (xpPlayer * 90) / 100;
                                          break;
                                      case 14:
                                          xpPlayer -= (xpPlayer * 95) / 100;
                                          break;
                                      case 15:
                                          xpPlayer -= (xpPlayer * 99) / 100;
                                          break;
                                  }
                              }
                              if (player.getMorphMode()) {
                      ...
                      if (winKamas != 0 && i.isInvocation() && !i.isDouble()
                      ...
                      for (Fighter fighter : winners) {
                          if (fighter.getPersonnage() != null) {
                              if (fighter.getPersonnage().ipDrop && fighter.getPersonnage() != player
                                      && fighter.getPersonnage().getAccount().getCurrentIp()
                                      .equals(player.getAccount().getCurrentIp())) {
                                  target = fighter;
                                  break;
                              }
                          }
                  ...
                  if (this.getType() == Constant.FIGHT_TYPE_KOLI) {
                      ...
                      temporary.append("2;").append(i.getId()).append(";").append(i.getPacketsName()).append(";")
                              .append(i.getLvl()).append(";").append((i.isDead() ? "1" : "0")).append(";");
                      ...
                      temporary.append(";").append(Formulas.getRandomValue(kamas.getLeft(), kamas.getRight()))
                              .append(";0;0;0;0|");
                  ...
                  if (this.getType() == Constant.FIGHT_TYPE_AGRESSION) {
                      ...
                      if (this.getType() == Constant.FIGHT_TYPE_AGRESSION) {
                          if (getInit1().getPersonnage().get_align() != 0
                                  && getInit0().getPersonnage().get_align() != 0) {
                              if (getInit1().getPersonnage().getAccount().getCurrentIp()
                                      .compareTo(getInit0().getPersonnage().getAccount().getCurrentIp()) != 0) {
                      ...
                      temporary.append("2;").append(i.getId()).append(";").append(i.getPacketsName()).append(";")
                              .append(i.getLvl()).append(";").append((i.isDead() ? "1" : "0")).append(";");
                      temporary.append(player.get_align() != Constant.ALIGNEMENT_NEUTRE
                              ? Main.world.getExpLevel(player.getGrade()).pvp
                              : 0).append(";");
                      ...
                      if (player != null) {
                          ...
                          temporary.append(player.get_align() != Constant.ALIGNEMENT_NEUTRE
                                  ...
                                  : 0).append(";");
                          temporary.append(player.get_honor()).append(";");
                          temporary.append(player.get_align() != Constant.ALIGNEMENT_NEUTRE ? maxHonor : 0)
                                  .append(";");
                          temporary.append(winH).append(";");
                          temporary.append(player.getGrade()).append(";");
                          temporary.append(player.getDeshonor()).append(";");
          ...
          winners.stream().filter(i -> i.isInvocation() && i.getMob() != null)
                  .filter(i -> i.getMob().getTemplate().getId() == 285)
          ...
          winners.stream()
                  .filter(fighter -> !(fighter.isInvocation() && fighter.getMob() != null
                          && fighter.getMob().getTemplate().getId() != 285))
                  .filter(fighter -> !fighter.isDouble() && gains.get(fighter.getId()) != null)
          ...
          for (Fighter i : loosers) {
              ...
              if (this.getType() != Constant.FIGHT_TYPE_AGRESSION && this.getType() != Constant.FIGHT_TYPE_CONQUETE) {
                  ...
                  if (i.getPdv() == 0 || i.hasLeft() || i.isDead()) {
                      temporary.append("0;").append(i.getId()).append(";").append(i.getPacketsName()).append(";")
                              .append(i.getLvl()).append(";1").append(";").append(i.xpString(";")).append(";;;;|");
                  } else {
                      temporary.append("0;").append(i.getId()).append(";").append(i.getPacketsName()).append(";")
                              .append(i.getLvl()).append(";0").append(";").append(i.xpString(";")).append(";;;;|");
                  }
                  ...
                  if (this.getType() == Constant.FIGHT_TYPE_AGRESSION) {
                      ...
                      packet.append("0;").append(i.getId()).append(";").append(i.getPacketsName()).append(";")
                              .append(i.getLvl()).append(";").append((i.isDead() ? "1" : "0")).append(";");
                      packet.append(player.get_align() != Constant.ALIGNEMENT_NEUTRE
                              ? Main.world.getExpLevel(player.getGrade()).pvp
                              : 0).append(";");
          ...
          if (!this.STAFF) {
              if (Collector.getCollectorByMapId(getMap().getId()) != null && getType() == Constant.FIGHT_TYPE_PVM) {
                  ...
                  long winxp = Formulas.getXp(collector, winners, totalXP, nbbonus,
                          (getMobGroup() != null ? getMobGroup().getStarBonus(getMobGroup().getInternalStarBonus())
                                  : 0),
                          challXp, lvlMax, lvlMin, lvlLoosers, lvlWinners, 0) / 15;
                  // atsuka xp difficulty
                  ...
                  packet.append("5;").append(collector.getId()).append(";").append(collector.getFullName())
                          .append(";").append(Main.world.getGuild(collector.getGuildId()).getLvl()).append(";0;");
                  ...
                  if (collector.getPodsTotal() < collector.getMaxPod()) {
                      for (Drop drop : temporary) {
                          final double jet = Double
                                  .parseDouble(formatter.format(Math.random() * 100).replace(',', '.')),
                                  chance = (int) (drop.getLocalPercent()
                                          * (Main.world.getGuild(collector.getGuildId())
                                          .getStats(Constant.STATS_ADD_PROS) / 100.0));
                          ...
                          if (jet < chance) {
                              ...
                              if (itsOk) {
                                  objectsWon.put(objectTemplate.getId(),
                                          (objectsWon.get(objectTemplate.getId()) == null ? 0
                                                  : objectsWon.get(objectTemplate.getId())) + 1);
                      ...
                      for (Entry<Integer, Integer> entry : objectsWon.entrySet()) {
                          ...
                          if (objectTemplate == null || collector.getPodsTotal()
                                  + objectTemplate.getPod() * entry.getValue() >= collector.getMaxPod()) {
                              continue;
                          }
                          if (drops.length() > 0) {
                              drops += ",";
                          ...
                          GameObject newObj = Main.world.getObjTemplate(objectTemplate.getId())
                                  .createNewItemWithoutDuplication(collector.getOjects().values(), entry.getValue(),
                                          false);
                          int guid = newObj.getGuid();// FIXME: Ne pas recrÃ©e un item pour l'empiler aprÃ¨s
          ...
          e.printStackTrace();
          System.out.println("An error occurred when server went to give the 'GE' packet : " + e.getMessage() + " "
  ...
  /*      */   public String parseFightInfos() {
	  /* 6144 */     StringBuilder infos = new StringBuilder();
	  ...
	  /*      */       case 0:
	  /* 6152 */         infos.append("0,");
	  /* 6153 */         infos.append(getTeamSizeWithoutInvocation(getTeam0().values())).append(";");
	  /*      */         
	  /* 6155 */         infos.append("0,");
	  /* 6156 */         infos.append("0,");
	  /* 6157 */         infos.append(getTeamSizeWithoutInvocation(getTeam1().values())).append(";");
	  /*      */         break;
	  /*      */       
	  /*      */       case 1:
	  /* 6161 */         infos.append(getInit0().getPersonnage().get_align()).append(",");
	  /* 6162 */         infos.append(getTeam0().size()).append(";");
	  /*      */         
	  /* 6164 */         infos.append("0,");
	  /* 6165 */         infos.append(getInit1().getPersonnage().get_align()).append(",");
	  /* 6166 */         infos.append(getTeamSizeWithoutInvocation(getTeam1().values())).append(";");
	  /*      */         break;
	  /*      */       
	  /*      */       case 2:
	  /* 6170 */         infos.append(getInit0().getPersonnage().get_align()).append(",");
	  /* 6171 */         infos.append(getTeamSizeWithoutInvocation(getTeam0().values())).append(";");
	  /*      */         
	  /* 6173 */         infos.append("0,");
	  /* 6174 */         infos.append(getPrism().getAlignement()).append(",");
	  /* 6175 */         infos.append(getTeamSizeWithoutInvocation(getTeam1().values())).append(";");
	  /*      */         break;
	  /*      */       
	  /*      */       case 4:
	  /* 6179 */         infos.append("0,");
	  /* 6180 */         infos.append(getTeamSizeWithoutInvocation(getTeam0().values())).append(";");
	  /*      */         
	  /* 6182 */         infos.append("1,");
	  /* 6183 */         if (getTeam0().isEmpty()) {
	  /* 6184 */           infos.append("0,");
	  /*      */         } else {
	  /* 6186 */           infos.append(((Fighter)getTeam1().get(getTeam1().keySet().toArray()[0])).getMob().getTemplate().getAlign()).append(",");
	  /* 6187 */         }  infos.append(getTeamSizeWithoutInvocation(getTeam1().values())).append(";");
	  /*      */         break;
	  /*      */       
	  /*      */       case 3:
	  /* 6191 */         infos.append("0,");
	  /* 6192 */         infos.append(getTeamSizeWithoutInvocation(getTeam0().values())).append(";");
	  /*      */         
	  /* 6194 */         infos.append("1,");
	  /* 6195 */         if (getTeam0().isEmpty()) {
	  /* 6196 */           infos.append("0,");
	  /*      */         } else {
	  /* 6198 */           infos.append(((Fighter)getTeam1().get(getTeam1().keySet().toArray()[0])).getMob().getTemplate().getAlign()).append(",");
	  /* 6199 */         }  infos.append(getTeamSizeWithoutInvocation(getTeam1().values())).append(";");
	  /*      */         break;
	  /*      */       
	  /*      */       case 5:
	  /* 6203 */         infos.append("0,");
	  /* 6204 */         infos.append(getTeamSizeWithoutInvocation(getTeam0().values())).append(";");
	  /*      */         
	  /* 6206 */         infos.append("4,");
	  /* 6207 */         infos.append("0,");
	  /* 6208 */         infos.append(getTeamSizeWithoutInvocation(getTeam1().values())).append(";");
	  /*      */         break;
	  /*      */     } 
	  /* 6211 */     return infos.toString();
	  /*      */   }
  ...
  /*      */   public boolean cancast(Fighter fighter) {
	  /* 6265 */     if (this.turn != null && System.currentTimeMillis() - this.turn.getStartTime() >= 45000L) return false; 
	  /* 6266 */     return true;
	  /*      */   }

/*      */   public void playerPass(Fighter fighter) 
	 {
		    //final Fighter fighter=getFighterByPerso(player);
		    if(fighter!=null)
		        if (fighter.canPlay() && !this.isCurAction() && !this.isTraped())
    ...
    public boolean canLaunchSpell(Fighter caster, Spell.SortStats spell, GameCase cell) {
        if (spell == null)
            return false;
    ...
    public ArrayList<Fighter> getAllFighters() {// Entre 0 et 7, binaire([spec][t2][t1]).
        ArrayList<Fighter> fighters = new ArrayList<>();
